...
theme: indaco
paginate: true
header: "Tideline · OKR review Q2 2026"
---
...
- Active buyers to 18,000 `100%` `on-track`
- Buyer activation rate to 62% `95%` `on-track`
- Repeat-purchase rate to 48% `88%` `on-track`
...
- Verified suppliers to 4,200 `92%` `on-track`
- Dispute rate below 1.5% `100%` `on-track`
- Buyer trust score to 4.5 `78%` `at-risk`
...
- Median time-to-fill under 48h `60%` `at-risk`
- Order fill rate to 90% `45%` `blocked`
- Supplier payout under 24h `85%` `on-track`
...
- 71%
  - order fill rate against a 90% target — too few suppliers covering high-demand categories.
...
1. 90%
   - Order fill rate
   - up from 71% `Target` `Supply`
2. 1,200
   - New suppliers (focus cats)
   - 3 underserved categories `Target` `Growth`
3. 36h
   - Median time-to-fill
   - down from 61h `Target` `Ops`
...
`Q3 2026 · Liquidity plan`

## How we close the liquidity gap, by workstream.

| Workstream | Now `Q2` | Build `Q3` | Scale `Q4` |
| --- | --- | --- | --- |
| Supplier growth | [x] 4,200 verified | [-] Focus-category drive | [ ] Referral engine |
| Matching | [x] Basic search | [-] Demand-aware routing | [ ] Predictive restock |
...
## Two wins, one focus — liquidity is Q3.
