...
- If a fix is non-obvious, add a `reference/engineering/gotchas.md` entry **before** committing. The commit message then links to it.
...
For inner-loop iteration, scoped test scripts (`test:palette`, `test:components`, …), `test:watch`, the pre-commit / pre-push / commit-msg hooks, coverage, and the integration-test cache all live in `reference/engineering/development.md`. That file is the tooling reference; this one is the process reference.
