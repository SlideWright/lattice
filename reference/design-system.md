...
to use) lives in `reference/skill.md` and the visual gallery; it is not
...
colors. See `design.md` §1.3 and `reference/theming.md`.
...
- `reference/design-system.gallery.md` — the slide-rendered demo of this doc.
...
- **`lib/_legacy.css` fully retired.** The 5,938-line monolith was split across 7 phases into 8 new source files (`_root.css`, `_base.css`, `_modifiers.css`, `_syntax-highlight.css`, `_chart-family.css`, `_backgrounds.css`, `_semi-universal.css`, `_diagram-overrides.css`) + 17 component folders. Bundle position of every block preserved to maintain cascade outcomes. See `reference/notes/2026-05-16-post-foundation-followups.md` for the open follow-ups (specificity-bump hacks introduced during extraction, @layer activation as the principled retirement path).
...
**Still deferred (see `reference/notes/2026-05-18-component-reorg-and-modular-css.md`):**
...
- **Broad `@layer` activation — blocked, not deferred.** The Phase 3.5
  ...
  source-order cascade to keep working. See `reference/engineering/cascade.md`
...
- `reference/engineering/cascade.md` captures the cascade architecture
