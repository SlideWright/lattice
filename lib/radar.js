/**
 * Radar / spider chart — native build-time SVG transform. Rendered from a
 * Markdown nested list, no third-party charting library. Shared between the
 * build path (lattice-emulator.js) and the Marp Core engine plugin
 * (marp.config.js); mirrored by lattice-runtime.js for the marp-vscode
 * preview (three-renderer parity contract).
 *
 ...
 *   - lattice-emulator.js — calls transformRadarSection per slide
...
/**
 * Transform a single section's inner HTML for the radar layout. Returns the
 * input verbatim when the section isn't a radar slide, is already
 * transformed, or has no source list — the caller can splice the result back
 * unconditionally.
 */
function transformRadarSection(inner, cls) {
  const tokens = String(cls).trim().split(/\s+/);
  if (!tokens.includes('radar')) return inner;
  if (/class="radar-figure"/.test(inner)) return inner;

  const ul = findOuterUL(inner);
  if (!ul) return inner;

  const variant = pickVariant(tokens);
  const isMinimal = tokens.includes('minimal');
  const model = parseRadar(ul.inner, variant === 'quadrant');
  if (!model) return inner;

  const scale = resolveScale(model, matchEyebrowText(inner));
  const fig = buildRadar(model, variant, scale, isMinimal);
  return inner.slice(0, ul.start) + fig + inner.slice(ul.end);
}

/**
 * Transform every radar `<section>` in a Marpit render() HTML output. Used by
 * the Marp Core engine plugin in marp.config.js so the preview renders the
 * same DOM the export pipeline does.
 */
function applyToRenderedHtml(html) {
  ...
  while (i < html.length) {
    ...
    const isRadar = tokens.includes('radar');
    ...
    const newInner = transformRadarSection(inner, cls);
