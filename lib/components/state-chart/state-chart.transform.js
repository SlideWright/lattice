...
function installStateChartLayout(rootDoc) {
  ...
  // Which gutter an edge routes through. TB: forward → right (max), back →
  // left (min). LR is the transpose (forward → top/min, back → bottom/max).
  // A convergence edge (sink → the single exit) flips to the OPPOSITE gutter
  // from a normal forward edge, so the join fan balances against the
  // outgoing fan instead of piling on one side.
  ...
  function edgeTB(t, from, to, ev, style) {
    ...
    // Left gutter. Real back-edges dash (data-dir="back"); convergence edges
    // route here too but flow forward to the exit, so they stay solid.
  ...
  function draw(fig) {
    ...
    if (terminals.length) {
      ...
      nodes.push(fnode);
      byIndex[fi] = fnode;
      terminals.forEach((tn) => { transitions.push({ from: tn.index, to: fi, event: '', isSelf: false, _converge: true }); });
    }
