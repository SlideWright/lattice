...
function installStateChartLayout(rootDoc) {
  ...
  function assignBows(transitions, byIndex, dir, labelW) {
    ['skip', 'back'].forEach((side) => {
      ...
      list.forEach((it) => {
        const t = it.t, from = byIndex[t.from], to = byIndex[t.to];
        ...
        t._peak = peak;
        // Outer extent: the curve apex (0.75·peak + 0.25·endpoint) plus the
        // label half-width. The label centres on that apex, so it overhangs
        // by half its width on the gutter side.
        const midEp = (ext(from) + ext(to)) / 2;
        const apex = 0.75 * peak + 0.25 * midEp;
        const half = labelW(t.event || '') / 2;
        const reserve = (half > 0 ? half : 0) + 8;
        const outer = apex + (max ? reserve : -reserve);
  ...
  function edgeTB(t, from, to, ev) {
    ...
    if (t.to > t.from + 1) {
      ...
      const peak = t._peak != null ? t._peak : (t._maxR != null ? t._maxR : Math.max(xs, tipx)) + G.gap + G.laneBase;
    ...
    const bpeak = t._peak != null ? t._peak : (t._minL != null ? t._minL : Math.min(bxs, btip)) - G.gap - G.laneBase;
  ...
  function edgeLR(t, from, to, ev) {
    ...
    if (t.to > t.from + 1) {
      ...
      const peak = t._peak != null ? t._peak : (t._maxB != null ? t._maxB : Math.max(ys, tipy)) + G.gap + G.laneBase;
    ...
    const bpeak = t._peak != null ? t._peak : (t._minT != null ? t._minT : Math.min(bys, btip)) - G.gap - G.laneBase;
  ...
  function draw(fig) {
    ...
    assignBows(transitions, byIndex, dir, makeLabelW(fig));
    const parts = [];
