...
function installStateChartLayout(rootDoc) {
  ...
  function edgeTB(t, from, to, ev) {
    ...
    // Side-face racetrack. Forward skips leave the upper node's RIGHT face,
    // round out to the gutter peak, run STRAIGHT down the gutter at peak (so
    // every intermediate node — however wide — is cleared by construction),
    // then round back into the lower node's right face (arrow points left).
    // Endpoint y comes from the 5-slot picker. Back-edges mirror on the left.
    ...
    if (t.to > t.from + 1) {
      ...
      return '<path class="state-edge" data-dir="forward" d="' + track(peak, xs, xe) + '"/>' +
        ...
        (ev ? edgeLabel(peak, (yi + yj) / 2, ev, 'forward', 'middle') : '');
    }
    ...
    return '<path class="state-edge" data-dir="back" d="' + track(bpeak, bxs, bxe) + '"/>' +
      ...
      (ev ? edgeLabel(bpeak, (yi + yj) / 2, ev, 'back', 'middle') : '');
  }
  ...
  function edgeLR(t, from, to, ev) {
    ...
    if (t.to > t.from + 1) {
      ...
      return '<path class="state-edge" data-dir="forward" d="' + track(peak, ys, ye) + '"/>' +
        ...
        (ev ? edgeLabel((xi + xj) / 2, peak, ev, 'forward', 'middle') : '');
    }
    ...
    return '<path class="state-edge" data-dir="back" d="' + track(bpeak, bys, bye) + '"/>' +
      ...
      (ev ? edgeLabel((xi + xj) / 2, bpeak, ev, 'back', 'middle') : '');
  }
