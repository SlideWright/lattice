  ...
  function assignPorts(transitions, byIndex, dir) {
    ...
    Object.keys(reg).forEach((idx) => {
      ...
      ['fwd', 'back'].forEach((kind) => {
        ...
        // the top slots, those on the leading side take the bottom slots, and
        // each group nests with its farthest endpoint outermost. (When all
        // edges go the same way this is just "farthest → top".)
        const self = flowOf(+idx);
        list.sort((a, b) => {
          const fa = flowOf(a.other), fb = flowOf(b.other);
          const aUp = fa < self, bUp = fb < self;
          if (aUp !== bUp) return aUp ? -1 : 1;
          return aUp ? fa - fb : fb - fa;
        });
        const N = list.length;
        list.forEach((item, k) => {
          let frac;
