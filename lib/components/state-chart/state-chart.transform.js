...
function installStateChartLayout(rootDoc) {
  ...
  // Uniform gutter assignment: give every non-adjacent edge its natural side,
  // then repeatedly spill the busier gutter's deepest edge to the other while
  // one side is congested well past the other. Direction is the prior; load
  // only relieves genuine overload. No per-type special cases.
  function assignGutters(transitions, dir) {
    const ge = transitions.filter((t) => !t.isSelf && t.to !== t.from + 1);
    ge.forEach((t) => { t._gutterMax = dir === 'lr' ? t.to < t.from : t.to > t.from; });
    const span = (t) => [Math.min(t.from, t.to), Math.max(t.from, t.to)];
    const profile = (sideMax) => {
      const occ = {};
      let mx = 0, peak = 0;
      ge.forEach((t) => {
        if (t._gutterMax !== sideMax) return;
        const [lo, hi] = span(t);
        for (let r = lo; r <= hi; r++) { occ[r] = (occ[r] || 0) + 1; if (occ[r] > mx) { mx = occ[r]; peak = r; } }
      });
      return { mx, peak };
    };
    for (let guard = 0; guard < ge.length; guard++) {
      const R = profile(true), L = profile(false);
      const busy = R.mx >= L.mx;
      const hi = busy ? R : L, lo = busy ? L : R;
      if (hi.mx - lo.mx <= 1) break;                 // tolerate ±1 imbalance
      const cands = ge.filter((t) => t._gutterMax === busy && span(t)[0] <= hi.peak && span(t)[1] >= hi.peak);
      if (!cands.length) break;
      cands.sort((a, b) => (Math.abs(b.to - b.from) - Math.abs(a.to - a.from)) || (a.from - b.from));
      cands[0]._gutterMax = !busy;
    }
  }
