...
// Single-column geometry. State nodes stack on the centre axis; the SVG
// edge layer is absolutely positioned over the whole figure and routes
// adjacent-forward edges straight down through the centre, forward skips
// out to the right gutter, back-edges out to the left gutter, and self-
// loops as small bumps on the right side. Start markers (filled disc)
// sit above the first start state; terminal markers (concentric ring)
// sit below each terminal state. The HTML node column and the SVG layer
// share these numbers so the geometry stays in lockstep.
const GEOM = {
  figureWidth: 480,       // total figure width (px and svg-units)
  nodeWidth: 200,         // state-node width
  nodeHeight: 48,         // state-node height
  rowGap: 18,             // vertical gap between rows
  padTop: 56,             // room above row 1 for start marker
  padBottom: 56,          // room below last row for terminal marker
  laneStep: 24,           // x-distance between successive edge lanes
  arrowSize: 7,           // arrowhead half-length
  startRadius: 7,         // filled-disc start marker
  terminalOuterR: 10,     // outer ring of terminal marker
  terminalInnerR: 5,      // filled centre of terminal marker
  markerOffset: 22,       // distance from state edge to marker centre
  selfLoopOffset: 12,     // y-distance from row centre to self-loop departure/return
  selfLoopPeak: 24,       // x-distance from node right to self-loop apex
  viewBoxPad: 80,         // svg padding to host lanes that spill outside the figure
};
const ROW_STEP = GEOM.nodeHeight + GEOM.rowGap;
const CENTER_X = GEOM.figureWidth / 2;
const NODE_LEFT = CENTER_X - GEOM.nodeWidth / 2;
const NODE_RIGHT = CENTER_X + GEOM.nodeWidth / 2;
...
// A vertical stack means each non-adjacent transition needs a lane offset
// away from the node column so arcs don't overlap. Greedy interval-packing:
// sort by row-span ascending (shorter arcs hug the column, longer arcs
// push outward), then for each edge pick the lowest lane where no
// already-assigned edge at that lane intersects its [min, max] row span.
//
// Forward skips (to > from + 1) and back edges (to < from) use separate
// lane pools — forward skips bow out to the right of the node column,
// back edges bow out to the left. Adjacent-forward edges (to === from + 1)
// render as a straight line through the centre and need no lane.
// Self-loops are fixed shapes on the right side and need no lane.
...
function rowTop(index)    { return GEOM.padTop + (index - 1) * ROW_STEP; }
function rowBottom(index) { return rowTop(index) + GEOM.nodeHeight; }
function rowCentreY(index) { return rowTop(index) + GEOM.nodeHeight / 2; }

// Start marker — filled disc above the first start state, with a short
// arrow down into the state's top edge.
function svgStartMarker(stateIndex) {
  const cx = CENTER_X;
  const stateTop = rowTop(stateIndex);
  const discCy = stateTop - GEOM.markerOffset;
  const lineStart = discCy + GEOM.startRadius;
  const lineEnd = stateTop - GEOM.arrowSize;
  return `<g class="state-marker" data-kind="start">` +
    ...
    arrowhead(cx, stateTop, 90, 'start') +
    `</g>`;
}

// Terminal marker — concentric ring below a terminal state, with a short
// arrow down from the state's bottom edge.
function svgTerminalMarker(stateIndex) {
  const cx = CENTER_X;
  const stateBottom = rowBottom(stateIndex);
  const ringCy = stateBottom + GEOM.markerOffset;
  ...
  return `<g class="state-marker" data-kind="terminal">` +
    ...
    `<circle class="state-marker-ring" cx="${cx}" cy="${ringCy.toFixed(1)}" r="${GEOM.terminalOuterR}" fill="none"/>` +
    `<circle class="state-marker-disc" cx="${cx}" cy="${ringCy.toFixed(1)}" r="${GEOM.terminalInnerR}"/>` +
    `</g>`;
}

// Transition edge — one of four kinds:
//   adjacent forward (j === i+1):  straight line down through the centre
//   forward skip     (j  >  i+1):  cubic Bezier bowing right of the column
//   back             (j  <  i  ):  cubic Bezier bowing left  of the column
//   self             (j === i  ):  small loop on the right side of the row
function svgEdge(edge, lane) {
  const labelAttr = edge.event ? ` data-event="${escAttr(edge.event)}"` : '';

  if (edge.isSelf) {
    const y = rowCentreY(edge.from);
    const yTop = y - GEOM.selfLoopOffset;
    const yBot = y + GEOM.selfLoopOffset;
    const xStart = NODE_RIGHT;
    const xPeak = xStart + GEOM.selfLoopPeak;
    const xEnd = xStart + GEOM.arrowSize;       // line ends arrow-length away so the head fits
    const path = `M ${xStart} ${yTop.toFixed(1)} C ${xPeak} ${yTop - 4}, ${xPeak} ${yBot + 4}, ${xEnd} ${yBot.toFixed(1)}`;
    const arrow = arrowhead(xStart, yBot, 180, 'self');
  ...
  // Adjacent forward — straight vertical line through the centre.
  if (edge.to === edge.from + 1) {
    const x = CENTER_X;
    const yStart = rowBottom(edge.from);
    const yEnd = rowTop(edge.to) - GEOM.arrowSize;
    const path = `M ${x} ${yStart} L ${x} ${yEnd.toFixed(1)}`;
    const arrow = arrowhead(x, rowTop(edge.to), 90, 'forward');
    const labelEl = edge.event
      ? edgeLabel(x + 8, (yStart + yEnd) / 2, edge.event, 'forward', 'start')
      : '';
    return `<g class="state-edge-group" data-dir="forward"${labelAttr}>` +
      `<path class="state-edge" data-dir="forward" d="${path}"/>` +
      arrow + labelEl +
      `</g>`;
  }

  // Forward skip — bow out to the right.
  if (edge.to > edge.from + 1) {
    const xStart = NODE_RIGHT;
    const xEnd = NODE_RIGHT + GEOM.arrowSize;
    const xPeak = NODE_RIGHT + (lane + 1) * GEOM.laneStep;
    const path = `M ${xStart} ${yi.toFixed(1)} C ${xPeak} ${yi.toFixed(1)}, ${xPeak} ${yj.toFixed(1)}, ${xEnd} ${yj.toFixed(1)}`;
    const arrow = arrowhead(NODE_RIGHT, yj, 180, 'forward');
    const labelEl = edge.event
      ? edgeLabel(xPeak + 6, (yi + yj) / 2, edge.event, 'forward', 'start')
      : '';
    return `<g class="state-edge-group" data-dir="forward"${labelAttr}>` +
      `<path class="state-edge" data-dir="forward" d="${path}"/>` +
      arrow + labelEl +
  ...
  // Back edge — bow out to the left.
  const xStart = NODE_LEFT;
  const xEnd = NODE_LEFT - GEOM.arrowSize;
  const xPeak = NODE_LEFT - (lane + 1) * GEOM.laneStep;
  const path = `M ${xStart} ${yi.toFixed(1)} C ${xPeak} ${yi.toFixed(1)}, ${xPeak} ${yj.toFixed(1)}, ${xEnd} ${yj.toFixed(1)}`;
  const arrow = arrowhead(NODE_LEFT, yj, 0, 'back');
  const labelEl = edge.event
    ? edgeLabel(xPeak - 6, (yi + yj) / 2, edge.event, 'back', 'end')
    : '';
  return `<g class="state-edge-group" data-dir="back"${labelAttr}>` +
...
function renderStateNodes(states, variant) {
  const items = states.map(s => {
    ...
    return `<li class="state-node" data-index="${s.index}"${kindAttr}>` +
      labelEl + statusEl + indexEl + annotationEl +
      `</li>`;
  }).join('');
...
function buildStateChart(model, variant) {
  ...
  const edgesHtml = model.transitions
    ...
    .join('');
  // Start markers above every start state, terminal markers below every
  // terminal state. Implicit start/terminal already resolved in parseStateChart.
  const startMarkers = model.states.filter(s => s.isStart).map(s => svgStartMarker(s.index)).join('');
  const terminalMarkers = model.states.filter(s => s.isTerminal).map(s => svgTerminalMarker(s.index)).join('');
  const totalHeight = GEOM.padTop + model.states.length * ROW_STEP - GEOM.rowGap + GEOM.padBottom;
  const viewBox = `${-GEOM.viewBoxPad} 0 ${GEOM.figureWidth + 2 * GEOM.viewBoxPad} ${totalHeight}`;
  const svg = `<svg class="state-chart-edges" viewBox="${viewBox}" role="img" aria-hidden="true" preserveAspectRatio="xMidYMin meet">${startMarkers}${edgesHtml}${terminalMarkers}</svg>`;
  return `<div class="state-chart-figure" data-variant="${escAttr(variant)}" data-states="${model.states.length}" data-transitions="${model.transitions.length}" style="--state-figure-w:${GEOM.figureWidth}px;--state-node-w:${GEOM.nodeWidth}px;--state-node-h:${GEOM.nodeHeight}px;--state-row-gap:${GEOM.rowGap}px;--state-pad-top:${GEOM.padTop}px;--state-pad-bottom:${GEOM.padBottom}px;">${nodesHtml}${svg}</div>`;
}
