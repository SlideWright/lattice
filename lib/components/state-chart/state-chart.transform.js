...
const GEOM = {
  ...
  nodeInnerGap: 12,         // gap between the label region and the status chip
  charPx: 7.2,              // approximate average char width at 13px sans-serif
  statusCharPx: 6,          // approximate uppercase char width at 9px
  statusChipPadX: 8,        // horizontal padding inside the status chip
  statusChipMinW: 38,       // floor for very short status keywords (done/live)
  statusChipH: 16,          // status chip height
  rowGap: 38,               // vertical gap between consecutive node rects
  ...
  laneStep: 30,             // x-distance between successive edge lanes
  laneBase: 56,             // x-distance from node edge to first lane peak (gives curves real bow)
  arrowSize: 7,             // arrowhead tip-to-base length
  endpointGap: 4,           // gap between arrow tip and node boundary so edges don't touch the shapes
  selfLoopPeak: 36,         // x-distance from node-right to self-loop apex
  selfLoopHalfY: 13,        // y-offset of self-loop top/bottom from row centre
...
// (x, y, w, h) from its label text plus an optional status chip. Width is
// content-aware (clamped to [nodeMinWidth, nodeMaxWidth]); height is fixed.

// Status chip width — single source of truth shared by layoutNodes (to
// reserve the space) and renderNode (to place the chip). Uppercase 9px
// text ≈ 6px/char plus symmetric inner padding.
...
function layoutNodes(states) {
  ...
  for (const s of states) {
    ...
    // Node interior is [padX | label | (gap | chip) | padX]. Reserve the
    // exact chip width + gap so the centred label can never overlap it.
...
function renderNode(node) {
  ...
  // Carve the interior into a label region and an optional chip region so
  // the two never overlap. The chip is right-aligned (padX margin); the
  // label is centred in whatever remains to its left.
  let statusEl = '';
  ...
  if (node.status) {
    ...
    statusEl =
      ...
      `<rect class="state-status-bg" data-s="${escAttr(node.status)}" x="${stX.toFixed(1)}" y="${stY.toFixed(1)}" width="${chipW.toFixed(1)}" height="${stH}" rx="8" ry="8"/>` +
      `<text class="state-status-text" data-s="${escAttr(node.status)}" x="${(stX + chipW / 2).toFixed(1)}" y="${(stY + stH / 2).toFixed(1)}" text-anchor="middle" dominant-baseline="central">${escHtml(node.status)}</text>` +
      `</g>`;
  }
...
function renderStartMarker(node) {
  ...
  return `<g class="state-marker" data-kind="start">` +
    ...
    arrowhead(cx, arrowTipY, 90, 'start') +
    `</g>`;
}
...
function renderTerminalMarker(node) {
  ...
  return `<g class="state-marker" data-kind="terminal">` +
    `<path class="state-marker-line" d="M ${cx} ${lineStart.toFixed(1)} L ${cx} ${lineEnd.toFixed(1)}"/>` +
    arrowhead(cx, arrowTipY, 90, 'terminal') +
...
function renderForwardSkip(edge, from, to, lane) {
  ...
  const xStart = from.x + from.w + GEOM.endpointGap;     // depart a hair outside the source's right edge
  const xEndArrow = to.x + to.w + GEOM.endpointGap;       // arrow tip stops short of the target's right edge
  const xEndPath = xEndArrow + GEOM.arrowSize;            // line lands at the arrow's base
  // Peak x is offset from the further-right of the two endpoints so the
  // curve always bows outward — laneBase is generous so the bow reads.
...
function renderSelfLoop(edge, from) {
  ...
  const xArrowTip = xDepart;                                 // arrow tip on the gap line, not at node edge
  const xLineEnd = xArrowTip + GEOM.arrowSize;               // path ends at the arrowhead base
  ...
  return `<g class="state-edge-group" data-dir="self">` +
    ...
    arrowhead(xArrowTip, yBot, 180, 'self') +
    labelEl +
    `</g>`;
}
