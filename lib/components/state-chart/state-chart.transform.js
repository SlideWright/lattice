...
function installStateChartLayout(rootDoc) {
  ...
  function assignPorts(transitions, byIndex, dir) {
    ...
    // Self-loops route on the EMPTIER side face (ties go to the conventional
    // side — right for TB, bottom for LR) and join that face's slot pool, so
    // they never collide with the skip/back fan.
    selfs.forEach((t) => {
      const r = R(t.from);
      ...
      (useBack ? r.back : r.fwd).push({ t, role: 'self', other: t.from });
    });
    const flowOf = (idx) => { const n = byIndex[idx]; return dir === 'lr' ? n.x + n.w / 2 : n.y + n.h / 2; };
    const keyOf = (item) => (item.role === 'self' ? -Infinity : flowOf(item.other));
    Object.keys(reg).forEach((idx) => {
      ...
      ['fwd', 'back'].forEach((kind) => {
        ...
        list.forEach((item, k) => {
          ...
          let off = lo + frac * ext;
          if (item.role === 'self') {
            const pad = G.selfHalf + 2;                  // keep the loop body on-node
            off = Math.max(lo + pad, Math.min(lo + ext - pad, off));
            item.t.selfOff = off;
  ...
  function edgeTB(t, from, to, ev) {
    if (t.isSelf) {
      ...
      const left = t._selfSide === 'left';
      const xd = left ? from.x - G.gap : from.x + from.w + G.gap;
      const xp = left ? xd - G.selfPeak : xd + G.selfPeak;
      const xl = left ? xd - G.arrow : xd + G.arrow;
      ...
      return '<path class="state-edge" data-dir="self" data-self="true" d="' + d + '"/>' +
        arrowhead(xd, yBot, left ? 0 : 180, 'self') +
    ...
    // Side-face bow. Forward skips leave the upper node's RIGHT face and enter
    // the lower node's right face (arrow points left into it); a single cubic
    // bows out to the gutter peak. Endpoint y comes from the 5-slot picker.
    ...
    // Back-edge: leaves the lower node's LEFT face and enters the upper node's
    // left face (arrow points right into it); bows out to the left gutter.
  ...
  function edgeLR(t, from, to, ev) {
    if (t.isSelf) {
      ...
      const top = t._selfSide === 'top';
      const yd = top ? from.y - G.gap : from.y + from.h + G.gap;
      const yp = top ? yd - G.selfPeak : yd + G.selfPeak;
      const yl = top ? yd - G.arrow : yd + G.arrow;
      const d = 'M ' + xLeft.toFixed(1) + ' ' + yd.toFixed(1) +
        ' C ' + (xLeft - 4).toFixed(1) + ' ' + yp + ', ' + (xRight + 4).toFixed(1) + ' ' + yp +
        ', ' + xRight.toFixed(1) + ' ' + yl.toFixed(1);
      return '<path class="state-edge" data-dir="self" data-self="true" d="' + d + '"/>' +
        arrowhead(xRight, yd, top ? 90 : 270, 'self') +
    ...
    // Side-face bow (LR transpose). Forward skips leave the left node's TOP
    // face and arc over the top into the right node's top face (arrow down
    // into it). Endpoint x comes from the 5-slot picker.
    ...
    // Back-edge: leaves the right node's BOTTOM face and arcs under into the
    // left node's bottom face (arrow up into it).
