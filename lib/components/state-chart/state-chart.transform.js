...
function parseStateChart(olInner) {
  ...
  if (!anyExplicitEnd && states.length) {
    // Mirror the auto-start: when no `end` is declared, mark only the LAST
    // state as terminal, and only if it's a sink. Marking every dead-end
    // (e.g. each parallel handler in a router) misreads as a chain of ends.
    const hasOutgoing = new Set(transitions.map(t => t.from));
    const last = states[states.length - 1];
    if (!hasOutgoing.has(last.index)) last.isTerminal = true;
  }
