...
function installStateChartLayout(rootDoc) {
  ...
  // forward skips bow down, back-edges bow up, self-loops below.
  function edgeTB(t, from, to, lane, ev) {
    ...
    if (t.to > t.from + 1) {
      ...
      const d2 = 'M ' + xs.toFixed(1) + ' ' + yi.toFixed(1) +
        ' C ' + peak + ' ' + yi.toFixed(1) + ', ' + peak + ' ' + yj.toFixed(1) + ', ' + xe.toFixed(1) + ' ' + yj.toFixed(1);
    ...
    const d3 = 'M ' + bxs.toFixed(1) + ' ' + yi.toFixed(1) +
      ' C ' + bpeak + ' ' + yi.toFixed(1) + ', ' + bpeak + ' ' + yj.toFixed(1) + ', ' + bxe.toFixed(1) + ' ' + yj.toFixed(1);
  ...
  function edgeLR(t, from, to, lane, ev) {
    ...
    const xi = from.x + from.w / 2, xj = to.x + to.w / 2;
    if (t.to === t.from + 1) {
      const y = from.y + from.h / 2;
      const xs = from.x + from.w + G.gap;
    ...
    if (t.to > t.from + 1) {
      ...
      const peak = Math.max(ys, tipy) + G.laneBase + lane * G.laneStep;
      const d2 = 'M ' + xi.toFixed(1) + ' ' + ys.toFixed(1) +
        ' C ' + xi.toFixed(1) + ' ' + peak + ', ' + xj.toFixed(1) + ' ' + peak + ', ' + xj.toFixed(1) + ' ' + ye.toFixed(1);
      return '<path class="state-edge" data-dir="forward" d="' + d2 + '"/>' +
    ...
    const bpeak = Math.min(bys, btip) - G.laneBase - lane * G.laneStep;
    const d3 = 'M ' + xi.toFixed(1) + ' ' + bys.toFixed(1) +
      ' C ' + xi.toFixed(1) + ' ' + bpeak + ', ' + xj.toFixed(1) + ' ' + bpeak + ', ' + xj.toFixed(1) + ' ' + bye.toFixed(1);
    return '<path class="state-edge" data-dir="back" d="' + d3 + '"/>' +
