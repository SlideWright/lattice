...
function installStateChartLayout(rootDoc) {
  ...
  function assignBows(transitions, dir, labelW) {
    ['skip', 'back'].forEach((side) => {
      const max = side === 'skip';                 // skip → max gutter, back → min
      const list = [];
      transitions.forEach((t) => {
        if (t.isSelf) return;
        const isSkip = t.to > t.from + 1, isBack = t.to < t.from;
        if (max ? !isSkip : !isBack) return;
  ...
  // ── Ports ──
  // Edges attach to the faces that face each other: the upper node's lead
  // face (TB bottom / LR right) and the lower node's trail face (TB top /
  // LR left). assignPorts distributes the attachment offset ALONG that face
  // (TB: across width; LR: down height). Forward edges bulge to the max
  // gutter so their stubs sit past centre toward the max edge; back-edges
  // bulge to the min gutter so their stubs sit toward the min edge; the
  // centre is left for the straight adjacent spine. Sets fromOff / toOff
  // (the along-face coordinate); skip/back always get one, adjacency stays
  // centred.
  function assignPorts(transitions, byIndex, dir) {
    ...
    const place = (list, center, edge) => {
      if (!list.length) return;
      list.sort((a, b) => a.span - b.span);
      const reach = (edge - center) * 0.82;
      const m = list.length;
      list.forEach((item, k) => {
        const off = center + reach * ((k + 1) / (m + 1));
    ...
    Object.keys(faces).forEach((idx) => {
      ...
      ['lead', 'trail'].forEach((face) => {
        place(faces[idx][face].fwd, c, hi);
        place(faces[idx][face].back, c, lo);
      });
    });
  }
  ...
  function edgeTB(t, from, to, ev) {
    ...
    if (t.to > t.from + 1) {
      const sx = t.fromOff != null ? t.fromOff : from.x + from.w * 0.62;
      const ex = t.toOff != null ? t.toOff : to.x + to.w * 0.62;
    ...
    const sx = t.fromOff != null ? t.fromOff : from.x + from.w * 0.38;
    const ex = t.toOff != null ? t.toOff : to.x + to.w * 0.38;
  ...
  function edgeLR(t, from, to, ev) {
    ...
    // node's right face and arrive at the right node's left face (arrow
    // right); the curve sweeps down to the gutter peak, runs across, peels in.
    if (t.to > t.from + 1) {
      const sy = t.fromOff != null ? t.fromOff : from.y + from.h * 0.62;
      const ey = t.toOff != null ? t.toOff : to.y + to.h * 0.62;
      const sxs = from.x + from.w + G.gap;
      const tip = to.x - G.gap;
      const end = tip - G.arrow;
      const peak = t._peak != null ? t._peak : (t._maxB != null ? t._maxB : Math.max(from.y + from.h, to.y + to.h)) + G.gap + G.laneBase;
    ...
    // Back-edge: from is the right node (leaves its left face), to the left
    // node (arrow left into its right face); the curve bows up.
    const sy = t.fromOff != null ? t.fromOff : from.y + from.h * 0.38;
    const ey = t.toOff != null ? t.toOff : to.y + to.h * 0.38;
