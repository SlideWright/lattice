...
function wifiPayload(f) {
  ...
  const secRaw = pick(f, 'security', 'auth', 'encryption').toLowerCase().replace(/\s/g, '');
  const T = SEC[secRaw] !== undefined && SEC[secRaw] !== '' ? SEC[secRaw] : (pass ? 'WPA' : 'nopass');
...
function renderCard(inner) {
  ...
  const ssid = pick(fields, 'ssid', 'network');
  const pass = pick(fields, 'password', 'pass');
  const sec = pick(fields, 'security', 'auth', 'encryption');
  const open = !pass;
  ...
  const svg = encode(wifiPayload(fields));
