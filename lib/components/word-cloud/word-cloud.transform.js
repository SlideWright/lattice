...
//
// Heritage chart-N cycle — same well piechart wedges, kanban lanes,
// and radar series draw from. Five hand-curated boardroom hues; cycle
// wraps past 5.
const CAT_ROTATION = [
...
function colorForWord(rank, weight, opts) {
  ...
  if (opts.color === 'accent-pair') {
    // constellation — accent for top tier, second Heritage slot for
    // mid (slate), muted for low. Sparse density rewards a tight palette.
...
function buildCanvas(rawItems, variant) {
  ...
  const spans = packed.map(w => {
    ...
    return (
      `<span class="wc-word" data-weight="${w.weight}" data-rank="${w.rank}"` +
