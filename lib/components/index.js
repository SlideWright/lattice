 ...
 *   anatomy      string  — (optional) ASCII slide diagram for docs.md.
...
function validate(m, source) {
  ...
  if (m.anatomy !== undefined && (typeof m.anatomy !== 'string' || !m.anatomy)) {
    errors.push(`${prefix}anatomy must be a non-empty string if present`);
  }
 ...
 * Accepts two shapes during migration:
 *   1. Flat:   lib/components/<name>.json
 *   2. Folder: lib/components/<name>/manifest.json
 *
 * The folder shape carries the rest of the component (styles.css,
 * transform.js, example.md, README.md) alongside the manifest per
