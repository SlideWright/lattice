 ...
 * engineering/decisions/2026-05-17-* for the architectural rationale.
 */
 ...
 * component CSS and engineering/gotchas.md.
 */
...
/**
 * Panel-split layouts whose RIGHT-panel list items are "title + nested body"
 * slots. The slot label is rendered bold by slotLabelLift — but the lift
 * only fires when the item HAS a nested body (it wraps the lead up to the
 * nested <ul>/<ol>). A top-level item with NO nested child bullet therefore
 * gets no <strong>: the inline `- Title. body` and bare-`- Title` shapes both
 * render as flat body text, collapsing the right panel's hierarchy. The
 * contract is the nested shape:
 *
 *   - Title
 *     - body
 *
...
/**
 * Number-slot layouts whose list items are "big number + nested label" — the
 * number is the lifted parent-li lead, the label/detail its nested sublist.
 * Same nested-body requirement as SPLIT_SLOT_LAYOUTS: a bare item with no
 * nested child gets no <strong>, so the number doesn't render in display type.
 * (Reuses findSplitBodylessItem — the detector is shape-generic.)
 */
 ...
 * slide. Returns the first offending line, or null. A top-level item is a
 * line starting (column 0) with `- `, `* `, or `N. `; it is well-formed when
 * the next non-blank line is an indented bullet (its body). Catches both the
 * inline `- Title. body` and the bare `- Title` shapes — neither gets lifted.
 */
...
function validate(m, source) {
  ...
  if (CARD_STYLE_LAYOUTS.includes(m.name)) {
    ...
    const skelOffender = m.skeleton ? findInlineTitleBodyLine(m.skeleton) : null;
    if (skelOffender) {
      errors.push(
