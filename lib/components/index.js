 ...
 * them everywhere (`FAMILY_MODIFIER_TOKENS`). Each group is scoped by exact
 * component name and/or bucket.
 *
 *   state-markers — the checkbox disc-style variants (`checks-*`) + the `heat`
 *                   load/risk overlay, for the four state-bearing layouts.
 *   chart         — the opt-in `canvas` surface panel, for the chart family.
 *
...
const FAMILY_MODIFIERS = Object.freeze({
  'state-markers': Object.freeze({
    ...
    components: Object.freeze(['checklist', 'verdict-grid', 'obligation-matrix', 'roadmap']),
  }),
  chart: Object.freeze({
...
/** Family modifiers applicable to a manifest, scoped by component name / bucket. */
function familyModifiersFor(m) {
  const bucket = manifestBucket(m);
  const out = new Set();
  for (const g of Object.values(FAMILY_MODIFIERS)) {
    if (g.components?.includes(m.name) || g.buckets?.includes(bucket)) {
