 ...
 *
 * See reference/design-system.md §6 for the contract and rationale.
 *
 ...
 *
 * See reference/design-system.md §6.5.
 */
const UNIVERSAL_GROUPS = Object.freeze({
  ...
  typography: Object.freeze(['with-period', 'no-period']),
  chrome: Object.freeze(['silent', 'no-header', 'no-footer', 'no-paginate']),
  state: Object.freeze([
    'wip',
    'draft',
    'tbd',
...
/**
 * Card-style layouts where the li represents a card with a title slot
 * (rendered bold by the parent li's font-weight:700) AND an optional
 * body slot. For these layouts, inline `- **Title.** body` format
 * triggers the body-inherits-bold bug — the body text after `<strong>`
 * inherits the parent's bold weight. The canonical authoring contract
 * for these layouts is the nested-list format:
 *
 *   - Title
 *     - body
 *
 * The validator flags `sample` / `variantDocs[*].sample` fields that
 * use inline format for any layout in this list. See the
 * `agnostic-styling` discussion captured at
 * reference/notes/2026-05-17-* for the architectural rationale.
 */
const CARD_STYLE_LAYOUTS = Object.freeze([
  'cards-grid', 'cards-side', 'cards-stack', 'cards-wide',
  'featured', 'split-list', 'compare-prose',
  'matrix-2x2', 'verdict-grid', 'before-after', 'decision', 'citation-card',
]);

/**
 * Detect the inline `- **Title.** body` authoring pattern in a sample
 * string. Returns the first offending line, or null if clean. The
 * pattern is `<bullet> <bold-wrapped title> <punctuation?> <space>
 * <body text>` — body text after a strong on the same bullet.
 *
 * Permitted in samples that don't have a title+body split (single-
 * concept lists like actors, principles).
 */
...
/**
 * Layouts whose ordered-list items render as a counter + a single
 * declarative statement laid out in a CSS grid row (counter | statement).
 * A `<strong>` span inside the statement splits it into separate grid
 * items — the bold fragment lands in the statement column while the rest
 * wraps under the counter — so these layouts require PLAIN ordered-list
 * statements (the layout already sets display weight). See the principles
 * component CSS and reference/engineering/gotchas.md.
 */
const STATEMENT_OL_LAYOUTS = Object.freeze(['principles']);

/**
 * Detect a `**bold**` span inside an ordered-list item (`1. … **x** …`).
 * Returns the first offending line, or null. Used to keep authors from
 * re-introducing the grid-splitting bug on STATEMENT_OL_LAYOUTS.
 */
...
/**
 * Layouts whose list items carry a trailing backtick weight that the
 * engine reads on a fixed 1–5 scale (1 = small/muted, 5 = hero). A
 * frequency-style count (e.g. `124`) silently clamps to 5, collapsing
 * every word to the same size and colour tier. Keep sample weights in
 * range. See lib/components/chart/word-cloud/word-cloud.transform.js
 * (clampWeight) and reference/engineering/gotchas.md.
 */
const WEIGHTED_WORD_LAYOUTS = Object.freeze(['word-cloud']);

/**
 * Detect a backtick weight outside the 1–5 scale on a list item
 * (`- word \`6\``). Returns the first offending line, or null.
 */
function findOutOfRangeWordWeight(sample) {
  if (!sample) return null;
  for (const line of sample.split('\n')) {
    const m = /^\s*[-*]\s+.*`(\d+(?:\.\d+)?)`\s*$/.exec(line);
    if (m) {
      const v = Number(m[1]);
      if (v < 1 || v > 5) return line;
    }
  }
  return null;
}
...
function validate(m, source) {
  ...
  if (CARD_STYLE_LAYOUTS.includes(m.name)) {
    const offender = m.sample ? findInlineTitleBodyLine(m.sample) : null;
    if (offender) {
      errors.push(
    ...
    if (m.variantDocs) {
      for (const [vname, vdoc] of Object.entries(m.variantDocs)) {
        const vOffender = vdoc && vdoc.sample ? findInlineTitleBodyLine(vdoc.sample) : null;
        if (vOffender) {
          errors.push(
  ...
  if (STATEMENT_OL_LAYOUTS.includes(m.name)) {
    ...
    if (m.variantDocs) {
      for (const [vname, vdoc] of Object.entries(m.variantDocs)) {
        const vOffender = vdoc && vdoc.sample ? findBoldOrderedStatement(vdoc.sample) : null;
        if (vOffender) {
          errors.push(
  ...
  // Weighted-word layouts read a fixed 1–5 weight; a frequency-style
  // count silently clamps to 5, collapsing every word to one size/tier.
  if (WEIGHTED_WORD_LAYOUTS.includes(m.name)) {
    const offender = m.sample ? findOutOfRangeWordWeight(m.sample) : null;
    if (offender) {
      errors.push(
        `${prefix}sample has a word weight outside the 1–5 scale on '${m.name}' — ` +
        `it clamps to 5, collapsing the cloud to one size/colour. First offending line: ${JSON.stringify(offender)}`,
      );
    }
    if (m.variantDocs) {
      for (const [vname, vdoc] of Object.entries(m.variantDocs)) {
        const vOffender = vdoc && vdoc.sample ? findOutOfRangeWordWeight(vdoc.sample) : null;
        if (vOffender) {
          errors.push(
            `${prefix}variantDocs."${vname}".sample has a word weight outside 1–5 on '${m.name}'. ` +
            `First offending line: ${JSON.stringify(vOffender)}`,
          );
        }
      }
    }
