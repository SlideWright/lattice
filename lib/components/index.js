 ...
 * docs/notes/2026-05-17-* for the architectural rationale.
 */
const CARD_STYLE_LAYOUTS = Object.freeze([
  'cards-grid', 'cards-side', 'cards-stack', 'cards-wide',
  'featured', 'split-panel', 'compare-prose',
 ...
 * Accepts two shapes:
 *   1. Flat:   lib/components/<name>.json    (legacy)
 *   2. Folder: lib/components/<name>/<name>.manifest.json    (current, dotted)
 *              lib/components/<name>/manifest.json           (pre-Phase 1, still tolerated)
 *
 * The folder shape carries the rest of the component (<name>.styles.css,
 * <name>.transform.js, etc.) alongside the manifest per
...
function loadAll(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isFile() && entry.name.endsWith('.json')) {
      manifestPath = path.join(root, entry.name);
    } else if (entry.isDirectory()) {
      // Dotted convention (Phase 1+): lib/components/<name>/<name>.manifest.json
      const dotted = path.join(root, entry.name, `${entry.name}.manifest.json`);
      // Legacy folder shape pre-Phase 1: lib/components/<name>/manifest.json
      const legacy = path.join(root, entry.name, 'manifest.json');
      if (fs.existsSync(dotted)) {
        manifestPath = dotted;
        label = `${entry.name}/${entry.name}.manifest.json`;
      } else if (fs.existsSync(legacy)) {
        manifestPath = legacy;
        label = `${entry.name}/manifest.json`;
      }
    }
