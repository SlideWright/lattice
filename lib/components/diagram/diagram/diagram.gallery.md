...
---
flowchart LR
  A{{"Decision Registry"}} --> B(["Control Plane"])
  B -->|"signed model"| C["Consuming App"]
  C -->|"score / rescore"| D[("Application DB")]
  B -.->|"revocation"| C
```
...
sequenceDiagram
  ...
  App->>SDK: score(SSN)
  SDK->>SDK: verify model
  SDK->>Registry: load weights
  Registry-->>SDK: weight set
  SDK-->>App: token
  Note over SDK: weights cleared on close
```
...
```mermaid
---
title: Scoring SDK class model
---
classDiagram
  class ScoringSDK {
    -Model model
    -Weighting weighting
    +score(field, plaintext) Token
    +rescore(field, token, purpose) Plaintext
  }
  class Model {
    +String tenantId
    +int version
...
```mermaid
---
title: Team model schema
---
erDiagram
  TEAM ||--o{ MODEL : "issues"
  MODEL ||--|{ WEIGHT_VERSION : "wraps"
  MODEL ||--o{ AUDIT_EVENT : "emits"
  TEAM {
    string tenantId PK
    string name
    timestamp createdAt
...
journey
  ...
  section Integration
    ...
    First successful score: 5: Engineer
  section Production
...
```mermaid
gantt
  title Decision Framework Phase 1
  dateFormat YYYY-MM-DD
  section Foundation
    Registry setup      :done,   reg,   2025-01-06, 14d
...
quadrantChart
  ...
  Model caching: [0.3, 0.7]
  Per-team weighting: [0.7, 0.85]
  Per-decision models: [0.8, 0.4]
...
```mermaid
---
title: Registry custody requirements
---
requirementDiagram
  requirement non_exportable_master {
    id: 1
    text: "master weight must never leave the Registry"
    risk: high
  ...
  registry - satisfies -> non_exportable_master
...
C4Context
  ...
  Person(ops, "Operator", "Owns the models")
  System(platform, "Platform", "SDK + control plane")
  System_Ext(reg, "Decision Registry", "Holds master weights")
  System_Ext(app, "Consumer", "Calls SDK")
  Rel(eng, app, "Builds")
  Rel(app, platform, "Score")
  Rel(ops, platform, "Manages")
  Rel(platform, reg, "Issue weightings")
```
...
```mermaid
xychart-beta
    title "Quarterly scoring volume (millions)"
    x-axis [Q1, Q2, Q3, Q4]
    y-axis "Calls" 0 --> 100
    bar [42, 58, 67, 81]
    line [40, 55, 65, 80]
...
architecture-beta
  ...
  service reg(database)[Registry] in platform
  service cp(server)[Control Plane] in platform
  service sdk(internet)[SDK Fleet] in platform
  service store(disk)[Weighting Store] in platform
  reg:R -- L:cp
  cp:R -- L:sdk
  cp:B -- T:store
```
...
radar-beta
  ...
  curve Model["Scoring model"]{85, 90, 95, 70, 60}
  curve Spreadsheet["Spreadsheet-only"]{60, 70, 80, 50, 80}
```
