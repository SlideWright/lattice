...
function buildMap(model, variant, classTokens = []) {
  ...
  const entries = matched.map((r, i) => {
    let cls, style, swatchCls, keyFill, keyStroke;
    ...
    return { ...r, style, swatchCls, keyFill, keyStroke };
  });
  ...
  if (!rows.length) {
    ...
    return `<div class="map-figure" data-basemap="${esc(basemap.id)}"${unmatchedAttr}>${svg}</div>`;
  }
  const [, , vbW, vbH] = basemap.viewBox.split(/\s+/).map(Number);
  const hasValues = rows.some((r) => r.value != null);
  const key = buildSvgLegend({ rows, diagramRight: vbW, diagramHeight: vbH, hasValues });
  const svg = `<svg class="map-svg" viewBox="0 0 ${key.viewW} ${key.viewH}" ` +
    `preserveAspectRatio="xMidYMid meet" role="img"><title>Map</title>` +
    `<defs>${key.defs}</defs>` +
    `<g transform="translate(0 ${key.diagramDy})">${paths}</g>` +
    `${key.body}</svg>`;
  return `<div class="map-figure" data-basemap="${esc(basemap.id)}"${unmatchedAttr}>${svg}</div>`;
}
