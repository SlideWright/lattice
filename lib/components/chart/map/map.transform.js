...
/**
 * Resolve which basemap a section's class tokens select. `world` picks the
 * world basemap (Equal Earth by default; `robinson` swaps in the Robinson
 * variant); `us` (or nothing) is the default. Forward-compatible — a new
 * basemap is one more token + one more required JSON.
 */
function pickBasemap(classTokens) {
  if (!Array.isArray(classTokens) || !classTokens.includes('world')) return US_BASEMAP;
  return classTokens.includes('robinson') ? WORLD_ROBINSON_BASEMAP : WORLD_BASEMAP;
}
...
function parseMap(ulInner, basemap = BASEMAP) {
  ...
  const rows = topLevelLis(ulInner).map((item) => {
    ...
    return { name, valueRaw, num, kind, ids, slug };
  ...
  for (const r of rows) {
    ...
    if (r.kind === 'region') {
      ...
      if (prev) { prev.valueRaw = r.valueRaw; prev.num = r.num; continue; }
    }
    ordered.push({ ...r });
  }
...
function buildMap(model, variant, classTokens = []) {
  ...
  const entries = matched.map((r, i) => {
    let cls, style, swatchCls;
    if (variant === 'highlight') {
      const slot = (i % CAT_SLOTS) + 1;
      style = `--region-hue:var(--cat${slot}-hue)`;
      cls = 'map-region map-region--hl'; swatchCls = 'map-swatch map-swatch--hl';
    } else {
      style = `--mix:${rampMix(r.num, minNum, maxNum)}%`;
      cls = 'map-region map-region--on'; swatchCls = 'map-swatch map-swatch--on';
    }
    for (const id of r.ids) fillById.set(id, { cls, style });
    return { ...r, style, swatchCls };
  });
  ...
  const rowHtml = (e) =>
    `<li class="map-legend-row${e.kind === 'group' ? ' map-legend-row--group' : ''}">` +
    `<span class="${e.swatchCls}" style="${e.style}"></span>` +
    `<span class="map-legend-name">${esc(e.name)}</span>` +
    (e.valueRaw ? `<span class="map-legend-value">${esc(e.valueRaw)}</span>` : '') +
    `</li>`;
  ...
  if (grouped) {
    ...
    legendBody = keys.map((k) =>
      (k ? `<li class="map-legend-head">${esc(k)}</li>` : '') +
      clusters.get(k).map(rowHtml).join(''),
    ).join('');
  } else {
    legendBody = legendEntries.map(rowHtml).join('');
  }
  ...
  const legend = (legendBody || unmatchedItems)
    ? `<ul class="map-legend">${legendBody}${unmatchedItems}</ul>`
    : '';
