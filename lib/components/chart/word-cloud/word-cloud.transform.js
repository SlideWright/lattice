...
const CANVAS_W = 1100;
const CANVAS_H = 320;
// 70/30 split: the cloud packs into the left fraction of the canvas; the right
// rail holds the "size = frequency" key (matching the chart-family legend rail).
const WC_CLOUD_FRAC = 0.68;
...
function buildCanvas(rawItems, variant) {
  ...
  // Pack into the LEFT 70% of the canvas, leaving the right 30% as the key rail.
  const packed = packCloud(visualed, { w: Math.round(CANVAS_W * WC_CLOUD_FRAC), h: CANVAS_H }, opts.spiral);
  ...
  return (
    `<div class="word-cloud-canvas" data-count="${packed.length}" data-variant="${variant}"` +
      ` style="--wc-canvas-w:${CANVAS_W}px;--wc-canvas-h:${CANVAS_H}px;--wc-cloud-frac:${WC_CLOUD_FRAC}">` +
      spans + keyHtml +
    `</div>`
  );
}
