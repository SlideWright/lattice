...
function renderHtmlNode(s) {
  ...
  return `<li class="state-node" data-index="${s.index}"${kindAttr}${toneAttr}>` +
    indexEl + labelEl +
    `</li>`;
}
...
function renderInline(model, dir) {
  ...
  const items = model.states.map(s => {
    ...
    return `<li class="state-node-row" data-index="${s.index}"${kindAttr}${toneAttr}>` +
      labelEl + indexEl + chipsEl +
      `</li>`;
  }).join('');
  return `<div class="state-chart-figure" data-variant="inline" data-sc-dir="${d}" data-states="${model.states.length}" data-transitions="${model.transitions.length}"><ol class="state-rows">${items}</ol></div>` + buildStatusLegend(model);
}
