...
// active theme supplies --c1-dark … --c8-dark via its categorical block.
//
...
const RADAR_PALETTE = [
  'var(--cat1-hue)', 'var(--cat2-hue)', 'var(--cat3-hue)', 'var(--cat4-hue)',
  'var(--cat5-hue)', 'var(--cat6-hue)', 'var(--cat7-hue)', 'var(--cat8-hue)',
];
...
// don't confuse the top-level walker. Mirrors the journey transform.
...
// Apple-Stocks-style area fade. SVG fill cannot take a CSS gradient, so each
// series gets its own <radialGradient> centred on the plot: near-transparent
// at the centre, denser toward the data rim — the hue rides --catN-hue via an
// inline stop-color so it still flips with the canvas. Unique ids per render
...
function areaGradient(color) {
  ...
  const def = `<radialGradient id="${id}" gradientUnits="userSpaceOnUse" ` +
    `cx="${GEOM.cx}" cy="${GEOM.cy}" r="${GEOM.R}">` +
      ...
      `<stop offset="58%" style="stop-color:${color}" stop-opacity="0.15"/>` +
      `<stop offset="100%" style="stop-color:${color}" stop-opacity="0.36"/>` +
    `</radialGradient>`;
  return { id, def };
}
...
function renderStandard(model, scale, isMinimal) {
  ...
  const grads = isMinimal
    ? null
    : series.map((s, idx) => areaGradient(RADAR_PALETTE[idx % RADAR_PALETTE.length]));
  ...
  series.forEach((s, idx) => {
    ...
    plot += `<polygon class="radar-poly" data-series="${idx}" style="--series-color:${color}${fill}" ` +
