...
// Per-region vertical depth gradient. Spans the FULL plot height (not each
// cell) so the four regions fade continuously top→bottom rather than seaming
// at the split. stop-color rides var(--catN-fill) — inherited from the
// section.chart-frame ancestor, so it still flips with the canvas; --cell-fill
// can't be used here because a <defs> stop isn't a descendant of the rect that
// sets it. Inline `style="fill:url()"` beats the .quadrant-tint CSS fill rule.
...
function quadrantTintsSvg(splitX, splitY) {
  ...
  cells.forEach((c, i) => {
    ...
    defs.push(`<linearGradient id="${gradId}" gradientUnits="userSpaceOnUse" ` +
      `x1="0" y1="${plot.y0}" x2="0" y2="${plot.y1}">` +
      `<stop offset="0%" style="stop-color:var(--cat${i + 1}-fill)" stop-opacity="1"/>` +
      `<stop offset="100%" style="stop-color:var(--cat${i + 1}-fill)" stop-opacity="0.74"/>` +
      `</linearGradient>`);
