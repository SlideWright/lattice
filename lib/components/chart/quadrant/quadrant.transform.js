...
function renderCohort(model, scale) {
  ...
  const bodyInner =
    ...
    plot +
    labels;
  const key = buildSvgLegend({ rows, diagramRight: GEOM.vbW, diagramHeight: GEOM.vbH, hasValues: true });
  const svg = `<svg class="quadrant-svg quadrant-svg--cohort" ` +
    `viewBox="0 0 ${key.viewW} ${key.viewH}" role="img"><title>Quadrant cohort chart</title>` +
    `<defs>${key.defs}</defs>` +
