...
// split. Tuned as a quiet background WASH (0.04→0.18): the zones stay an Apple-
// clean field while the vivid --cell-ink dots carry the colour, sharing the
// hub→rim fade language of radar / the pie wedges so the family reads as one.
...
function quadrantTintsSvg(splitX, splitY) {
  ...
  cells.forEach((c, i) => {
    ...
    defs.push(`<radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" ` +
      ...
      `<stop offset="0%" style="stop-color:var(--cat${i + 1}-hue)" stop-opacity="0.04"/>` +
      `<stop offset="60%" style="stop-color:var(--cat${i + 1}-hue)" stop-opacity="0.10"/>` +
      `<stop offset="100%" style="stop-color:var(--cat${i + 1}-hue)" stop-opacity="0.18"/>` +
      `</radialGradient>`);
