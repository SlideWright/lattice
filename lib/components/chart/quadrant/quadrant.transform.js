...
function quadrantTintsSvg(splitX, splitY) {
  ...
  cells.forEach((c, i) => {
    ...
    defs.push(`<radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" ` +
      ...
      `<stop offset="0%" style="stop-color:color-mix(in oklab, ${hue} 42%, var(--bg))"/>` +
      `<stop offset="62%" style="stop-color:color-mix(in oklab, ${hue} 58%, var(--bg))"/>` +
      `<stop offset="100%" style="stop-color:color-mix(in oklab, ${hue} 82%, var(--bg))"/>` +
      `</radialGradient>`);
