...
function dotWithLabelSvg(p, label, cellIdx, opts) {
  ...
  const markAttr = opts && opts.mark != null ? ` data-mark="${opts.mark}"` : '';
  let out = '';
  out += `<circle class="quadrant-dot" data-cell="${cellIdx}"${markAttr} ` +
...
function renderStandard(model, scale, variant) {
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += dotWithLabelSvg(p, labelEveryDot ? it.label : '', cellIdx, { mark: it.mark });
    }
  });
  plot += '</g>';
...
function renderBubble(model, scale) {
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += `<circle class="quadrant-bubble" data-cell="${cellIdx}" data-mark="${it.mark}" ` +
        `cx="${p.x.toFixed(2)}" cy="${p.y.toFixed(2)}" r="${r.toFixed(2)}"/>`;
      const inside = r >= 11;
...
function renderTrail(model, scale) {
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += `<circle class="quadrant-trail-after" data-cell="${cellIdx}" data-mark="${it.mark}" ` +
...
function renderCohort(model, scale, orientation) {
  ...
  const hulls = model.groups.map((g, gi) => {
    const pts = g.items.map(it => ({ ...plotPoint(it.x, it.y, scale), mark: it.mark }));
  ...
  hulls.forEach(h => {
    for (const p of h.points) {
      plot += `<circle class="quadrant-dot" data-cell="${h.cellIdx}" data-mark="${p.mark}" ` +
...
function renderThreshold(model, scale) {
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += dotWithLabelSvg(pp, it.label, cellIdx, { mark: it.mark });
    }
  });
  plot += '</g>';
