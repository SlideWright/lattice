...
function buildSvgLegend({ rows, diagramRight, diagramHeight, hasValues = true }) {
  ...
  // A no-value chart (radar) lets its labels use the would-be value column too.
  const labelAvail = FS * LABEL_COL_R + (hasValues ? 0 : valueColW);
  ...
  measured.forEach((m, i) => {
    ...
    const stroke = m.swatchStroke
      ...
      : '';
    body.push(
      `<rect class="chart-key-swatch" x="${swatchX.toFixed(2)}" y="${swatchY.toFixed(2)}" ` +
  ...
  return { defs, body: body.join(''), viewW, viewH, diagramDy, fs: FS };
}
