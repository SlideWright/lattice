...
// Legend text rides inside SVG <text>. Strip any inline HTML the markdown
// produced (e.g. <em>) to PLAIN text for wrapping/char-counting; xmlEsc escapes
// & < > " only at emit, AFTER wrapping/slicing, so a label like "R&D" or "<6 mo"
// counts by its visible length and can never be cut mid-entity nor break the SVG.
function svgText(s) {
...
// crazy" guarantee). The SVG-native key keeps that promise because the caller
// FIT-SCALES the whole key to the viewBox height, so more lines just shrink the
// font, they never overflow. Pure-JS (no DOM/font-metrics): the kernels run in
// three render paths with no browser, so maxChars is derived from a CONSERVATIVE
// average advance by the caller — it breaks EARLY, never past the value column.
...
function wrapLabelToLines(label, maxChars) {
  ...
  for (let word of words) {
    // Hard-break a single word longer than the budget across lines (no ellipsis),
    // matching CSS overflow-wrap, so even one long token can't overrun the rail.
...
function buildPieChart(ulInner, isDonut) {
  ...
  // ── SVG-native legend geometry (computed FIRST: its stack height sets the unit
  //    height, and the disc centres on that) ───────────────────────────────────
  // diagram + spine + key share ONE viewBox, so everything is in user units and
  // the WHOLE unit scales with the CONTAINER (height:100cqh / the cover cqi size)
  // — the key never fights `cqh` in a CSS font-size. font-family/fill come from
  // tokens so the sketch finish reskins the labels. The legend font is a FIXED
  // number of user units: a constant RATIO to the disc, so it scales with the
  // container exactly like the disc — but is NEVER re-shrunk per chart by slice
  // count. The spine floats in a generous ~symmetric gap on both sides (the
  // comfortable divider breathing room the previous CSS rail had). See
  // engineering/decisions/2026-06-13-svg-native-legend.md.
  const FS = 9;                          // legend font, user units — FIXED ratio
  const LH = FS * 1.16, ROW_GAP = FS * 0.72, SW = FS * 1.04;
  const MARGIN_Y = 8;
  const DISC_R_EDGE = 180;               // disc right edge (cx 100 + R 80)
  const GAP = 32;                        // divider gap, each side of the spine
  const SPINE_X = DISC_R_EDGE + GAP;     // 212
  const VB_W = 376;                      // disc + two gaps + a roomy key rail
  const swatchX = SPINE_X + GAP;         // matching gap spine→key
  const labelX = swatchX + SW + 5;
  const valueX = VB_W - 8;               // values right-aligned to the rail edge
  const labelRight = valueX - FS * 3.0;  // reserved value column (≈ "100%")
  // Conservative average advance (em) → wrap breaks early, never past the value.
  const maxChars = Math.max(5, Math.floor((labelRight - labelX) / (FS * 0.6)));
  const rows = parsed.map((p, idx) => ({
    slot: (idx % PIE_PALETTE.length) + 1,
    value: svgText(p.valueRaw),
    lines: wrapLabelToLines(svgText(p.label), maxChars),
  }));
  const rowHeights = rows.map(r => Math.max(SW, (r.lines.length - 1) * LH + FS));
  const stackH = rowHeights.reduce((s, h) => s + h, 0) + ROW_GAP * (rows.length - 1);
  // The unit is as tall as its tallest column. ≤6 slices (the pie's perceptual
  // cap) always fit the disc's 200 box at the fixed font, so the viewBox stays
  // 200 and EVERY real pie's key is identical. A pathological long-tail key grows
  // the viewBox HEIGHT — never the font — so the WHOLE unit (disc AND key) scales
  // down in lockstep; the key is never singled out and shrunk vs the diagram.
  const VB_H = Math.max(200, Math.ceil(stackH + MARGIN_Y * 2));

  const cx = 100, cy = VB_H / 2, R = 80, r = 50;
  let cumul = 0;
  const defs = [];
  ...
  // ── Legend rows (centred on the unit; swatch pins to the FIRST line) ─────────
  let y = (VB_H - stackH) / 2;
  const legendParts = rows.map((r, ri) => {
    const rowTop = y;
    y += rowHeights[ri] + ROW_GAP;
    const baseFirst = rowTop + FS;                    // first-line baseline
    // Swatch centres on the FIRST line's optical middle (not the whole multi-line
    // block), so a wrapped 2–3 line label still reads as one keyed row.
    const swatchY = baseFirst - FS * 0.34 - SW / 2;
    const swatchFill = `color-mix(in oklab, var(--chart-cat-${r.slot}-hue) 82%, var(--bg))`;
    const tspans = r.lines.map((ln, li) =>
      `<tspan x="${labelX.toFixed(2)}" y="${(baseFirst + li * LH).toFixed(2)}">${xmlEsc(ln)}</tspan>`).join('');
    return `<rect class="pie-key-swatch" x="${swatchX.toFixed(2)}" y="${swatchY.toFixed(2)}" width="${SW.toFixed(2)}" height="${SW.toFixed(2)}" rx="${(SW * 0.22).toFixed(2)}" fill="${swatchFill}" stroke="var(--chart-cat-${r.slot}-ink)" stroke-width="${(SW * 0.09).toFixed(2)}"/>` +
      `<text class="pie-key-label" font-size="${FS}">${tspans}</text>` +
      `<text class="pie-key-value" font-size="${FS}" x="${valueX}" y="${baseFirst.toFixed(2)}" text-anchor="end">${xmlEsc(r.value)}</text>`;
  }).join('');

  // The gradient spine on the 70/30 boundary — an SVG mirror of the CSS
  // `--chart-spine` (accent→transparent at both ends, so it melts into the
  // canvas). SVG fill can't take a CSS gradient var, hence the inline stops.
  const spineId = `pie-spine-${++PIE_GRAD_SEQ}`;
  const spineH = VB_H * 0.78, spineY = (VB_H - spineH) / 2, spineW = 1.6;
  defs.push(`<linearGradient id="${spineId}" x1="0" y1="0" x2="0" y2="1">` +
  ...
  const spine = `<rect x="${(SPINE_X - spineW / 2).toFixed(2)}" y="${spineY.toFixed(2)}" width="${spineW}" height="${spineH.toFixed(2)}" rx="${(spineW / 2).toFixed(2)}" fill="url(#${spineId})"/>`;

  const svg = `<svg class="piechart-svg" viewBox="0 0 ${VB_W} ${VB_H}" role="img"><title>Pie chart</title><defs>${defs.join('')}</defs>${wedges}${spine}${legendParts}</svg>`;
  return `<div class="piechart-figure">${svg}</div>`;
}
  ...
  if (chartLayout === 'progress') {
    ...
    if (ulExtract) {
      ...
      if (model) {
        ...
        const figureHtml = map.buildMap(model, variant, classTokens);
        html = html.slice(0, ulExtract.start) + figureHtml + html.slice(ulExtract.end);
      }
    }
    ...
    html = journey.transformJourneySection(html, cls);
