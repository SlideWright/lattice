...
function buildPieChart(ulInner, isDonut) {
  ...
  const key = buildSvgLegend({ rows, diagramRight: DISC_R_EDGE, diagramHeight: DIAGRAM_H, hasValues: true });
  ...
  const svg = `<svg class="piechart-svg" viewBox="0 0 ${key.viewW} ${key.viewH}" role="img"><title>Pie chart</title>` +
    `<defs>${defs.join('')}${key.defs}</defs>` +
    `<g transform="translate(0 ${key.diagramDy})">${wedges}</g>` +
    `${key.body}</svg>`;
