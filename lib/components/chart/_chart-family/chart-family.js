...
function transformChartSection(innerHtml, cls) {
  ...
  if (chartLayout === 'progress') {
    ...
    // Two variants opt out of the bottom key: `horizons` (cards already fill the
    // body height and carry their own Now/Next/Later temporal framing, so a key
    // row would overlap the last card) and `status` (the heavy treatment already
    // prints SHIPPED / IN FLIGHT / … on every cell, so a key is redundant).
    const skipKey = /\b(?:horizons|status)\b/.test(cls);
