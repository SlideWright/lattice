...
function buildPieChart(ulInner, isDonut) {
  ...
  const wedges = parsed.map((p, idx) => {
    ...
    // Radial depth: hub slightly translucent, full at the rim — restrained
    // glassy volume, the same fade family as radar.
    const gradId = `pie-wedge-${++PIE_GRAD_SEQ}`;
    defs.push(`<radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" cx="${cx}" cy="${cy}" r="${R}">` +
      `<stop offset="0%" style="stop-color:${fill}" stop-opacity="0.72"/>` +
