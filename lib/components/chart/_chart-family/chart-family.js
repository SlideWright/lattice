...
function buildPieChart(ulInner, isDonut, orientation) {
  ...
  //   1. Present/Practice — an inert <template> per slice (renders nothing), read
  //      by the parent-hosted reveal layer via data-slice.
  ...
  const noteComment = buildPieDetailNote(parsed);
  return `<div class="piechart-figure">${svg}${detailWrap}</div>${noteComment}`;
}

// Fold a pie's per-slice detail into one Marp-faithful speaker-note comment
// (`<!-- … -->`). notes-core then lifts it into the slide note (PDF annotation +
// hidden aside) and strips the comment before render — so a detail-pie's PDF
// gains the notes WITHOUT touching the chart pixels. Returns '' when no slice
// carries detail (a plain pie stays byte-identical, comment-free). One line per
// detailed slice: `Label (value): item · item`. Output is comment-safe (a `-->`
// in the detail can't terminate the note early) and entity-decoded to plain text.
function buildPieDetailNote(parsed) {
  ...
  for (const p of parsed) {
    ...
    const items = parseTopLevelLis(p.detail).map(flatten).filter(Boolean);
    const body = items.length ? items.join(' · ') : flatten(p.detail);
    if (!body) continue;
    const label = flatten(p.label);
    const value = p.valueRaw ? `(${flatten(p.valueRaw)})` : '';
    // `.trim()` keeps the prefix clean when a degenerate slice has no label
    // (no leading space before the value) — purely cosmetic on the note text.
    lines.push(`${[label, value].filter(Boolean).join(' ')}: ${body}`.trim());
  }
  if (!lines.length) return '';
  // Never let a stray `-->` (or a long dash run) inside the detail close the
  // comment early; collapse any dash run that could form a terminator.
