...
function buildPieChart(ulInner, isDonut) {
  ...
  const wedges = parsed.map((p, idx) => {
    ...
    // with kanban's language: a single gentle vertical hue-into-base wash shared
    ...
    // The wash mixes toward --chart-cat-base (light: --bg; dark: black) so warm
    // wedges stay hue-true on the navy canvas instead of mudding into it.
    defs.push(`<linearGradient id="${gradId}" gradientUnits="userSpaceOnUse" x1="${cx}" y1="${cy - R}" x2="${cx}" y2="${cy + R}">` +
      `<stop offset="0%" style="stop-color:color-mix(in oklab, ${hue} 48%, var(--chart-cat-base))"/>` +
      `<stop offset="100%" style="stop-color:color-mix(in oklab, ${hue} 62%, var(--chart-cat-base))"/>` +
      `</linearGradient>`);
