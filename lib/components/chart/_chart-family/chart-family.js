...
// Wedge / legend-swatch fills, six-rotation to match the CSS that strokes
// each wedge with the slot's --catN-ink border (piechart.styles.css). Uses
// the shared chart category model: --catN-fill is a pale tint on a light
// canvas, the deep hue on a dark canvas — paired with the deep/pale --catN-ink
// border. Six is the perceptual cap; pies past it should consolidate.
const PIE_PALETTE = [
  'var(--cat1-fill)', 'var(--cat2-fill)', 'var(--cat3-fill)',
  'var(--cat4-fill)', 'var(--cat5-fill)', 'var(--cat6-fill)',
];
...
function buildPieChart(ulInner, isDonut) {
  ...
  const wedges = parsed.map((p, idx) => {
    ...
    const fill = `var(--cat${slot}-fill)`;
    const hue = `var(--cat${slot}-hue)`;
    // Radial depth: the slot tint at the hub, deepening toward the rim by
    // mixing in the vivid hue — a clearly legible glassy volume (the earlier
    // opacity-only nudge was invisible). Same tonal idea drives the quadrant.
    ...
    defs.push(`<radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" cx="${cx}" cy="${cy}" r="${R}">` +
      ...
      `<stop offset="62%" style="stop-color:${fill}"/>` +
      `<stop offset="100%" style="stop-color:color-mix(in oklab, ${fill}, ${hue} 48%)"/>` +
      `</radialGradient>`);
