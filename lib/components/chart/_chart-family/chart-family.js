...
// Pie/donut wedge fills — PALE tier (--cN-light) so a chart-family pie reads
// the same as a Mermaid pie (editorial pale fill + deep stroke), not a
// saturated outlier. Wedges carry a --c-stroke outline (see piechart.styles).
// Marks that are thin strokes / text (kanban lane stripe+tag, radar curves,
// timeline dots, word-cloud words) keep the DEEP tier — pale would vanish.
// Chart-family draws from the DECOUPLED Apple-HIG chart palette (--chart-N),
// not the Mermaid-constrained cN tokens. Fallback to --cN-light keeps themes
// that haven't defined --chart-* yet rendering until they're rolled out.
const PIE_PALETTE = [
  'var(--chart-1, var(--c1-light))', 'var(--chart-2, var(--c2-light))',
  'var(--chart-3, var(--c3-light))', 'var(--chart-4, var(--c4-light))',
  'var(--chart-5, var(--c5-light))', 'var(--chart-6, var(--c6-light))',
  'var(--chart-7, var(--c7-light))', 'var(--chart-8, var(--c8-light))',
];
