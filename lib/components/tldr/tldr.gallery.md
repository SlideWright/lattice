---
marp: true
theme: indaco
paginate: true
header: "Lattice · tldr"
---

<!-- _class: title silent -->

# tldr

`Inventory · Stack · Structure`

Single-line takeaways — the deck or section's headline points.

---

<!-- _class: tldr -->
<!-- _footer: "Default · tldr" -->

## What this section will tell you, in five lines.

- Components stay short — `cards-grid` not `inventory.grid.cards`.
- The four layers organise the catalog; they do not name components.
- Manifests are the single source of truth for every component.
- Discovery happens via the scaffolder and IDE snippets, not the directive.
- Forty-five components ship — one folder each.


---

<!-- _class: tldr numbered -->
<!-- _footer: "Numbered takeaways · tldr numbered" -->

## Five takeaways from this section.

1. Components stay short — `cards-grid` not `inventory.grid.cards`.
2. The four layers organise the catalog; they do not name components.
3. Manifests are the single source of truth for every component.
4. Discovery happens via the scaffolder and IDE snippets, not the directive.
5. Forty-five components ship — one folder each.


---

<!-- _class: tldr dark -->
<!-- _footer: "Composition: dark · tldr dark" -->

## What this section will tell you, in five lines.

- Components stay short — `cards-grid` not `inventory.grid.cards`.
- The four layers organise the catalog; they do not name components.
- Manifests are the single source of truth for every component.
- Discovery happens via the scaffolder and IDE snippets, not the directive.
- Forty-five components ship — one folder each.


---

<!-- _class: tldr compact -->
<!-- _footer: "Composition: compact · tldr compact" -->

## What this section will tell you, in five lines.

- Components stay short — `cards-grid` not `inventory.grid.cards`.
- The four layers organise the catalog; they do not name components.
- Manifests are the single source of truth for every component.
- Discovery happens via the scaffolder and IDE snippets, not the directive.
- Forty-five components ship — one folder each.


---

<!-- _class: tldr accent -->
<!-- _footer: "Composition: accent · tldr accent" -->

## What this section will tell you, in five lines.

- Components stay short — `cards-grid` not `inventory.grid.cards`.
- The four layers organise the catalog; they do not name components.
- Manifests are the single source of truth for every component.
- Discovery happens via the scaffolder and IDE snippets, not the directive.
- Forty-five components ship — one folder each.


---

<!-- _class: list -->
<!-- _footer: "Anti-patterns · tldr" -->

## When NOT to reach for tldr.

- **Category labels, not claims.** "Pricing" or "Architecture" wastes the slot — the audience can't act on a label. Each line is the claim itself: "Pricing simplifies to three tiers," not "Pricing."
- **Wall of long lines.** Past about fifteen words the line stops scanning and the recap reads as paragraph soup. Trim ruthlessly or move the longer item to its own subtopic slide.
- **Mid-deck use.** tldr earns its weight at the end. In the middle of a deck it pre-empts the section it's recapping. Use subtopic to open a section and tldr to close it.

---

<!-- _class: closing silent -->

# See also.

`Related components`

- `list` — lines are general bullets, not section takeaways
- `principles` — each line is a stated rule with a justification body
- `closing` — the slide is the deck's final word, not a section recap
- `agenda` — previewing what's coming at the top of the deck
