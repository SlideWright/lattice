...
const ALL_LAYOUTS = new Set([
  ...
  'glossary', 'image', 'journey', 'kanban', 'kpi', 'list', 'list-criteria',
  ...
  // Contract-tier Layouts (lib/contracts/*/*.layouts.json) — a sibling tier to
  // components that `satisfies` a component's DOM with a css-only look. They carry
  // direct-child bodies just like a component, so the wrap detection must know them
  // (else they fall into the generic-prose branch and wrap before their CSS is
  // ready). The drift test asserts this set == components ∪ contract layouts, so
  // these can't silently drift from lib/contracts.
  'layout-ledger', 'layout-cards', 'layout-timeline', 'layout-editorial',
]);
...
const STAGE_MIGRATED = new Set([
  ...
  'compare-prose',
  'code',
  // Contract-tier inventory Layouts — CSS migrated to `> .cell-stage > …` in this
  // change (#558), so they wrap like any migrated component (title lifts to the
  // masthead, body is the bounded stage cell). See lib/contracts/inventory.
  'layout-ledger', 'layout-cards', 'layout-timeline', 'layout-editorial',
]);
