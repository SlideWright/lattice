...
const TRANSFORMERS = [
  ...
  compareCode,
  // QR-card components (connect bucket): parse a postfix-key list into a payload,
  // encode a palette-blind QR, emit the card. Disjoint section sets
  // (section.wifi, section.contact) from every other transformer, so position is
  // immaterial to correctness; grouped with the structural transforms.
  wifi,
  contact,
