...
(() => {
  ...
  function injectOrientationStyle(section) {
    ...
    const scale = aspect >= 0.95
      ? 1.2
      : Math.min(1.6, Math.round((1.2 + (1 - aspect) * 0.75) * 100) / 100);
    ...
    el.textContent = `section { --canvas-scale: ${scale}; justify-content: center; }`;
  ...
  function patchSectionGeometry() {
    ...
    const patch = (s) => {
      ...
      // the engine's per-section stamp (lib/engine/slides.js). Idempotent — only
      // write on change, or the attributes:true observer loops every frame.
