...
// Islands runtime DOM injectors — the single, unit-tested implementation of
// the meta / progress / watermark DOM mirrors (lib/runtime/islands-dom.js).
const islandsDom = require('./islands-dom');
...
(() => {
  ...
  // meta island (Phase 2) — fill the masthead bay built by the registry's
  // masthead-lift pass with the front-matter `meta:` directive. Mirrors
  // marp.config.js's applyMastheadMetaToHtml; no-ops in the vscode webview
  // sandbox (fetch blocked) exactly like the logo path.
  // meta island (Phase 2) — fetch the source `.md`, read `meta:`, and fill the
  // masthead bays. The DOM mutation is the shared, unit-tested
  // islandsDom.injectMastheadMeta; only the fetch wrapper lives here (it
  // no-ops in the vscode webview sandbox, like the deck-logo path).
  function applyMastheadMetaFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        islandsDom.injectMastheadMeta(document, m ? m[1].trim() : '');
      })
      .catch(() => { /* fetch blocked / 404 / sandbox — no-op */ });
  }
  ...
  function bootstrap() {
    ...
    islandsDom.injectProgressRail(document);
    islandsDom.injectWatermark(document);
  ...
  function startOverflowWatcher() {
    ...
    const check = () => {
      ...
      for (const s of document.querySelectorAll('section[data-marpit-slide]')) {
        ...
        // lets it react to its own class change and churn every frame — which
        // is what flickers the red overflow ring on a slide (e.g. a `grouped`
        // map legend) sitting right at the threshold. `.overflow` is an inset
        // box-shadow, so it never shifts layout: a stable measurement here
        // means a stable class, and the loop settles.
  ...
  // The Marpit plugin (functionPlotFences in marp.config.js) emits
  ...
  if (typeof document === "undefined") return;
  function boot() { bootstrap(); inflateFunctionPlots(); }
