...
(() => {
  ...
  function stampOrientation(s) {
    ...
    if (!w || !h) return;
    const a = w / h;
    const o = a > 1.05 ? null : (a >= 0.95 ? 'square' : 'portrait');
    ...
    // [data-family] rules generated from each Frame's manifest `slicing`, plus
    // the cross-band relocation table). `wide` is the authored default → leave it
    // unstamped (and clear a stale stamp on a resize back to wide), so a
    // runtime-less render is byte-unchanged. Family taxonomy is the single source
    // in lib/adaptive/families.js. See 2026-06-21-reflow-as-form-capability.md.
