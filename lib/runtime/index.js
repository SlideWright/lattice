...
// Form runtime DOM injectors — the single, unit-tested implementation of
// the meta / progress / watermark DOM mirrors (lib/runtime/form-dom.js).
const formDom = require('./form-dom');
...
(() => {
  ...
  // formDom.injectMastheadMeta; only the fetch wrapper lives here (it
  ...
  function applyMastheadMetaFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        formDom.injectMastheadMeta(document, m ? m[1].trim() : '');
      })
      .catch(() => { /* fetch blocked / 404 / sandbox — no-op */ });
  }
  ...
  function bootstrap() {
    ...
    formDom.injectProgressRail(document);
    formDom.injectWatermark(document);
