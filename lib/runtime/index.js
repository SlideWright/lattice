...
(() => {
  ...
  function initFluidView() {
    ...
    // default to the device — fluid in a portrait viewport (a phone), the
    // authored fixed deck in a landscape one (a laptop).
    const hint = (window.location && (window.location.search + window.location.hash)) || "";
    if (/[?&#]view=fixed|#fixed\b/.test(hint)) apply(false);
    else if (/[?&#]view=fluid|#fluid\b/.test(hint)) apply(true);
