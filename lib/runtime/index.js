...
(() => {
  ...
  function applyMastheadMetaFromFrontMatter() {
    ...
    fetch(mdUrl)
      ...
      .then((src) => {
        ...
        for (const bay of document.querySelectorAll('section.islands .isl-masthead .m-bay')) {
  ...
  // progress island (Phase 2b) — derive sections from divider slides and
  // inject the footer-centre dot-rail into each islands slide. DOM mirror of
  // marp.config.js's applyProgressRailToHtml; pure DOM, no fetch needed.
  function applyProgressRailFromDom() {
    if (typeof document === 'undefined') return;
    const sections = [...document.querySelectorAll('section[data-marpit-slide]')];
    const labels = [];
  ...
  // watermark island (Phase 2c) — section-number ghost behind `islands
  // watermark` slides. DOM mirror of marp.config.js's applyWatermarkToHtml.
  function applyWatermarkFromDom() {
    if (typeof document === 'undefined') return;
    const sections = [...document.querySelectorAll('section[data-marpit-slide]')];
  ...
  function bootstrap() {
    ...
    applyProgressRailFromDom();
    applyWatermarkFromDom();
