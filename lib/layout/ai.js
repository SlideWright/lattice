...
const COMPONENT_CANON =
  ...
  '\n' +
  '• DECLINE when out of scope. You author ONLY transform-free components — pure CSS over a native `ul>li`/prose DOM. If the request needs ' +
  'a chart, a Mermaid/flow diagram, generated code, a math render, or a non-`ul>li` structure (timeline, roadmap, journey), you CANNOT ' +
  'author it as CSS alone — return a decline (see the output contract) routing to a first-party build, NOT a broken fake.\n';
...
const ASK_SYSTEM =
  ...
  '`--fs-*` type only, NO non-reset margin, the `> .cell-stage` root, cards as `<li>`.\n' +
  '  "skeleton": a markdown slide that invokes `<!-- _class: <name> -->` and exercises the component (cards as `- Title` / `  - body`).\n' +
  'If the request is OUT OF SCOPE (a chart, a diagram, generated code, a math render, or a non-`ul>li` structure), instead return ' +
  '`{"decline": true, "reason": "<one sentence>", "route": "<chart|diagram|code|math|dsl>", "suggestion": "<an existing component or a first-party build>"}`.';
