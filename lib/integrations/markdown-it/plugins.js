...
const { continuationFlags, groupOnSlideRule } = require('../../core/slide-rule');
...
function focusSteps(markdown) {
  markdown.core.ruler.before('lattice_slide', 'lattice_focus_steps', (state) => {
    ...
    // EVERY BOUNDARY THIS EMITS CARRIES A FLAG, and getting that wrong desynchronized the
    // reader-view carrier. This rule REBUILDS the token stream, so the original `hr` tokens —
    // and with them every `meta.latticeContinuation` mark `headingSplit` set — are dropped and
    // replaced by fresh ones. Emitting them bare did two things: each focus copy counted as a
    // NEW authored slide (`lib/engine/slides.js` reads the flag to decide), and every heading
    // split in the WHOLE deck lost its mark, whether or not it was near a focus slide.
    //
    // Measured on the real CLI: a `brief` view of a deck whose slide 2 carries three focus steps
    // baked the map `brief:[0,3]` against frames `0..5`, so the reader was shown a slide the view
    // EXCLUDES while one of its own members was unreachable — the exact failure the authored-slide
    // number exists to prevent, reached with no crafted markdown. `examples/focus.md`, a deck this
    // repo ships, was one of them.
    //
    // Two rules, and both matter. A boundary BETWEEN groups keeps whatever the original carried,
    // so a heading split stays a continuation and an author's `---` stays a new slide. A boundary
    // between the COPIES of one slide is always a continuation: the steps are one authored slide.
    const originalFlags = continuationFlags(state.tokens);
    const out = [];
    let first = true;
    let groupAt = 0;
    const pushHr = (continues) => {
      if (first) { first = false; return; }
      const hr = new state.Token('hr', 'hr', 0);
      hr.markup = '---';
      hr.block = true;
      if (continues) hr.meta = { latticeContinuation: true };
      out.push(hr);
    };
    for (const group of groups) {
      const boundaryContinues = originalFlags[groupAt] === true;
      groupAt++;
      ...
      if (steps.length === 0) {
        pushHr(boundaryContinues);
        out.push(...group);
        continue;
      }
      let firstCopy = true;
      for (const step of steps) {
        pushHr(firstCopy ? boundaryContinues : true);
        firstCopy = false;
        group.forEach((t, i) => {
