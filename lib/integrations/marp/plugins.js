 ...
 * glossary tables, heading-period adjustment, split-list numbering, deck-wide
...
/**
 * Marpit plugin: numbers each `.split-list` slide and writes
 * `data-split-list-n="01"` onto the section, read by CSS via attr().
 */
 ...
 * `.verdict-grid` sections in `<span class="badge {sem} {shape}">`.
 */
...
function slotLabelLift(markdown) {
  ...
  // word boundary).
  const SLOT_LAYOUTS =
    /(?<![\w-])(compare-prose|before-after|decision|split-brief|split-metric|split-steps|split-compare|split-statement|statute-stack|regulatory-update|authority-chain|redline|timeline|list-criteria|actors|kpi|stats)(?![\w-])/;
