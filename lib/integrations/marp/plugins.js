 ...
 * (masthead-lift, meta / progress / watermark) works unchanged. Skipped on:
...
/** Read the deck-wide `islands:` front-matter toggle → boolean. */
function readIslandsFrontMatter(src) {
  if (typeof src !== 'string' || !src.length) return false;
  const fmMatch = src.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!fmMatch) return false;
  const m = fmMatch[1].match(/^\s*islands:\s*["']?(.*?)["']?\s*$/m);
  return m ? /^(true|on|yes)$/i.test(m[1].trim()) : false;
}

/**
 * Resolve one section's class list under the deck toggle: append `islands`
 * unless it already opts in/out (`islands` / `no-islands`) or its layout is
 * in the skip set. Idempotent; returns the (possibly unchanged) class string.
 */
function islandsToggleClass(classAttr) {
 ...
 * and left untouched. (The emulator applies `islandsToggleClass` per-section
 * in parseSlide instead — same eligibility, earlier in its pipeline.)
 */
function applyIslandsToggleToHtml(html, markdown) {
  if (!readIslandsFrontMatter(markdown)) return html;
  return splitSections(html).map((p) => {
    if (p.type === 'gap') return p.text;
    const next = islandsToggleClass(p.cls);
