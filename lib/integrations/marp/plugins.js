...
/**
 * Front-matter reader for the Form `meta:` directive (Phase 2 of the
 * Form model). One deck-wide string, ` | ` to split into stacked lines.
...
/**
 * HTML-stage helper: the Form `meta:` directive. Fills the reserved,
 * still-empty `.masthead-bay` of every masthead-lifted (`form`) section with the
 * meta Tile. Runs AFTER the transformer registry (which builds the bay) and
 * after the logo injector. Idempotent — only an empty `.masthead-bay` matches, so a
...
/**
 * HTML-stage helper: the progress Tile (Phase 2b). Derives sections from
 * `divider` slides, then injects a footer-centre dot-rail into every
 * `form` slide that falls within a section — one dot per section, the
...
/**
 * HTML-stage helper: the watermark Tile (Phase 2c). On any `form
...
function applyWatermarkToHtml(html) {
  ...
  return pieces.map((p) => {
    ...
    if (tokens.includes('form') && tokens.includes('watermark') && cur > 0 &&
        !inner.includes('class="tile-watermark"')) {
      const n = String(cur).padStart(2, '0');
      inner += `<div class="tile-watermark" aria-hidden="true">${n}</div>`;
    }
    return p.openTag + inner + '</section>';
  }).join('');
}
