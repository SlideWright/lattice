...
// splitSections (the depth-aware <section> HTML walker) moved to the shared
// lib/core/split-sections.js so the progress-rail injector here and the
// self-contained watermark Tile kernel (lib/forms/tile/watermark) share ONE copy.
...
/* The watermark Tile's HTML-stage helper moved to its self-contained kernel
 * (lib/forms/tile/watermark/watermark.transform.js → applyToHtml), which now
 * also owns the DOM adapter — one source for all three render paths. See #356. */
