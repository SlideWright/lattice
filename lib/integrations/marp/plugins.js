...
function headingSplit(markdown) {
  // A `<!-- … -->` directive/comment — unambiguously the following slide's
  // frontmatter, so it travels with the heading it precedes. marp-core
  // pre-tokenizes these as `marpit_comment`; the lib/engine path leaves them as
  // an `html_block` (or `html_inline`), so match all three for path parity.
  const isComment = (t) => t.type === 'marpit_comment'
    || ((t.type === 'html_block' || t.type === 'html_inline') && /^<!--[\s\S]*-->$/.test(t.content.trim()));
  // An eyebrow paragraph: inline content that is ONLY `code` spans (+ soft
  // breaks / empty text). Mirrors the kicker convention used across the decks.
  // An eyebrow is not a special construct — it's a plain paragraph whose ONLY
  // child is one inline-`code` span, which the base CSS styles via the adjacency
  // rule `p:has(> code:only-child) + h1/h2`. Match that exact shape (a single
  // `code_inline` child) so the splitter's notion of an eyebrow is identical to
  // the one the renderer styles — no looser, no stricter.
  const isEyebrowInline = (t) => t && t.type === 'inline'
  ...
  markdown.core.ruler.before('marpit_slide', 'lattice_heading_split', (state) => {
    ...
    for (const token of state.tokens) {
      // An author-written top-level `---` is a slide boundary — reset state so
      // the first heading of the next slide does not inject a redundant break.
      ...
      if (
        ...
        if (slideHasHeading) {
          // Find the insertion point: walk back over this heading's lead-in run
          // (directive comments + an eyebrow paragraph) already pushed to `out`,
          // so they move onto the NEW slide with the heading.
          let at = out.length;
          for (;;) {
            const prev = out[at - 1];
            if (prev && isComment(prev)) { at -= 1; continue; }
            // eyebrow paragraph = [paragraph_open, inline, paragraph_close]
            if (prev && prev.type === 'paragraph_close' && at >= 3
              && out[at - 3].type === 'paragraph_open' && isEyebrowInline(out[at - 2])) {
              at -= 3; continue;
            }
            break;
          }
...
/**
 * Front-matter reader for the islands `meta:` directive (Phase 2 of the
 * islands model). One deck-wide string, ` | ` to split into stacked lines.
 * Returns the trimmed string, or null when unset.
 */
function readMastheadMetaFrontMatter(src) {
  if (typeof src !== 'string' || !src.length) return null;
  const fmMatch = src.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!fmMatch) return null;
...
/** Render a `meta:` string as the `.isl-meta` island markup. */
function mastheadMetaToHtml(meta) {
  const esc = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const lines = meta.split('|').map((s) => esc(s.trim())).filter(Boolean).join('<br>');
  return `<div class="isl-meta">${lines}</div>`;
}

/**
 * HTML-stage helper: the islands `meta:` directive. Fills the reserved,
 * still-empty `.m-bay` of every masthead-lifted (`islands`) section with the
 * meta island. Runs AFTER the transformer registry (which builds the bay) and
 * after the logo injector. Idempotent — only an empty `.m-bay` matches, so a
 * second pass is a no-op. Sibling: lattice-emulator.js + lattice-runtime.js.
 */
function applyMastheadMetaToHtml(html, markdown) {
  const meta = readMastheadMetaFrontMatter(markdown);
  if (!meta) return html;
  const block = mastheadMetaToHtml(meta);
  return html.replace(/<div class="m-bay"><\/div>/g, `<div class="m-bay">${block}</div>`);
}

/**
 * Split rendered Marpit HTML into ordered pieces — `gap` (text between
 * sections) and `section` ({ openTag, inner, cls }) — with a depth-aware
 * </section> scan. Shared by the progress-rail injector; mirrors the walker
 * in lib/core/*.js. Reassemble with `openTag + inner + '</section>'`.
 */
...
/** Build the `.isl-progress` dot-rail markup for section `idx` of `total`. */
function progressRailHtml(idx, total, label) {
  ...
  return `<nav class="isl-progress" aria-hidden="true">${seg}${dots}</nav>`;
}

/**
 * HTML-stage helper: the progress island (Phase 2b). Derives sections from
 * `divider` slides, then injects a footer-centre dot-rail into every
 * `islands` slide that falls within a section — one dot per section, the
 * current one marked, labelled with the section's divider title. No-op when
 * the deck has no dividers (nothing to orient against). Idempotent. Sibling:
 * lattice-emulator.js (same fn) + lattice-runtime.js (DOM mirror).
 */
function applyProgressRailToHtml(html) {
  ...
  return pieces.map((p) => {
    ...
    let inner = p.inner;
    if (tokens.includes('islands') && cur > 0 &&
        !tokens.includes('no-progress') && !tokens.includes('silent') &&
        !inner.includes('class="isl-progress"')) {
      inner += progressRailHtml(cur, total, labels[cur - 1]);
    }
...
/**
 * HTML-stage helper: the watermark island (Phase 2c). On any `islands
 * watermark` slide that falls within a section, inject a big ghosted
 * background glyph — the two-digit section number — behind the content
 * (z-behind, clipped by the section). Reuses the same divider-derived
 * section model as the progress rail; no-op without dividers. Idempotent.
 * Sibling: lattice-emulator.js (same fn) + lattice-runtime.js (DOM mirror).
 */
function applyWatermarkToHtml(html) {
  const pieces = splitSections(html);
  ...
  return pieces.map((p) => {
    ...
    if (tokens.includes('divider')) cur += 1;
    let inner = p.inner;
    if (tokens.includes('islands') && tokens.includes('watermark') && cur > 0 &&
        !inner.includes('class="isl-watermark"')) {
      const n = String(cur).padStart(2, '0');
      inner += `<div class="isl-watermark" aria-hidden="true">${n}</div>`;
    }
    return p.openTag + inner + '</section>';
  }).join('');
}
 ...
 * The deck-level feature flag that enables the islands model across a whole
 * deck without tagging each slide. It resolves to the per-slide `islands`
 * class on every ELIGIBLE section, so all the existing islands logic
 * (masthead-lift, meta / progress / watermark) works unchanged. Three modes:
 *   · off (default; also `false`/`no`) — disabled.
 *   · on  (also `true`/`yes`)          — masthead band + bay + progress rail.
 ...
 * A per-slide `no-islands` token opts a single slide out. Build-time only —
 ...
 * config plugins); use a per-slide `islands` token there. */
const ISLANDS_TOGGLE_SKIP = [
...
/** The three values the `islands:` toggle accepts. */
const ISLANDS_MODES = ['off', 'on', 'minimal'];

/** Read the deck-wide `islands:` front-matter toggle → 'off' | 'on' | 'minimal'. */
function readIslandsMode(src) {
  if (typeof src !== 'string' || !src.length) return 'off';
  const fmMatch = src.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!fmMatch) return 'off';
  const m = fmMatch[1].match(/^\s*islands:\s*["']?(.*?)["']?\s*$/m);
  if (!m) return 'off';
  const v = m[1].trim().toLowerCase();
  if (/^(true|on|yes)$/.test(v)) return 'on';
 ...
 * deck-level islands mode). Appends `islands` (plus `no-progress` in `minimal`
 * mode) unless the section already opts in/out (`islands` / `no-islands`) or
 * its layout is in the skip set. Idempotent; returns the class string.
 */
function islandsToggleClass(classAttr, mode = 'on') {
  if (mode === 'off') return classAttr;
  const tokens = (classAttr || '').split(/\s+/).filter(Boolean);
  if (tokens.includes('islands') || tokens.includes('no-islands')) return classAttr;
  if (tokens.some((t) => ISLANDS_TOGGLE_SKIP.includes(t))) return classAttr;
  tokens.push('islands');
...
/**
 * HTML-stage helper: apply the deck-wide `islands:` toggle to rendered HTML
 * by adding the `islands` class to every eligible TOP-LEVEL `<section>`. Runs
 ...
 */
function applyIslandsToggleToHtml(html, markdown) {
  const mode = readIslandsMode(markdown);
  if (mode === 'off') return html;
  return splitSections(html).map((p) => {
    if (p.type === 'gap') return p.text;
    const next = islandsToggleClass(p.cls, mode);
