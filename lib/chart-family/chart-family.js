...
// cN-light family per the chart-colour philosophy: pale tint on light
// canvas, deep hue on dark canvas, same hue both modes. Kanban lanes
// (LANE_COLOR_VARS below) still ride cN-dark — they're foreground
// stripes with different ergonomics and stay on the cN-dark track
// pending the kanban-side cleanup.
const PIE_PALETTE = [
  'var(--c1-light)', 'var(--c2-light)', 'var(--c3-light)', 'var(--c4-light)',
  'var(--c5-light)', 'var(--c6-light)', 'var(--c7-light)', 'var(--c8-light)',
];
...
// cN-dark saturated cycle — kanban lanes carry a categorical signal
// at small surface sizes (3px card-left stripe, ~10px lane pill) where
// the cN-light pale tints don't carry enough hue identity. Stays on
// cN-dark deliberately; piechart's wedges are large enough to read
// from cN-light, kanban's lane cues aren't. See radar's RADAR_PALETTE
// for the same trade-off on series strokes.
