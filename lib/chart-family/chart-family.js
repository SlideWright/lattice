...
// Heritage chart palette — chart-1..5 cycle, wrap past 5. Same palette
// drives kanban (LANE_COLOR_VARS), radar (RADAR_PALETTE in radar's
// transform), word-cloud, every categorical chart cycle. Defined in
// base.tokens.css; themes override --chart-N for brand-specific values.
const PIE_PALETTE = [
...
// Same Heritage chart-N cycle as PIE_PALETTE. Both card stripes and
// lane pills draw from the same well, so a kanban lane and a pie wedge
// in the same deck read as siblings.
const LANE_COLOR_VARS = [
  'var(--chart-1)','var(--chart-2)','var(--chart-3)','var(--chart-4)',
  'var(--chart-5)','var(--chart-1)','var(--chart-2)','var(--chart-3)',
];

// Companion soft tints emitted alongside --lane-color so kanban lane
// pills can render as soft bg + canvas-aware text without color-mix.
const LANE_SOFT_VARS = [
  'var(--chart-1-soft)','var(--chart-2-soft)','var(--chart-3-soft)','var(--chart-4-soft)',
  'var(--chart-5-soft)','var(--chart-1-soft)','var(--chart-2-soft)','var(--chart-3-soft)',
];

function buildKanbanBoard(ulInner) {
  const laneColorMap = {};   // lane → idx, so soft + base resolve from the same cycle position
  let laneColorIdx = 0;
  const getLaneIdx = (lane) => {
    if (!lane) return -1;
    const key = lane.toLowerCase();
    if (laneColorMap[key] === undefined) laneColorMap[key] = laneColorIdx++ % LANE_COLOR_VARS.length;
  ...
  const columnsHtml = columns.map(col => {
    ...
    if (colSub) {
      ...
      cardsHtml = cardItems.map(cardContent => {
        ...
        const laneEl    = label
          ? `<span class="kanban-lane" style="--lane-color:${laneColor || 'var(--chart-accent)'};--lane-soft:${laneSoft || 'var(--chart-1-soft)'}">${escAttr(label)}</span>`
          : '';
        const statusEl  = status
