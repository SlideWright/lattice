...
${lensViews ? `
 /* …except in the reader-view control, whose buttons have no icon to fall back on. */
 #lp-bar .lp-lens-seg .lp-tab-text{position:static;width:auto;height:auto;margin:0;overflow:hidden;clip:auto}
 #lp-bar .lp-lens-seg button{max-width:8ch;padding:10px 6px}` : ''}
...
function lensScript(views) {
	...
	const data = JSON.stringify(views.map((v) => ({ id: v.id, label: v.label, i: v.indices })));
	return `var LENS_VIEWS=${data};
var lensId=LENS_VIEWS[0].id;
var allSlides=slides,allFrames=frames;
 ...
 */
export async function buildArticle(doc, stampSlideIndex = false) {
	...
	const tocHtml = toc
		.map((t) => {
			...
			const stamp = stampSlideIndex && i ? ` data-lp-i="${i[1]}"` : '';
			return `<a href="#${t.id}"${t.level === 2 ? ' class="lp-lvl2"' : ''}${stamp}>${escapeText(t.text)}</a>`;
		})
		.join('\n');
	return { article: stampSlideIndex ? stampArticleParts(doc, articleHtml) : articleHtml, toc: tocHtml };
}
 ...
 * test/unit/export/player-lens.test.js, so a change to the emission order fails there
 ...
 */
function stampArticleParts(doc, articleHtml) {
	...
	for (let k = 0; k < kids.length; k++) if (owner[k] != null) kids[k].setAttribute('data-lp-i', owner[k]);
...
export async function assemblePlayer(data, caps) {
	...
	const slidesHtml = [...doc.querySelectorAll('section[data-lattice-slide]')]
		...
		.map((s, i) => `<div class="lp-frame"${lensViews ? ` data-lp-i="${i}"` : ''}>${s.outerHTML}</div>`)
		.join('\n');
	...
	const { article, toc } = await buildArticle(doc, !!lensViews);
	...
	const envelope = buildEnvelope(
		{ source, title, theme: data.theme, config: data.config, notes: data.notes, glossary: data.glossary, readAlong: buildReadAlong(data.readAlong?.voice, { hasAudio }) },
...
<header id="lp-bar">
 ...
 <span id="lp-count" aria-hidden="true"></span>${lensViews ? `\n <div class="lp-seg lp-lens-seg" role="group" aria-label="Reader view">\n${lensViews.map((v, i) => `  <button data-lp-lens="${escapeAttr(v.id)}" type="button" aria-pressed="${i === 0}"><span class="lp-tab-text">${escapeText(v.label)}</span></button>`).join('\n')}\n </div>` : ''}
