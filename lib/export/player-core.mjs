...
${lensViews ? `
/* READER VIEWS — a native <select>, only present in a multi-view carrier.
   NOT a segmented control, and the reason is the labels. A view's name is the author's own
   noun ("Brief", "Q3 restructuring — board only"), there is no icon that could stand for
   one, and there is no bound on how many views a deck declares or how long each name is.
   A button per view therefore sizes the control to the deck: three long names blew the
   390px bar's budget before any of the rest of the chrome was laid out, and the fix was a
   pile of width caps that truncated the names to "B… E… T…". A <select> costs ONE control
   whatever the deck does — the names live in the popup, at full length, where the platform
   already knows how to present a long list (a full-screen picker on a phone, a scrolling
   menu on the desktop) — and it is a control every reader already knows how to drive with
   a keyboard, a screen reader or a thumb.
   FLEX, so the bar shares its width instead of budgeting it: the select takes what its
   longest name needs, gives it back down to its min-width when the bar is tight, and
   ellipsizes rather than pushing a button off the edge.
   The native arrow and popup are DELIBERATELY not restyled: appearance:none buys a
   custom chevron and costs the platform picker — the one part of this control a phone user
   actually touches. */
#lp-bar #lp-lens-sel{font:inherit;font-size:13px;flex:0 1 auto;min-width:6.5ch;max-width:min(34vw,28ch);
 padding:6px 8px;border:1px solid var(--border,#ddd);border-radius:10px;background:var(--bg-alt,#f5f5f5);
 color:var(--text-secondary,#333);cursor:pointer;line-height:1.2;text-overflow:ellipsis}
#lp-bar #lp-lens-sel:hover{background:var(--bg-alt,#eee)}
...
/* THE CARRIER'S BAR AT PHONE WIDTH. The bar's budget is fixed and a carrier adds another
   control to it.
   NO NUMBER IS QUOTED FOR THE CONTROL THIS REPLACED, and the reason is worth more than the
   number would be. Two figures have stood here — a four-part breakdown summing to 418px
   against 390, then a scrollWidth of 406 — and an independent check could re-derive
   NEITHER, measuring 390/390 with the old control's width caps both intact and defeated.
   The control itself is gone from the tree, and "with its caps defeated" is not a
   reproducible recipe, so any figure about it now rests on a setup nobody can rebuild.
   Under #23 that is worse than no number, twice over. What is measurable is the control
   that SHIPS, and it is measured below.
   The qualitative claim stands on its own: a button per view sizes this bar to how many
   views a deck declares and how long each is named, and a <select> does not. That is a
   property of the two controls, not of one deck's pixels:
     · the reader-view select ellipsizes inside a share of the bar instead of sizing to text;
...
@media(max-width:560px){
 /* The deck title still stands down, and the reason is no longer "it does not fit". With a
    <select> it DOES fit — measured on the SHIPPING control, in real Chromium, with the title
    forced visible: the bar's last control lands at 382 of 390, 352 of 360 and 312 of 320,
    no overflow at any of the three. (Independently re-derived to the pixel.) What it costs
    is the one label on this bar a reader cannot infer from an icon: at 390px the title takes
    86px and the view name collapses from "Evidence" to "Evid…". The title is decoration the
    file's own window already carries; the view name is the control. So the title goes. */
 #lp-bar .lp-brand{display:none}
 /* The select keeps its full name where it fits and shrinks to a stub where it does not —
    it is the only control on the bar whose text a reader cannot infer from an icon, so it
    is the last to give up room, and the popup carries the full names regardless. */
 #lp-bar #lp-lens-sel{max-width:46vw;padding:8px 6px}
 ...
 *
 * `lp-active` IS CLEARED ON EVERY FRAME, not just the visible ones — as DEFENCE IN DEPTH,
 * which is a weaker claim than this comment used to make and the only one measurement
 * supports. It said a stale active frame "would keep showing THROUGH `hidden`", because
 * `.lp-frame.lp-active{display:block}` outranks the UA sheet's `[hidden]{display:none}`.
 * Driven in real Chromium with the clear removed, it does not: this same change ships
 * `.lp-frame[data-lp-i][hidden]{display:none!important}`, which outranks both. So the line
 * is not what stops the leak today; it stops the leak if that CSS rule is ever narrowed,
 * and it keeps `render()` from re-activating a frame the current view does not carry.
 *
 ...
 */
function lensScript(views, defaultId) {
	// Only id/label/indices reach THIS BLOCK — a view's approval hash and its `hidden`/`kind`
	// flags are authoring state, and one of them names views the reader was NOT given.
	//
	// The envelope below no longer widens that: `projectForExport` prunes the front matter's
	// `lenses:` registry and every slide's `_lens` tag down to the views actually being
	// exported (lib/core/lens-export.mjs), so the ids, labels, approval digests and
	// membership of a withheld view are absent from the file rather than merely absent from
	// this block. `--lens-source full` is the one exception, and it is the author asking for
	// the whole deck by name.
	...
	const opensOn = views.some((v) => v.id === defaultId) ? defaultId : views[0].id;
	return `var LENS_VIEWS=${data};
var lensId=${scriptJson(opensOn)};
...
function setLens(id){
 ...
 if(lensSel&&lensSel.value!==id)lensSel.value=id;
...
if(lensSel)lensSel.onchange=function(){setLens(lensSel.value);};
setLens(lensId);
`;
}
...
export async function playerJs(animaJs = '', beats = null, captions = !!beats, canvas = PLAYER_CANVAS, chart = null, lensViews = null, lensDefault = '') {
	...
	const lensJs = Array.isArray(lensViews) && lensViews.length > 1 ? lensScript(lensViews, lensDefault) : '';
...
addEventListener('keydown',function(e){if(view!=='present')return;${lensJs ? `
 // A KEY AIMED AT A FORM CONTROL BELONGS TO THAT CONTROL. This listener is on window, so it
 // saw every keystroke in Present and called preventDefault on the whole PRESENT_KEYMAP —
 // Space, Home, End, the arrows. The reader-view <select> is the first focusable control on
 // this bar that NEEDS those keys: Space is how a keyboard user opens a select's popup, and
 // it was being swallowed to advance the slide instead. The button group this replaced
 // escaped by accident, because Enter (which the keymap does not claim) activated a button;
 // a closed <select> ignores Enter, so the switch left Alt+ArrowDown as the only way in — in
 // a control whose whole justification is that the platform already knows how to present its
 // list. It costs Present nothing — the stage is never one of these elements.
 // SCOPED TO TEXT-ENTRY AND CHOICE CONTROLS, NOT EVERY BUTTON. The bar's other controls are
 // <button>s, and Space is still swallowed on them: tab to Fullscreen or the theme toggle,
 // press Space, and the slide advances instead. That predates this control and is left alone
 // here rather than widened into a change to Present's key handling on a surface this PR did
 // not otherwise touch — Enter activates all of them, so nothing is unreachable. Logged, not
 // silently inherited.
 var tg=e.target,tn=tg&&tg.tagName?tg.tagName.toLowerCase():'';
...
function stampArticleParts(doc, articleHtml, authoredPerPage) {
	...
	//
	// AN ID ALONE IS NOT PROOF, because article bodies are the deck's own markup: `projectGeneric`
	// re-emits author `outerHTML` and DOMPurify keeps `id`, so a slide can carry its own
	// `<p id="lp-sec-0">` and re-own itself — and everything after it — to a slide a DIFFERENT
	// view shows. Measured: a reader on `brief` read an evidence-only slide's whole body that way.
	// The `lensScript` docblock hardened the ROOT lookup against exactly this class (`#lp-article`
	// resolved through `#lp-doc`, never by id) and left this ownership scan trusting an id.
	//
	// So match the CONTRACT `projectDeckToProse` actually emits, not the id in isolation: one
	// heading element per section, ids ascending `lp-sec-0 … lp-sec-N-1` in document order. An
	// element is a real section heading only if it is a heading tag AND its id is the next one
	// expected. A forged id out of sequence is ignored and simply inherits the current owner.
	//
	// Residual, stated rather than papered over: an author who guesses the exact next index and
	// places a heading immediately before the real one still displaces that ONE heading. The
	// blast radius is then a slide's own prose within its own view, not another view's content.
	let expect = 0;
	const heading = kids.map((el) => {
		const m = /^lp-sec-(\d+)$/.exec(el.id || '');
		if (!m || !/^H[1-6]$/.test(el.tagName || '') || Number(m[1]) !== expect) return null;
		expect += 1;
		return m[1];
	});
...
export async function assemblePlayer(data, caps) {
	...
	// Which view the file OPENS on. The CLI has already refused a default naming a view this
	// export does not carry, so falling back to the first here is a belt-and-braces default
	// for a direct caller, not a policy that could silently override an author's choice.
	const lensDefault = lensViews?.some((v) => v.id === data.lensDefault) ? String(data.lensDefault) : (lensViews?.[0]?.id ?? '');
	...
	// READ the authored slide number; do not reconstruct it.
	//
	// The engine stamps `data-authored-slide` when it cuts the deck into sections, and
	// auto-split's continuation pages copy the open tag, so the number survives BOTH
	// splitters. Every page of one authored slide carries the same value — the "2.1 /
	// 2.2" model — which is what a reader view means by "that slide".
	//
	// This replaces reconstructing the mapping from `data-split-run` markers after the
	// fact. That worked for auto-split, which leaves markers, and could not work for
	// heading-split, which leaves none — so a `brief` reader was shown a slide `brief`
	// excludes on any deck with two headings on a slide, which is the DEFAULT split mode.
	// Asking the number rather than deriving it is what closes that class for good.
	//
	// Falls back to position only if the stamp is absent — an older cached render, or a
	// host that assembled the DOM itself. That is the pre-existing behaviour, not a
	// silent guess: with no split it is the same number.
	// A READER-VIEW HOLE NEVER REACHES THE PLAYER, and this is the one surface where dropping it is
	// free. A hole exists so that `section:nth-of-type(N)` still lands on the slide the author aimed
	// at — but the player wraps every slide in its own `.lp-frame`, so each section is the ONLY
	// section in its parent and `nth-of-type` addresses nothing: measured on a real carrier,
	// `section:nth-of-type(5)` matches 0 elements and `nth-of-type(1)` matches all 8. Positional CSS
	// is already meaningless here, so a hole would buy nothing and cost the two things holes cost —
	// bytes, and letting a recipient count how many slides existed and which positions were withheld.
	//
	// The plain `.html` export is NOT this case: its sections are siblings, `nth-of-type` is live, and
	// the hole has to stay. That is the one format where the disclosure is real.
	const shipped = [...doc.querySelectorAll('section[data-lattice-slide]')].filter((el) => !el.classList.contains('lens-hole'));
	for (const el of doc.querySelectorAll('section.lens-hole')) el.remove();
	// THE STAMP IS DENSE, AND THAT IS A PRIVACY PROPERTY, NOT A STYLE CHOICE.
	//
	// Read the AUTHORED slide off each section — never DOM order, because auto-split turns one
	// authored slide into several sections and every page of it has to show and hide together.
	// Then rank the authored numbers that actually SHIP, and stamp the rank.
	//
	// Stamping the authored number itself is what an earlier draft did, and it published the shape
	// of the deck the recipient was not given: measured on an 8-slide deck carrying two views, the
	// frames read `data-lp-i="0,2,6"` in plaintext — the count and the exact withheld slots, beside
	// a DOM the same commit had carefully scrubbed of holes. Nothing needs the authored number here:
	// the player has no holes in it, so there is no positional CSS for the authored space to keep
	// honest, and the baked view map is remapped through this same table one call below. Two pages of
	// one authored slide still share a rank, which is the property auto-split needs.
	const authoredOf = (el, i) => {
		const stamped = Number(el.getAttribute('data-authored-slide'));
		return Number.isInteger(stamped) && stamped >= 0 ? stamped : i;
	};
	const shippedAuthored = shipped.map(authoredOf);
	/** authored slide number -> its rank among the authored slides this carrier ships. */
	const denseRank = new Map([...new Set(shippedAuthored)].sort((a, b) => a - b).map((at, i) => [at, i]));
	const authoredPerPage = lensViews ? shippedAuthored.map((at) => denseRank.get(at)) : null;
	const slidesHtml = shipped
	...
	// THE VIEW MAP IS REMAPPED THROUGH THE SAME TABLE THE FRAMES WERE STAMPED WITH. `lensViews[].indices`
	// arrive as AUTHORED slide numbers — the space the projection works in, where a withheld slide keeps
	// its slot — and the frames carry the DENSE rank (see `denseRank` above, and why). Handing the
	// switcher the authored numbers against dense stamps would make every view show the wrong slides
	// past the first hole, so the two are converted together or not at all. A member the carrier does
	// not ship is dropped rather than mapped to `undefined`: a view can name a slide outside the
	// exported union, and `mem[undefined]` would show nothing while reporting a count that includes it.
	const bakedViews = lensViews
		? lensViews.map((v) => ({ ...v, indices: (v.indices || []).map((at) => denseRank.get(at)).filter((i) => i !== undefined) }))
		: null;
	const js = await playerJs(hasScene ? ANIMA_PLAYER_JS : '', hasNarration ? beats : null, hasCaptions, canvas, hasChart ? { js: ANIMA_CHART_JS, deck: chartMotion } : null, bakedViews, lensDefault);
...
<header id="lp-bar">
 ...
 <span id="lp-count" aria-hidden="true"></span>${lensViews ? `\n <select id="lp-lens-sel" aria-label="Reader view">\n${lensViews.map((v) => `  <option value="${escapeAttr(v.id)}"${v.id === lensDefault ? ' selected' : ''}>${escapeText(v.label)}</option>`).join('\n')}\n </select>` : ''}
