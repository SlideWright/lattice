...
/* THE CARRIER'S BAR AT PHONE WIDTH. The bar's budget is fixed and a carrier adds another
   control to it — measured at 390px with the segmented control this replaced, the three
   view tabs (133px), the counter (51), the reader views (151) and the fullscreen + theme
   buttons (84) came to 418 against 390, and the theme toggle went off-screen entirely. One
   <select> is a great deal narrower than a button per view, but the budget is still tight
   enough that the carrier pays for its own control rather than the deck's chrome losing a
   button:
...
export async function assemblePlayer(data, caps) {
	...
	const lensDefault = lensViews && lensViews.some((v) => v.id === data.lensDefault) ? String(data.lensDefault) : lensViews ? lensViews[0].id : '';
