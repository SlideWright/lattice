...
function lensScript(views) {
	...
	//
	// SCOPED DELIBERATELY, because the wider claim is false and was made here once: the
	// envelope twenty lines below carries the deck's front matter verbatim, so the whole
	// `lenses:` registry ships with it — every view id, its human LABEL (which is prose an
	// author writes, e.g. "Board only — restructuring"), its approval hash, and entries marked
	// `hidden: true` that the reader was never offered. Filtering that is a real change with a
	// re-import cost, not a comment fix, and it is tracked rather than quietly done here.
