...
/* THE CARRIER'S BAR AT PHONE WIDTH. The bar's budget is fixed and a carrier adds another
   control to it. Measured at 390px on the segmented control this replaced, with its width
   caps defeated so the names could ask for the room they wanted: the bar's scrollWidth was
   406 against a clientWidth of 390, and the theme toggle's right edge landed at 406 — 16px
   past the viewport. (An earlier version of this comment carried a four-part breakdown
   summing to 418. Those figures came from an intermediate CSS state that was never
   committed and could not be re-derived from the tree, which makes them worse than no
   number at all — #23. These reproduce.) One <select> is a great deal narrower than a
   button per view, but the budget is still tight enough that the carrier pays for its own
   control rather than the deck's chrome losing a button:
...
@media(max-width:560px){
 /* The deck title still stands down, and the reason is no longer "it does not fit". With a
    <select> it DOES fit — the bar's last control lands at 382 of 390, 352 of 360 and 312 of
    320 with the title shown and no overflow at any of them. What it costs is the one label
    on this bar a reader cannot infer from an icon: at 390px the title takes 86px and the
    view name collapses from "Evidence" to "Evid…". The title is decoration the file's own
    window already carries; the view name is the control. So the title goes. */
...
addEventListener('keydown',function(e){if(view!=='present')return;${lensJs ? `
 ...
 // list. Skipping form controls is the general rule rather than a patch for this one element,
 // and it costs Present nothing: the stage is never a form control.
