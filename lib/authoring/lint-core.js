 ...
 */
// EMPTY as of 2026-06-25: the retire-landscape-locks program gave every formerly
// landscape-only layout a portrait form, so none remain. gantt/state-chart/journey/roadmap
// adapt to a tall box (2026-06-19-chart-adaptive-sizing.md §10); compare-table RESHAPES to
// card-per-row (cover-cards), compare-code re-authors to one block per page (cover-code),
// redline COLLAPSES then block-splits, and kanban gives each LANE its own slide
// (kanban-lanes). See 2026-06-25-retire-landscape-locks-portrait-everything.md. The list +
// its lint gate stay in place (a future layout could re-introduce a lock); the dead-scaffold
// cleanup is a separate closing pass.
const LANDSCAPE_ONLY_LAYOUTS = Object.freeze([]);
const PORTRAIT_ONLY_LAYOUTS = Object.freeze([]);
// Landscape-only layouts that DO adapt to a portrait box once `autosplit: on` is set —
// the orientation-mismatch warning is suppressed for them on an auto-split deck. Currently
// EMPTY: compare-table and compare-code both retired their locks entirely (they support
// both orientations now, so there is no mismatch warning to suppress;
// 2026-06-25-retire-landscape-locks-portrait-everything.md). The mechanism stays for any
// future layout that is landscape-only yet adapts via auto-split. (kanban + redline are
// still landscape-only and not yet portrait-verified, so they keep the warning.)
const AUTOSPLIT_ADAPTS = Object.freeze([]);
  ...
  slides.forEach((slide, idx) => {
    ...
    for (const t of tokens) {
      if (orientation === 'portrait' && landscapeOnly.has(t)) {
        if (autosplitOn && AUTOSPLIT_ADAPTS.includes(t)) continue; // carousels/paginates at export
        findings.push({
          slide: idx - fm + 1,
