...
// they re-author at export into a cover→content carousel (compare-code: one code block
// per page). So the orientation-mismatch warning is suppressed for them on an auto-split
// deck. See 2026-06-23-read-across-carousel.md. (compare-table left this list entirely
// when it retired its lock — it RESHAPES to cards and supports both orientations now;
// 2026-06-25-retire-landscape-locks-portrait-everything.md. kanban is also landscape-only
// + item-capacity, but its portrait auto-split result is not yet design-verified, so it
// keeps the warning for now — a deliberate exclusion.)
const AUTOSPLIT_ADAPTS = Object.freeze(['compare-code']);
