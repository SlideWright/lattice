...
const LANDSCAPE_ONLY_LAYOUTS = Object.freeze([
  ...
  'kanban', 'compare-code', 'compare-table', 'redline',
]);
...
// per page) or paginate rows (compare-table). So the orientation-mismatch warning is
// suppressed for them on an auto-split deck. See 2026-06-23-read-across-carousel.md.
// (kanban is also landscape-only + item-capacity, but its portrait auto-split result is
// not yet design-verified, so it keeps the warning for now — a deliberate exclusion.)
const AUTOSPLIT_ADAPTS = Object.freeze(['compare-code', 'compare-table']);
