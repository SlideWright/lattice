...
function lintTextWith(source, vocab) {
  ...
  // DIVIDED at export, so the capacity-OVERFLOW warning is moot — suppress it. The
  // milder capacity-crowd (soft < n ≤ hard) still applies: the splitter only fires
  // past `hard`, so a merely-crowded slide is not divided. See lib/core/auto-split.js.
    ...
    if (vocab.capacity) {
      for (const t of tokens) {
        ...
        if (cap.hard != null && n > cap.hard) {
          if (autosplitOn) continue; // auto-split divides it at export — not an overflow
          findings.push({
            slide: idx - fm + 1,
