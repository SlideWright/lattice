...
// The `debug:` vocabulary (mirrors docs/src/playground/debug-overlay.js): the reveal
// modes `on-hover` / `on-always` (with `hover`/`always`/`pinned` synonyms), an `off`
// value, and the optional `full` detail. There is deliberately NO bare `on` — a deck
// that still says `debug: on` (or a typo) warns and falls back to on-hover.
const DEBUG_VALID = new Set([
  'off', 'false', 'no', '0', // off
  'on-hover', 'hover', 'on-always', 'always', 'pinned', // reveal modes (+ synonyms)
  'full', 'all', // detail
]);
