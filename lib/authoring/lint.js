 ...
 *      (split-brief/metric/steps/compare) with no `## ` line; the transform
 *      extracts the <h2> into the left panel, so the headline / hero number
 *      silently goes missing.
 *   6. split-statement-missing-quote (warning) — a split-statement slide with
...
function lintText(source, opts = {}) {
  ...
  // (headline, or the split-metric hero number). split-statement is excluded —
  // its anchor is a blockquote, so it legitimately carries no `## `.
  const h2AnchoredSplit = new Set(['split-brief', 'split-metric', 'split-steps', 'split-compare']);
