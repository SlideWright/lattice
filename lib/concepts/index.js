 ...
 * every structural backbone edge must be honored by the live catalogs (a Cell
 * that really accepts a Frame for the recursion, the component fields the join
...
const EDGE_EVIDENCE = Object.freeze({
  ...
  'cell->frame:holds': (r) => r.cells.some((c) => (c.accepts || []).includes('frame')),
...
// catalogs actually honor the relationship — some Cell really `accepts: frame`
// (the recursion), every Component really carries the `function`/`form`/
// `substance` field the join edge claims, and so on. This is what makes "the
// relationships are enforced" true, not just "the node vocabularies are."
