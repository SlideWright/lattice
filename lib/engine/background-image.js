...
function installBackgroundImage(md) {
  md.core.ruler.after('marpit_directives_apply', 'marpit_background_image', (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === 'marpit_slide_open') { slide = token; continue; }
      if (token.type === 'marpit_slide_close') { slide = null; continue; }
