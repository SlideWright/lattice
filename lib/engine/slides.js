 ...
 *
 * Naming: the rulers/tokens this module owns carry the `lattice_` prefix. The
 * `div.marpit` wrapper class (below) keeps Marp's name on purpose — it is the
 * shared interop contract that Lattice CSS targets in BOTH our output and a
 * marp-cli-rendered Export-to-Marp bundle / the VS Code Marp preview. Likewise
 * `marpit_comment` (lib/core/heading-split-core.js) keeps Marp's token name
 * because marp-core emits it. Those two are Marp's strings, not ours; renaming
 * them would break the export/preview interop, so they stay.
 */
...
function readDirectiveComment(raw) {
  ...
  if (value === undefined && !FLAG_DIRECTIVES.has(key)) return null;
...
function installSlidePipeline(md, globalBase = {}, opts = {}) {
  ...
  md.core.ruler.after('lattice_directives', 'lattice_directives_apply', (state) => {
    ...
    for (const token of state.tokens) {
      ...
      for (const key of Object.keys(dirs)) {
        ...
        // `build` is a flag directive: a bare `_build` (empty value) means "build
        // the primary collection with default grouping", so it must still emit
        // `data-build=""`. Every other directive skips an empty value as before.
        if (!value && key !== 'build') continue;
        ...
        // No CSS custom property for `build` — it is a behavioural flag the build
        // resolver reads, not a style value.
        if (value && key !== 'build') style.set(`--${kb}`, value);
      }
    ...
    for (const token of paginated) token.attrSet('data-lattice-pagination-total', String(pageNumber));
  });
  ...
  md.core.ruler.after('lattice_header_footer', 'lattice_slide_containers', (state) => {
    if (state.inlineMode) return;
    const open = new state.Token('lattice_slide_containers_open', 'div', 1);
    open.block = true;
    open.attrSet('class', 'marpit');
    const close = new state.Token('lattice_slide_containers_close', 'div', -1);
    close.block = true;
