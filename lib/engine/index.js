...
function createEngine(opts = {}) {
  ...
  function renderHtml(body, globalBase, source, baseUrl) {
    ...
    // (matches on the `finish` class, so it resolves at this stage). The finish
    // compositor lives on it, and the deck-wide `backdrop:` controls stamp inline
    // `--backdrop-*` vars on it (strength). Backdrop-controls work (#669).
