/**
 * Split-* family DOM transform — HTML string rewrite for the five layouts
 * in the split-* sibling family: split-brief, split-metric, split-steps,
 * split-compare, split-statement. (split-list — the generic "right side
 * is a list" member — is handled by lattice-emulator.js inline; bringing
 * it into this dispatcher is tracked in docs/notes/2026-05-17-split-panel-rename.md.)
 *
...
const { parseTopLevelLis, extractFirstList } = require('../chart-family/chart-family');
...
function applyBrief(inner) {
  ...
  return header +
    ...
    `<div class="brief-right">${r3.trim()}</div>`;
}
...
function applyMetric(inner) {
  ...
  return header +
    ...
    `<div class="metric-right">${r3.trim()}</div>`;
}
...
function applySteps(inner) {
  ...
  return header +
    ...
    `<div class="steps-right">${r3.trim()}</div>`;
}
...
function applyStatement(inner) {
  ...
  return header +
    ...
    `<div class="statement-right">${r2.trim()}</div>`;
}
