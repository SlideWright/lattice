 ...
 * capped at 1.6 — so 4:5 (≈1.35) stays calm while 9:16 (≈1.53) and the very tall
 * 9:19.5 mobile (1.6) push type larger to fill the frame instead of floating in a
 * thin centred band. (A flat multiplier left the tallest formats sparse.) Square
 * is a calm flat 1.2. Tuned against the social demo decks
...
function orientationFor({ width, height }) {
  ...
  if (aspect >= 0.95) return { name: 'square', scale: 1.2 };
  const scale = Math.min(1.6, Math.round((1.2 + (1 - aspect) * 0.75) * 100) / 100);
