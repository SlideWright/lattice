...
// Mirror Marpit's pack (postcss/root/replace.js → increasing_specificity.js):
// `:root` becomes `:where(section):not([\20 root])`, NOT bare `:where(section)`.
// The `:not([\20 root])` guard — an attribute selector that can never match, since
// HTML forbids U+0020 SPACE in attribute names — restores the (0,1,0) specificity
// the `:root` pseudo-class carries. lattice.css declares its real cqi tokens on
// `:root` (0,1,0) and its universal fallbacks on `:where(:root)` (0,0,0), relying
// on the real tokens WINNING. Relocating `:root` to bare `:where(section)` zeroes
// it to (0,0,0), tying the fallbacks on source order — the subtle cascade slip
// behind the "collapsed cqi on mobile WebKit" bug (it happened to resolve right on
// desktop Chromium). Marpit's `:where(:root)` → `:where(:where(section):not([\20
// root]))` falls out of the same regex (the outer `:where()` keeps it at 0,0,0).
// We mirror the specificity but intentionally skip Marpit's `div.marpit >` container
// scoping: it only adds a uniform (0,0,1), so it never changes ordering among our
// own rules — it is the genuinely-inert baggage the clean scaffold drops.
const ROOT_SELECTOR_RE = /(^|[\s>+~(,}])(?:section)?:root\b/g;
function rootToSection(css) {
  return css.replace(ROOT_SELECTOR_RE, (_m, lead) => `${lead}:where(section):not([\\20 root])`);
}
 ...
 * equal specificity), then the theme with `@import 'lattice'` inlined and every
 * `:root` token block relocated onto the slide section.
 */
function composeCss({ themeCss = '', baseLatticeCss = '', sizeName } = {}) {
  ...
  // trigger a second inline), then relocate `:root` token blocks onto the slide.
  const resolvedTheme = rootToSection(stripComments(themeCss).replace(THEME_IMPORT_RE, () => base));
