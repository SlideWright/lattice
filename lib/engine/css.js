 ...
 * capped at 2.0 — so 4:5 (≈1.70) reads large while 9:16 (≈1.94) and the very tall
 * 9:19.5 mobile (2.0) push type bigger still to fill the frame instead of floating
 * in a thin centred band. (These are deliberately punchy: social/mobile is viewed
 * on a phone at arm's length, and the decks carry little text per slide.) Square
 * is a calm flat 1.35. Tuned against the social demo decks
...
function orientationFor({ width, height }) {
  ...
  if (aspect > 1.05) return { name: 'landscape', scale: 1 };
  if (aspect >= 0.95) return { name: 'square', scale: 1.35 };
  const scale = Math.min(2.0, Math.round((1.5 + (1 - aspect) * 1.0) * 100) / 100);
  return { name: 'portrait', scale };
}
...
section { --canvas-scale: ${scale}; justify-content: center; }`;
}
