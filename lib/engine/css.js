 ...
 * slide.) Square is a flat 1.5. Tuned up over two review rounds against the social
...
function orientationFor({ width, height }) {
  ...
  if (aspect >= 0.95) return { name: 'square', scale: 1.5 };
...
function orientationCss(geometry) {
  ...
  // top profile row and the bottom caption + action rail that Stories/Reels/TikTok
  // overlay on a vertical post. Emitted deck-wide as tokens; only the `.safe`
...
section { --canvas-scale: ${scale}; justify-content: center;${safe} }`;
}
