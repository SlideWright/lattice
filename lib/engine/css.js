...
// Mirror Marpit's rootReplace
// (@marp-team/marpit/lib/postcss/root/replace.js): rewrite the `:root`
// pseudo-class in SELECTORS to `:where(section)`. Lattice declares its cqi
// spacing/radius tokens (`--sp-*`, …) on `:root`; a cqi unit resolves against
// the element's query container, and `:root` has none — so it falls back to the
// viewport. Marpit relocates these blocks onto the slide `<section>` (the
// `container-type:size` query container) so cqi resolves against the SLIDE. This
// is the one load-bearing piece of Marpit's theme-pack the P1.1 clean emitter
// dropped: trapped on `:root`, the tokens render correctly on desktop Chromium
// but COLLAPSE on mobile WebKit (gaps → ~0, counters vanish — see
// engineering/gotchas.md). `:where(section)` keeps the 0 specificity Marpit
// intends, so component `section.x` rules still override the base tokens.
const ROOT_SELECTOR_RE = /(^|[\s>+~(,}])(?:section)?:root\b/g;
function rootToSection(css) {
  return css.replace(ROOT_SELECTOR_RE, (_m, lead) => `${lead}:where(section)`);
}
