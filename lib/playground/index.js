...
// CSS source for the owned engine. Default false → delegate to marp's packer (the
// mobile-correct stylesheet). `?css=engine` opts into the owned emitter for the
// pending real-device verification; it only takes effect when active === 'lattice'.
let ownCss = false;
function setCssSource(name) {
  ownCss = name === 'engine' || name === 'own' || name === 'lattice';
  return ownCss;
}
...
if (typeof window !== 'undefined') {
  ...
  try {
    ...
    // Opt into the owned CSS emitter for real-device verification (P5). Implies
    // the owned engine, so `?css=engine` alone is enough to test the full path.
    const cssParam = search.get('css');
    if (cssParam) {
      setCssSource(cssParam);
      if (cssParam === 'engine' || cssParam === 'own' || cssParam === 'lattice') setEngine('lattice');
    }
  } catch (_e) {
