...
function deriveTheme(essentials, options = {}) {
  ...
  //
  // SOLVE THE ARM AGAINST THE STOPS, NOT THE ANCHOR. The light arm is the brand
  // accent, which is what ten of the fourteen committed palettes use: it is
  // authored to clear AA on a light canvas, so it sits well below the white pole
  // and the derived stops have room in both directions. The dark arm CANNOT be
  // `darkAccent` — that is `withLightness(accent, 0.78)`, near-white by design
  // because it is an accent on a near-black canvas, and an anchor parked that
  // close to the high pole leaves --seq-700 nowhere to travel: 1.08-1.90:1 from
  // --seq-500 across the shipped palettes before they were re-anchored (#1697).
  // L 0.68 is the same mid-range the fourteen curated dark arms were re-solved
  // to, hue and chroma untouched — where the weaker of the two adjacent
  // perceptual steps is largest, subject to every painted stop clearing 3:1 on
  // its own canvas.
