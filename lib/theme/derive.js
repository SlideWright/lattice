...
const REQUIRED_TOKENS = Object.freeze({
  ...
  dark: [
    'dark-bg', 'dark-bg-alt', 'dark-border',
    'dark-text-heading', 'dark-text-body', 'dark-text-display',
    'dark-text-secondary', 'dark-text-label', 'dark-text-muted',
  ],
  categorical: [
    ...Array.from({ length: 12 }, (_, i) => `c${i + 1}-light`),
    ...Array.from({ length: 12 }, (_, i) => `c${i + 1}-dark`),
    'c-ink-light', 'c-ink-dark', 'c-stroke', 'c-line', 'c-accent-warm',
  ],
...
function deriveTheme(essentials) {
  ...
  t['dark-text-display'] = mix(e.bg, '#ffffff', 0.6);
  ...
  for (let i = 0; i < 12; i++) {
    ...
    t[`c${i + 1}-light`] = ensureContrast(paleRaw, inkLight, AA, 'lighten');
    t[`c${i + 1}-dark`] = ensureContrast(deepRaw, inkDark, AA, 'darken');
  }

  // ── Structural ────────────────────────────────────────────────────────────
  t['c-stroke'] = withChroma(withLightness(e.accent, 0.5), 0.09);
  t['c-line'] = ld(withLightness(e.textBody, 0.32), withLightness(darkBody, 0.78));
  ...
  t['c-alarm'] = ensureContrast('#c20000', inkDark, AA, 'darken');
