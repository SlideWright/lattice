 ...
 * `{ essentials, rampStrategy, filled, applied, ok }` — `filled` lists keys taken
 * from the fallback, `applied` lists keys the model actually set, `ok` = nothing
 * missing AND at least one key applied (so a no-model floor that echoes the
 * fallback reads as not-ok). `rampStrategy` is normalized to a known strategy
 * (DEFAULT_STRATEGY when absent/unknown) — it is advisory and never blocks `ok`.
 */
