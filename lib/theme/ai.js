 ...
 * model prompts and coercing the model's reply into a valid essential set are
 ...
 *
 * Contract: the model returns a JSON object of the ESSENTIAL_KEYS, each a hex
 * colour. `coerceEssentials` is forgiving — it accepts snake_case / aliased
 * keys and fills anything missing or malformed from a fallback — so a slightly
 * off model reply still yields a valid, derivable set rather than an error.
 */
...
const SEED_SYSTEM =
  'You are a palette designer for the Lattice slide engine. Given a short ' +
  'description, output ONLY a compact JSON object — no prose, no markdown — ' +
  ...
  'accentSoft is a pale tint of the accent. Output the JSON object only.';

/** Messages for an initial seed from a free-text description. */
function seedMessages(description) {
  return [
...
/** Messages for a conversational refine of the current essential set. */
function refineMessages(current, instruction) {
  return [
    { role: 'system', content: SEED_SYSTEM },
    { role: 'assistant', content: JSON.stringify(current) },
    {
      role: 'user',
      content:
        `Adjust the palette per this instruction, keeping it coherent and ` +
        `readable: "${String(instruction || '').trim()}". Output the full ` +
        `JSON object with all keys.`,
    },
  ];
}
...
module.exports = { SEED_SYSTEM, seedMessages, refineMessages, coerceEssentials };
