...
// `data-cell="0"…"3"` (reading-order: TL, TR, BL, BR) to the theme's
// `--c-quadrant-2/1/3/4-fill` brand tokens (Mermaid's Q-numbering, math
// convention). The kernel just emits the cell index; the theme picks
// the colour.  Cohort variant cycles `--c1` … `--c4` instead — see CSS.
  ...
  model.groups.forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += `<circle class="quadrant-bubble" data-cell="${cellIdx}" ` +
        `cx="${p.x.toFixed(2)}" cy="${p.y.toFixed(2)}" r="${r.toFixed(2)}"/>`;
      // Size pill ("$1.2M", "12") rendered inside the bubble when ≥11px,
      ...
      if (it.sizePill) {
        plot += `<text class="quadrant-bubble-value" data-pos="${inside ? 'inside' : 'above'}" ` +
          ...
          `text-anchor="middle" dominant-baseline="${labelBaseline}">${escHtml(it.sizePill)}</text>`;
      }
