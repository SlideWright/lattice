 ...
 * wrapping are owned by lib/chart-family.js (quadrant is one of the
...
// Per-quadrant tint palette. Four palette-cycled categorical tokens drive
// both the quadrant region fill and the cohort hull/label colour. Cycles
// after four, but the chart only has four quadrants.
...
// Same convention as lib/radar.js and lib/journey.js: scan with depth
...
// threshold lines).
function quadrantTintsSvg(splitX, splitY, palette) {
  const { plot } = GEOM;
  const cells = [
    { x: plot.x0, y: plot.y0,  w: splitX - plot.x0,    h: splitY - plot.y0,    color: palette[0] }, // TL
    { x: splitX,  y: plot.y0,  w: plot.x1 - splitX,    h: splitY - plot.y0,    color: palette[1] }, // TR
    { x: plot.x0, y: splitY,   w: splitX - plot.x0,    h: plot.y1 - splitY,    color: palette[2] }, // BL
    { x: splitX,  y: splitY,   w: plot.x1 - splitX,    h: plot.y1 - splitY,    color: palette[3] }, // BR
  ];
...
function quadrantLabelsSvg(names, palette, extraClass) {
  const { plot } = GEOM;
  const inset = 8;
  const positions = [
...
// One scatter dot + an inline name label tucked above it. Used by the
// default + magic + threshold variants. Cohort and bubble emit their own.
function dotWithLabelSvg(p, label, cellIdx, palette, opts) {
  const color = palette[cellIdx % palette.length];
  const r = opts && opts.r != null ? opts.r : GEOM.dotR;
  const offset = r + 4;
  const above = p.y - offset > GEOM.plot.y0 + 8;
  ...
  if (label) {
    out += `<text class="quadrant-dot-label" data-cell="${cellIdx}" style="--cell-color:${color}" ` +
      ...
      `text-anchor="middle" dominant-baseline="${baseline}">${escHtml(label)}</text>`;
  }
  return out;
}
...
function renderStandard(model, scale, variant) {
  ...
  model.groups.slice(0, 4).forEach((g, gi) => {
    for (const it of g.items) {
      ...
      plot += dotWithLabelSvg(p, labelEveryDot ? it.label : '', gi, QUADRANT_PALETTE);
    }
  });
  // Items in extra groups (5+) — render but without a tinted-cell label.
  for (let gi = 4; gi < model.groups.length; gi++) {
    for (const it of model.groups[gi].items) {
      const p = plotPoint(it.x, it.y, scale);
      plot += dotWithLabelSvg(p, labelEveryDot ? it.label : '', gi % 4, QUADRANT_PALETTE);
    }
  }
  plot += '</g>';
  ...
  const svg = openSvg(isMagic ? 'quadrant-svg--magic' : '') +
    quadrantTintsSvg(splitX, splitY, QUADRANT_PALETTE) +
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(labelScale) +
    quadrantLabelsSvg(names, QUADRANT_PALETTE, labelClass) +
    plot +
  '</svg>';
...
function renderBubble(model, scale) {
  ...
  let plot = '<g class="quadrant-plot">';
  model.groups.slice(0, 4).forEach((g, gi) => {
    const color = QUADRANT_PALETTE[gi % QUADRANT_PALETTE.length];
    for (const it of g.items) {
      const p = plotPoint(it.x, it.y, scale);
      const r = bubbleRadius(it.size, sizeRange);
      plot += `<circle class="quadrant-bubble" data-cell="${gi}" style="--cell-color:${color}" ` +
        `cx="${p.x.toFixed(2)}" cy="${p.y.toFixed(2)}" r="${r.toFixed(2)}"/>`;
      // Size pill ("$1.2M", "12") rendered inside the bubble when ≥10px,
      // beside it otherwise — Tufte detail.
      const inside = r >= 11;
      ...
      if (it.sizePill) {
        plot += `<text class="quadrant-bubble-value" data-pos="${inside ? 'inside' : 'above'}" ` +
          ...
          `text-anchor="${labelAnchor}" dominant-baseline="${labelBaseline}">${escHtml(it.sizePill)}</text>`;
      }
      // Item name below the bubble (or above when the size pill is below).
      if (it.label) {
        ...
        plot += `<text class="quadrant-bubble-label" data-cell="${gi}" style="--cell-color:${color}" ` +
          ...
          `text-anchor="middle" dominant-baseline="hanging">${escHtml(it.label)}</text>`;
      }
    }
  });
  plot += '</g>';
  ...
  const svg = openSvg('quadrant-svg--bubble') +
    ...
    plotFrameSvg(splitX, splitY, 'centerline') +
    axisLabelsSvg(scale) +
    quadrantLabelsSvg(names, QUADRANT_PALETTE) +
    plot +
  '</svg>';
...
function renderCohort(model, scale) {
  ...
  const hulls = model.groups.map((g, gi) => {
    ...
    return { name: g.name, color: QUADRANT_PALETTE[gi % QUADRANT_PALETTE.length], hull, points: pts, centroid: c };
  });
...
function renderThreshold(model, scale) {
  ...
  model.groups.slice(0, 4).forEach((g, gi) => {
    ...
    for (const it of g.items) {
      ...
      plot += dotWithLabelSvg(pp, it.label, gi, QUADRANT_PALETTE);
    }
  });
  // Extra groups (5+) — colour-cycle.
  for (let gi = 4; gi < model.groups.length; gi++) {
    for (const it of model.groups[gi].items) {
      const pp = plotPoint(it.x, it.y, scale);
      plot += dotWithLabelSvg(pp, it.label, gi % 4, QUADRANT_PALETTE);
    }
  }
  plot += '</g>';
  ...
  const svg = openSvg('quadrant-svg--threshold') +
    ...
    axisLabelsSvg(scale) +
    quadrantLabelsSvg(names, QUADRANT_PALETTE, 'quadrant-label--zone') +
    badges +
    plot +
  '</svg>';
