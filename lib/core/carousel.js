...
// The shared accent COVER for the paginating strategies (cover-paginate, cover-cards):
// an accent field with the heading hero + optional eyebrow (a leading <code> in the HEAD
// only, never an item's citation <code>) + the manifest intro lead-in. A layout can opt
// its OWN component class onto the cover via `recipe.coverClass`, so its stylesheet can
// carry its design language onto the split cover (compare-table's hairline rule + mono
// heads) and a split reads as the same deck. Absent the field, the cover stays generic —
// unchanged for glossary et al.
function buildPaginateCover(openTag, head, heading, chrome, recipe, count) {
  const { header, footer } = chrome;
  ...
  const lead = intro ? `<div class="split-cover-lead">${intro} &rarr;</div>` : '';
  const coverKlass = recipe.coverClass
    ? `content ${recipe.coverClass} lat-split-cover form`
    : 'content lat-split-cover form';
  const coverTag = roleOpenTag(openTag, coverKlass, true);
  return `${coverTag}${header}${eye}<div class="split-feat-h">${heading}</div>${lead}${footer}</section>`;
}

function coverPaginateSections(openTag, inner, chrome, recipe, ratio) {
  ...
  const cover = buildPaginateCover(openTag, head, heading, chrome, recipe, count);
...
// ── cover-cards strategy (the wide table in a NARROW frame) ───────────────────
// A multi-column table read across its columns can't survive ROW pagination in a
// portrait/narrow frame: the table is too WIDE, and splitting rows never reduces width.
// cover-cards is the answer — the responsive table→cards move: an accent COVER, then each
// ROW becomes a CARD (the row's first cell is the card heading; every other column renders
// as a label:value pair from the <thead>), so the comparison reflows into the narrow column
// and paginates DOWN the tall frame. The full row stays together (read-down instead of
// read-across), the <thead> labels travel WITH each value, and the cards re-split between
// members on the `item` axis (recipe.bodyAxis) if a page still overflows. Parses the
// rendered <table> directly (thead labels + tbody rows) — no countAxis/partitionAxis, since
// the body form (cards) differs from the source form (a table). See #499/#500.
function coverCardsSections(openTag, inner, chrome, recipe, ratio) {
  const tableAt = inner.indexOf('<table');
  if (tableAt < 0) return null;
  const head = inner.slice(0, tableAt);
  const heading = grab(head, /<h2[^>]*>([\s\S]*?)<\/h2>/);
  if (!heading) return null;
  const theadM = inner.match(/<thead\b[^>]*>([\s\S]*?)<\/thead>/);
  const labels = theadM ? [...theadM[1].matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/g)].map((m) => m[1].trim()) : [];
  if (labels.length < 2) return null; // need a label column + ≥1 compared column
  const tbodyM = inner.match(/<tbody\b[^>]*>([\s\S]*?)<\/tbody>/);
  const rowsHtml = tbodyM ? tbodyM[1] : inner.slice(theadM ? inner.indexOf('</thead>') + 8 : tableAt);
  const rows = [...rowsHtml.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/g)]
    .map((m) => [...m[1].matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/g)].map((c) => c[1].trim()))
    .filter((r) => r.length >= 2);
  if (rows.length < 2) return null;
  // Per-page cut — same contract as cover-paginate: the manifest perPage is the portrait
  // comfortable count; the measured ratio only cuts FURTHER (denser), never looser.
  const manifestPer = Number.isInteger(recipe.perPage) && recipe.perPage > 0 ? recipe.perPage : 4;
  const ratioBased = ratio > 1 ? Math.max(1, Math.floor((rows.length / ratio) * 0.82)) : manifestPer;
  const per = Math.min(manifestPer, ratioBased);
  const cover = buildPaginateCover(openTag, head, heading, chrome, recipe, rows.length);
  const { header, footer } = chrome;
  // Body pages keep the layout class + the re-split marker; the COVER kept the engine id,
  // so body pages drop it. The heading repeats, "(cont.)" from the 2nd body page on.
  const idless = openTag.replace(/\s+id="[^"]*"/, '');
  const tag = idless.replace(/(\sclass=")([^"]*)(")/, (_, a, c, b) => `${a}${c} lat-split-native${b}`);
  const card = (row) => {
    const pairs = row.slice(1)
      .map((v, i) => `<div class="split-card-pair"><dt>${labels[i + 1] ?? ''}</dt><dd>${v}</dd></div>`)
      .join('');
    return `<li class="split-card"><h3 class="split-card-h">${row[0] ?? ''}</h3><dl class="split-card-pairs">${pairs}</dl></li>`;
  };
  const out = [cover];
  for (let i = 0; i < rows.length; i += per) {
    const cards = rows.slice(i, i + per).map(card).join('');
    const h = i === 0 ? `<h2>${heading}</h2>` : `<h2>${heading} <span class="lat-cont">(cont.)</span></h2>`;
    out.push(`${tag}${header}${h}<ul class="split-cards">${cards}</ul>${footer}</section>`);
  }
...
function carouselize(openTag, inner, recipe, ratio) {
  ...
  if (recipe.strategy === 'cover-paginate') {
    const parts = coverPaginateSections(openTag, inner, chrome, recipe, ratio);
    return parts && parts.length >= 2 ? parts : null;
  }
  if (recipe.strategy === 'cover-cards') {
    const parts = coverCardsSections(openTag, inner, chrome, recipe, ratio);
    return parts && parts.length >= 2 ? parts : null;
  }
  return null;
}
