...
function coverPaginateSections(openTag, inner, chrome, recipe, ratio) {
  ...
  const { header, footer } = chrome;
  // Cover — shared accent field (`lat-split-cover`): heading hero + optional eyebrow (a
  // leading <code> in the HEAD only, never an item's citation <code>) + the manifest intro.
  ...
  const lead = intro ? `<div class="split-cover-lead">${intro} &rarr;</div>` : '';
  const coverTag = roleOpenTag(openTag, 'content lat-split-cover form', true);
  const cover = `${coverTag}${header}${eye}<div class="split-feat-h">${heading}</div>${lead}${footer}</section>`;
