...
function readCode(inner) {
  ...
  const cols = directChildren(colsBody, 'div')
    .map((s) => {
      ...
      return { label: grab(col, /<code>([\s\S]*?)<\/code>/), pre: (col.match(/<pre[\s\S]*?<\/pre>/) || [''])[0] };
    })
    .filter((c) => c.pre);
...
function coverCodeSections(openTag, code, chrome) {
  const { header, footer } = chrome;
  const wrap = (klass, first, body) => `${roleOpenTag(openTag, klass, first)}${header}${body}${footer}</section>`;
  const out = [];
  const eye = code.eyebrow ? `<div class="split-feat-eye">${code.eyebrow}</div>` : '';
  out.push(wrap('content compare-code-split compare-code-cover form', true, `${eye}<div class="split-feat-h">${code.heading}</div><div class="split-feat-lede">Two implementations &rarr;</div>`));
  code.cols.forEach((c) => {
...
function carouselize(openTag, inner, recipe) {
  ...
  if (recipe.strategy === 'cover-sides') {
    ...
    const parts = coverWindow(openTag, { heading }, subjects, chrome, 1, cls);
    if (synthesis) {
      const { header, footer } = chrome;
  ...
  if (recipe.strategy === 'cover-decision') {
    ...
    if (!heading || !points) return null;
    const cover = { heading, eyebrow: grab(inner, /<div class="masthead-lede">[\s\S]*?<code>([\s\S]*?)<\/code>/) };
  ...
  if (recipe.strategy === 'cover-code') {
    ...
    if (!code) return null;
    const parts = coverCodeSections(openTag, code, chrome);
    return parts.length >= 2 ? parts : null;
  }
  return null;
}
