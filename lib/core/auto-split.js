...
function applyRails(html) {
  ...
  return prefix + parts
    .map((p, idx) => (p.type !== 'section' ? p.text : `${p.openTag}${p.inner}${railOf.get(idx) || ''}</section>`))
    .join('');
}
...
function resplitDoc(docHtml, overflow, capacityMap) {
  ...
  const body = splitSections(docHtml.slice(firstSlide))
    .map((p) => {
      ...
      const axis = isSplitBody && cap.split && cap.split.axis ? cap.split.axis : cap.axis;
