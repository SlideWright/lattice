...
function emitParts(openTag, parts) {
  ...
  return parts.map((inner, k) => {
    ...
    // Mark the repeated heading "(cont.)" — but idempotently: a cover-cards/cover-paginate
    // BODY page re-split here may ALREADY carry a "(cont.)" from the carousel pass, and a
    // second insertion would read "(cont.) (cont.)".
    const cont = /\blat-cont\b/.test(inner)
      ? inner
