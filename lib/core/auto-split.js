...
function autoSplitDeck(html, capacityMap) {
  ...
  const out = splitSections(html).map((p) => {
    ...
    // Split into COMFORTABLE chunks (the `sweet` count), not the `hard` max — a
    // slide packed to `hard` still crowds the footer. The TRIGGER stays overflow
    // (> hard); only genuinely-overflowing slides split, and they split into
    // ideal-sized pieces (falls back soft → hard if a layout omits sweet).
    ...
    splits += 1;
    // Continuation copies (2nd+): strip the engine's `id="…"` so the split never
    // duplicates ids (the first keeps it; `data-lattice-slide` is the real per-slide
    // key, added downstream), and mark the repeated heading "(cont.)" so a split
    // slide reads as part of the previous one. Other attributes are slide-TYPE level
    // (class, theme, paginate…) and correctly repeat.
    const contTag = p.openTag.replace(/\s+id="[^"]*"/, '');
    return parts
      .map((inner, k) => {
        if (k === 0) return p.openTag + inner + '</section>';
