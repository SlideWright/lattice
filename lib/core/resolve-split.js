 ...
 *                       The default; existing decks are unchanged.
 ...
 * lib/integrations/marp/plugins.js `headingSplit`) so the emulator and the
...
// The recognized split modes. `rule` is the named default (the Marp-compatible
// baseline); omitting the key — or giving an unrecognized value — resolves to it.
 ...
 * or unrecognized value resolves to the default ('rule') — backward-compatible
 * by construction; the deck-lint surfaces a typo so it doesn't silently fall
 * back. An optional `override` (e.g. an env/CLI flag) wins when supplied.
 */
