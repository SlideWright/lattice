...
export function authorCss(renderedHtml, fmText) {
	...
	// MENTIONS a `<style>` — `<!-- TODO: drop the <style> hack from the old deck -->` — was refused,
	// and told a `<style>` element it does not have is the reason.
	const html = String(renderedHtml ?? '').replace(/<!--[\s\S]*?-->/g, '');
