...
/**
 * Does the author's own CSS still select the SAME SLIDES once the deck is shorter?
 *
 * THE ONE CROSS-SLIDE MECHANISM THAT COMPARING TWO RENDERS CANNOT SEE, asked as its own question.
 * `section:nth-of-type(3) p { display: none }` written on a slide the view KEEPS is byte-identical in
 * both documents, and so is every slide's markup — nothing a diff can reach has moved. What moved is
 * the slide the selector counts to, so a paragraph the author had hidden comes back in the file that
 * is sent. Measured on the real CLI: `display: none` and 0x0 in the preview, `display: block` and
 * 770x36 in the export, and the same shape with `::after { content: "CONFIDENTIAL - INTERNAL ONLY" }`
 * drops the marking from the exported PDF, pdftotext 1 to 0.
 *
 * SO ASK THE BROWSER WHICH SLIDES EACH RULE TOUCHES, in the deck as authored and in the deck as
 * projected, and require the second to be the first mapped through `kept`. That is exact: no selector
 * is parsed, so `:is()`, `:has()`, CSS escapes, attribute selectors and every spelling yet to be
 * invented are covered by construction. A SCANNER for the same question was built first and removed;
 * measured against 24 real idioms it refused 7 of 12 harmless ones and missed 6 of 12 dangerous ones,
 * because `section[id="3"]` selects by position too and the space of spellings has no end.
 *
 * Measured on this comparison: 8 of 8 leaks caught — including all five spellings the scanner missed
 * — and 0 false alarms over 14 CSS idioms real authors write, `* + *` and `li:nth-child(2)` among
 * them, both of which the scanner refused.
 *
 * THE CALLER SUPPLIES THE MATCH SETS, for the reason it supplies the renderer: `lib/core` must not
 * reach for a browser, and a boolean saying "I checked" is the shape this file already regrets.
 *
 * @param {Record<string, number[]>} authored  selector -> authored slides it matches in the full deck
 * @param {Record<string, number[]>} shipped   selector -> slides it matches in the projection
 * @param {number[]} kept                      authored indices, in shipped order
 * @returns {{ selector: string, was: number[], now: number[] } | null} the first rule that moved
 */
export function selectorSlideDrift(authored, shipped, kept) {
	for (const [selector, was] of Object.entries(authored ?? {})) {
		// What this rule SHOULD land on: the slides it matched, minus the withheld ones, renumbered.
		const want = was.filter((slide) => kept.includes(slide)).map((slide) => kept.indexOf(slide));
		const now = shipped?.[selector] ?? [];
		if (want.length !== now.length || want.some((v, i) => v !== now[i])) return { selector, was: want, now };
	}
	return null;
}

/**
 * Does this deck carry CSS of its own — the precondition for the check above?
 *
 * The gate, and the reason the browser cost falls on almost nobody: a rule can only select a slide by
 * position if the deck has rules at all. Of the 150 decks in `examples/`, 2 carry a `<style>` and none
 * declares front-matter `style:`, so 148 skip the comparison entirely.
 *
 * Front matter is read here; a slide-body `<style>` is NOT, because a ```css fence teaching an example
 * renders as `<pre><code>` and applies to nothing, which no regex over the markdown can tell. The
 * caller reads those off the PARSED document (`section style`) and passes them in, along with any
 * `--css` file — the same "ask the parser, not the bytes" this branch paid for six times on `_lens`.
 *
 * @param {string} source    the deck, front matter included
 * @param {...string} extra  CSS the caller found elsewhere (the parsed body, a `--css` file)
 * @returns {boolean}
 */
export function carriesAuthorCss(source, ...extra) {
	const src = normalizeSourceText(String(source ?? ''));
	if (readGlobalStyle(frontMatterBlockOf(src)).trim()) return true;
	return extra.some((css) => String(css ?? '').trim().length > 0);
}
 ...
 *
 * ONE MEMBER OF THIS CLASS IS NOT CHECKED, and the bound is stated rather than papered over. CSS that
 * selects a slide by POSITION — `section:nth-of-type(3) p { display: none }` — lands on a different
 * slide once the deck is shorter, and comparing the two renders cannot see it: the stylesheet is
 * byte-identical on both sides and so is every slide's markup. Only what a BROWSER computes differs,
 * so catching it means rendering both decks in a real browser and comparing computed style — measured
 * at +5.2 s per export, against a class with zero observed instances. All 150 decks in `examples/`
 * were scanned: 2 carry a `<style>` at all, both machine-written by the Studio ("embedded finishes"),
 * both selecting by class; none selects by position, and none uses front-matter `style:`.
 *
 * A scanner for it WAS built and is deliberately gone. Measured against 24 real CSS idioms it refused
 * 7 of 12 harmless ones — `p:not(:last-child)`, and a deck whose only CSS was a comment — while
 * missing 6 of 12 dangerous ones, because `section[id="3"]` and `sec\74 ion:nth-of-type(3)` select by
 * position too and the space of spellings has no end. Wrong half the time in each direction is worse
 * than a stated bound: it broke CSS authors write for a leak nobody writes. The browser comparison is
 * the sound instrument and is filed as its own change, to be built when a real deck needs it.
 *
...
export const REFUSAL_REASONS = {
	...
	'positional-css': 'a rule in the deck\u2019s own CSS selects slides by POSITION, so it lands on a different slide once the projection makes the deck shorter \u2014 a paragraph you hid can come back in the file you send, and a classification marking can vanish from it. Nothing in the markup moves, so this is the one thing comparing two renders cannot show; the export asks the browser which slides each of your rules matches, before and after. Select the slide by a class you set on it (`<!-- _class: hushed -->`, then `section.hushed \u2026`) \u2014 a class travels with the slide instead of counting to it',
