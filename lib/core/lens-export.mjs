...
export const POSITION_NEUTRALIZERS = Object.freeze({
	...
	attrs: 'slide ids, the authored number, and the two pagination counters',
...
function documentGlobals(html) {
	...
	let out = '';
	for (const m of src.matchAll(/<(style|script)\b[^>]*>[\s\S]*?<\/\1\s*>/gi)) out += ` ${m[0]}`;
	for (const m of src.matchAll(/<link\b[^>]*>/gi)) out += ` ${m[0]}`;
	return normalize(out);
}

/**
 * CSS THAT PICKS A SLIDE BY ITS POSITION — the one cross-slide mechanism a comparison of RENDERS
 * cannot see, and so the one this file refuses on sight instead of measuring.
 *
 * Every other mechanism here is caught by rendering the deck twice and diffing. This one is not, and
 * the reason is structural rather than an oversight. `section:nth-of-type(3) p:nth-of-type(2) {
 * display: none }` written on a slide the view KEEPS is byte-identical in all three renders — the
 * stylesheet did not move and neither did any slide's markup. What moved is the slide the selector
 * counts to. Measured on the real CLI: the sentence "We expect to lose the Acme lawsuit and take a
 * $40M charge" is `display: none` in the deck the sender previews and `display: block`, 770×36
 * pixels, in the file they send; the same shape with `::after { content: "CONFIDENTIAL - INTERNAL
 * ONLY" }` drops the stamp from the exported PDF, `pdftotext` 1 → 0. Seeing either would take
 * comparing COMPUTED style in a real browser, which an export that has not launched one cannot do.
 *
 * AND THE REFUSAL MESSAGE USED TO WALK AUTHORS INTO IT. "Put deck-wide settings in the front matter,
 * or on a slide every view keeps" is the remedy printed for a real cross-slide find — and following
 * it moves a CAUGHT case into this uncatchable one, because front-matter `style:` is exactly where
 * such a rule then lives. So front matter is scanned too.
 *
 * WHAT IT FLAGS is narrow on purpose: a structural pseudo-class or sibling combinator applied to
 * `section` (the slide element), and any mention of the two slide-index attributes — which the
 * projection RENUMBERS, so `section[data-authored-slide="0"]` retargets even on a kept slide.
 * `li:nth-child(2)` inside a slide counts list items, not slides, and is left alone. Over the 147
 * example decks, 2 carry author CSS at all and none trips this.
 */
/**
 * FOUND BY WALKING, NOT BY ONE REGEX. The first attempt spelled the qualifier chain as
 * `section(?:[.#[:][^\s,{>+~]*)*` — a repeated group whose body can start with the same characters
 * the group does, so `section####…` has exponentially many parses and CodeQL flagged both patterns
 * high-severity. Author CSS is untrusted input on the docs-site Studio path (HARD RULE #22), so a
 * catastrophic-backtracking regex over it is not a style question.
 *
 * The walk asks the question directly instead: find each structural pseudo-class (or sibling
 * combinator), then read the COMPOUND SELECTOR it belongs to by scanning back to the nearest
 * delimiter. Every scan is linear, and the result is easier to read than the pattern it replaced —
 * `li:nth-child(2)` has the compound `li` and is left alone, which is the distinction that matters.
 */
const STRUCTURAL_PSEUDO = /:(?:nth-(?:last-)?(?:child|of-type)|(?:first|last|only)-(?:child|of-type))\b/gi;
// Quotes and `;` are delimiters too, because front-matter CSS arrives wrapped: `style: "section:nth-of-type(3) …"`
// puts a `"` immediately before the compound, and reading it as part of the selector made the whole
// single-line front-matter form invisible — to the regex this replaced as well as to the walk.
const SELECTOR_DELIMITER = ' \t\r\n,{}>+~()\'";';
const SLIDE_INDEX_ATTR = /data-(?:authored|lattice)-slide/i;

/** The compound selector ending at `at` — everything back to the nearest selector delimiter. */
function compoundEndingAt(css, at) {
	let from = at;
	while (from > 0 && !SELECTOR_DELIMITER.includes(css[from - 1])) from -= 1;
	return css.slice(from, at);
}

/** Does this compound select a SLIDE? `section…`, or nothing/`*`, which select every element. */
const selectsASlide = (compound) => {
	const c = compound.trim().toLowerCase();
	return c === '' || c === '*' || c.startsWith('section');
};

/** A structural pseudo-class whose own compound selects a slide, e.g. `section:nth-of-type(3)`. */
function positionalPseudo(css) {
	STRUCTURAL_PSEUDO.lastIndex = 0;
	for (let m = STRUCTURAL_PSEUDO.exec(css); m; m = STRUCTURAL_PSEUDO.exec(css)) {
		const compound = compoundEndingAt(css, m.index);
		if (selectsASlide(compound)) return `${compound}${m[0]}`;
	}
	return null;
}

/** A `+` or `~` whose left-hand compound selects a slide, e.g. `section.cover + section`. */
function slideSibling(css) {
	for (let at = 0; at < css.length; at += 1) {
		if (css[at] !== '+' && css[at] !== '~') continue;
		let before = at;
		while (before > 0 && ' \t\r\n'.includes(css[before - 1])) before -= 1;
		const compound = compoundEndingAt(css, before);
		if (compound && selectsASlide(compound)) return `${compound} ${css[at]}`;
	}
	return null;
}

/**
 * The author's own CSS, read from the SOURCE rather than the render — front-matter `style:` and every
 * `<style>` element, whatever their case. Reading the render instead would work today and break the
 * day the engine inlines a theme, at which point every deck in the tree would refuse at once.
 */
function authorCss(source) {
	const src = normalizeSourceText(String(source ?? ''));
	const fm = frontMatterBlockOf(src);
	let out = /^style\s*:/m.test(fm) ? fm : '';
	for (const m of src.matchAll(/<style\b[^>]*>([\s\S]*?)<\/style\s*>/gi)) out += `\n${m[1]}`;
	return out;
}

/** The first position-dependent selector in a deck's own CSS, or null. */
export function positionalCss(source) {
	const css = authorCss(source);
	if (!css) return null;
	for (const [what, find] of [
		['a structural pseudo-class on `section`', positionalPseudo],
		['a sibling combinator between slides', slideSibling],
		['a slide-index attribute the projection renumbers', (text) => (SLIDE_INDEX_ATTR.exec(text) ?? [null])[0]],
	]) {
		const hit = find(css);
		if (hit) return { what, selector: hit.trim() };
	}
	return null;
}
...
export function crossSlideDrift(fullSource, projected, kept, render) {
	...
	// AND THE ONE THING RENDERING TWICE CANNOT ANSWER, asked of the source instead — but only when the
	// projection is a REDUCTION. A view that keeps every slide moves nothing for a positional selector
	// to count differently, so refusing it would be refusing `--lens full`.
	if (authoredOrder(full).split(',').length > kept.length) {
		const positional = positionalCss(fullSource);
		if (positional) return { authored: -1, channel: 'positional-css', hop: 1, ...positional };
	}
...
export const REFUSAL_REASONS = {
	...
	'positional-css': 'the deck styles slides by POSITION — a `:nth-of-type` on `section`, a sibling combinator between slides, or a `data-authored-slide` attribute. A projection renumbers the slides those selectors count, so the rule lands on a different slide in the file that ships — and unlike every other check here, that cannot be seen by comparing the rendered markup, because nothing in the markup moved. Select the slide by a class you set on it (`<!-- _class: … -->`) instead',
