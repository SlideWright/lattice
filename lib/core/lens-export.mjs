...
export const REFUSAL_REASONS = {
	...
	'positional-css': 'a slide this view KEEPS shows something different once the deck is shorter, and nothing in the markup moved — which is the signature of CSS that selects slides by POSITION. `section:nth-of-type(3) { … }` counts to the third slide, and after a projection that is a different slide: a paragraph you hid can come back in the file you send, and a classification marking can land on the wrong slide or none. Comparing the two renders cannot show this, because the stylesheet and every slide\u2019s markup are identical on both sides \u2014 so the export renders the deck both ways and compares what a reader actually SEES on each slide it keeps. Style the slide through a class you set on it (`<!-- _class: hushed -->`, then `section.hushed …`): a class travels with the slide instead of counting to it',
