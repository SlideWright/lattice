 ...
 * that hides it: `section.lens-hole` is specificity (0,1,1) and loses to the engine's own
 * `section.form:has(> .cell-stage)` at (0,2,1) — measured, a hole styled from a deck rendered
 * `display:flex` and printed a page carrying its content. The engine emits `.cell-stage` even for
 * an empty slide, so there is no escaping that rule by being empty.
 *
