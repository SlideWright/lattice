...
import { applyTag, lensEligibility, parseLensRegistry, parseSlideTags, readerLenses, stripExtraLensTags, unknownLensTokens, upsertLensRegistry } from '@workwel/lente';
...
/** An empty registry, so `unknownLensTokens` reports EVERY id a slide's directives name rather than
 *  only the typos. Frozen because it is shared across calls and must never acquire a lens. */
const EVERY_TOKEN_IS_UNKNOWN = Object.freeze({ lenses: Object.freeze([]) });
...
export function projectForExport(source, requestedIds, options = {}) {
	...
	// a file.
	//
	// EVERY DIRECTIVE ON THE SLIDE, not just the first. `parseSlideTags` stops at the first, which
	// is what made a second tag invisible to the prune in the first place; asking
	// `unknownLensTokens` against an EMPTY registry returns every token in every directive body
	// instead, which is exactly the question here — what ids does this slide still name?
	for (const chunk of shipped) {
		for (const id of unknownLensTokens(chunk.chunk, EVERY_TOKEN_IS_UNKNOWN)) {
			if (!exported.has(id) && id !== FULL_VIEW) return { ok: false, lensId: id, reason: 'unprunable' };
		}
	}
...
export const REFUSAL_REASONS = {
	...
	'unprunable': 'a slide names a view this export does not carry, in a comment the projection cannot safely rewrite — if it is a real tag, put it on a line of its own; if it is a documented example, note that a fence opened inside a blockquote is not yet recognized as a fence (#2034)',
};
