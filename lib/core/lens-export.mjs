...
/**
 * A withheld slide, emitted as a section that HOLDS ITS POSITION and shows nothing.
 *
 * `_class: lens-hole` is what the engine's own rule hides. The class has to come from a directive
 * rather than a raw attribute because the projection emits MARKDOWN, and it has to be the engine
 * that hides it: a hole styled from a DECK loses the cascade fight outright — measured, one
 * rendered `display:flex` and printed a page carrying its content. The rule is
 * `lib/base/base.lens-hole.css`, last in the bundle and `!important`, and that file's own header
 * says why both are load-bearing (the short version: the selector may not name a class a deck can
 * switch off, and `form: off` switches off the one the first version leaned on).
 *
 * THE CLASS IS NOT A SECRET AND CANNOT BE MADE ONE. A deck can type `_class: lens-hole` on a real
 * slide, or `class: lens-hole` to hole every slide after it, and nothing in a markdown pipeline can
 * tell the projection's marker from an author's. Measured before `holeDrift` existed: a 3-slide
 * deck with the global form exported as a ONE-page PDF, silently, exit 0 — while the withheld
 * bodies rode out verbatim in the `.html` and in the player's base64 envelope. So the class is
 * checked rather than trusted. `holeDrift` reads the RENDERED document and requires the set of
 * holes to equal the set of slides the projection withheld, which makes a forged hole a refusal
 * whether or not a reader view is in play — and makes an UN-hidden hole one too.
 *
 * The body is an empty COMMENT, not a blank line. A blank line makes the deck body open with a
 * separator, which Marpit's leading-group rule renders as one section fewer — that shifted every
 * authored number by one and refused 147 of 147 example decks the last time it was tried.
 *
 * NOTHING ELSE IS IN IT. A hole discloses that a slide existed at this position and no more: no
 * heading, no class of the original, no `_lens` tag, no notes. For every rasterized format that is
 * moot — the exporter skips it and the artifact has no trace — and for `.html`, where the CSS must
 * stay live and so must the hole, it is the whole of the cost.
 */
const HOLE_BODY = '\n<!-- _class: lens-hole -->\n\n<!-- -->\n';
...
export function projectForExport(source, requestedIds, options = {}) {
	...
	// A VIEW'S INDICES ARE AUTHORED INDICES, unremapped — because the projection no longer moves
	// anything. This used to map each authored index to its rank in the shortened deck, and that
	// remap was load-bearing in the worst way: the carrier's baked map is read by position, so any
	// disagreement between the remap and the emitted deck showed a reader a slide their view
	// excludes. Two separate rewrites did exactly that. Holes delete the remap along with the class
	// of bug: what the author numbered is what ships.
	const views = resolved.map((v) => ({ ...v, indices: [...v.indices] }));
	...
	// EVERY SLOT IS EMITTED — a kept slide's pruned body, or a HOLE that holds its position.
	//
	// Deleting a withheld slide renumbers every slide after it, and that renumbering is the root of
	// the one disclosure class no comparison of two renders can see: `section:nth-of-type(3)` is
	// DOM position, so a rule the author aimed at slide 3 lands on whatever slide moved into third
	// place. Three detectors were built to catch that and all three lost, each to an unbounded
	// space — spellings, then selector syntax, then ways to hide a thing.
	//
	// A hole ends the class instead of detecting it. `nth-of-type` counts a `display:none` element
	// (it is a structural selector, not a rendered one — measured in Chrome), so the author's rule
	// lands exactly where they aimed, in both files. Under print the hole produces no page, so it
	// costs the artifact nothing; the raster exporters skip it explicitly.
	//
	// It is the shape `emptyWithheld` has always built as an internal PROXY, promoted to being what
	// actually ships. That is why hop 1 of the cross-slide check worked at all, and it is why the
	// neutralizer list can now shrink: the shipped deck is the same length as the authored one.
	const keptSet = new Set(kept);
	const holes = [];
	const slots = entries.map((entry, i) => {
		if (keptSet.has(i)) return pruneTags(entry.chunk, exported, reg);
		holes.push(i);
		return HOLE_BODY;
	});
	const chunks = kept.map((i) => slots[i]);
	...
	// Every slot is present, so the array index IS the authored index and each separator is the one
	// the author wrote in front of that slide. The old join had to look the separator up through
	// `kept` because the array had gaps in it.
	const joined = slots.reduce((acc, chunk, at) => {
		if (at === 0) return chunk;
		const sep = entries[at].sep ?? '---';
		return `${acc}${acc.endsWith('\n') ? '' : '\n'}\n${sep}\n${chunk}`;
	}, '');
	// A body that OPENED with a separator gets it back. Slide 0 always survives now — as itself or
	// as a hole — so the condition that used to guard this is gone with the gap it guarded against.
	...
	if (shipped.length !== slots.length) return { ok: false, lensId: ids.join(','), reason: 'unsplittable' };
 ...
 *
 * So the block is projected like the registry: entries for withheld slides are DROPPED, and every
 * entry that survives KEEPS ITS NUMBER. The block rule comes from `blockLines`, exported from the
 * parser that owns it, rather than restated here (HARD RULE #1) — a second copy of "where does a
 * front-matter block end" is precisely the duplication this branch has already paid for.
 *
 * KEEPING THE NUMBER IS THE FIX, AND IT REVERSES WHAT THIS FUNCTION USED TO DO. While the projection
 * DELETED withheld slides, a survivor really did move — slide 3 of 5 became slide 2 of 3 — so the
 * entries were renumbered to their rank among the kept. Position-holding projection ended that: the
 * shipped deck has every slot, so slide 3 is still in position 3 and the renumbering became a shift
 * by the number of preceding holes. Measured on the real CLI, a 5-slide deck exporting slides 1/3/5:
 *
 *     caption 1 -> slide 1   (right, by luck — nothing precedes it)
 *     caption 2 -> a HOLE    (silently lost)
 *     caption 3 -> slide 3's caption spoken over SLIDE 5
 *     slide 5   -> fell back to generated speech
 *
 * which is the same MISNARRATION this function was written to prevent, re-introduced by arithmetic
 * that had stopped being true. `0ba2c79` deleted three other pieces of the same arithmetic and
 * missed this one; all three adversarial lenses found it independently.
 */
function pruneCaptions(fm, kept) {
	...
	// The shipped deck keeps every slot, so a surviving entry's number is already right. What is
	// left to decide is only whether an entry survives at all.
	const survives = new Set(kept.map((authored) => authored + 1));
 ...
 *
 * IT IS SKIPPED, NOT STRIPPED. Removing it with `.replace(…, '')` is an incomplete multi-character
 * sanitization — one pass, so a removal that brings two halves together leaves a live construct
 * behind, and CodeQL flags the shape on sight (high, this PR; the second time in this file). Nothing
 * here is emitted, so the instance was harmless and the shape is still the objection. `documentGlobals`
 * already walks the elements one at a time, so declining to count one is a filter rather than a
 * rewrite, and a filter cannot join anything.
 */
const MERMAID_SCOPED_STYLE = /^\s*#lattice-mmd-\d+/;

function documentGlobals(html) {
	const src = String(html);
	let out = '';
	for (const m of src.matchAll(RAWTEXT_ELEMENT)) {
		if (m[1].toLowerCase() === 'style' && MERMAID_SCOPED_STYLE.test(m[2])) continue;
		out += ` ${m[0]}`;
	}
...
/** The class the projection marks a withheld slide with, and `base.lens-hole.css` hides. */
export const HOLE_CLASS_NAME = 'lens-hole';
const HOLE_CLASS_ATTR = /(?:^|\s)lens-hole(?:\s|$)/;

/**
 * ARE THE HOLES IN THIS DOCUMENT EXACTLY THE SLIDES THE PROJECTION WITHHELD?
 *
 * The hole is a marker in MARKDOWN, and a markdown pipeline cannot tell the projection's marker
 * from one an author typed. `<!-- _class: lens-hole -->` on a real slide, or the running
 * `<!-- class: lens-hole -->` form which holes every slide after it, both reach the renderer as
 * the same class the export puts there. Measured before this function existed: a 3-slide deck with
 * the running form exported as a ONE-page PDF, silent, exit 0 — and the two slides it swallowed
 * shipped verbatim in the `.html` written by the same command and in the player's base64 envelope.
 * The class was doing a security-relevant job on nothing but the honor system.
 *
 * So it is checked against the projection instead of trusted. `expected` is the set of authored
 * indices the view withheld — `total \ kept`, which the kernel already knows — and the rendered
 * document has to agree exactly. That closes both directions at once:
 *
 *   - a hole the projection did not put there is an author hiding a slide from the artifact while
 *     its bytes still ship, and it refuses whether or not `--lens` was passed at all (with no
 *     reader view, `expected` is empty and ANY hole is drift);
 *   - a slide the projection withheld that comes back WITHOUT the class is a hole that lost the
 *     cascade — the `form: off` defect, where every hole rendered as a live empty page and the PDF
 *     disclosed the withheld positions as blanks.
 *
 * IT READS THE RENDER, NOT THE SOURCE, and that is the point: the source is what we emitted, the
 * render is what the artifact is built from, and everything that can go wrong between them —
 * a directive read differently, a class stripped, a cascade lost — happens in that gap. It is the
 * same reason `crossSlideDrift` renders twice rather than reasoning about markdown.
 *
 * A section with no `data-authored-slide` is skipped rather than failed: an appended slide (the
 * auto-glossary) carries no authored number and is not a hole either way.
 *
 * @param {string} html      the rendered document
 * @param {number[]} expected authored indices that MUST be holes; every other slide must not be
 * @returns {null | { saw: number[], want: number[] }} null when they agree
 */
export function holeDrift(html, expected) {
	const want = [...new Set(expected ?? [])].sort((a, b) => a - b);
	const saw = new Set();
	for (const piece of splitSections(String(html))) {
		if (piece.type !== 'section') continue;
		const at = piece.openTag.match(/data-authored-slide="(\d+)"/);
		if (!at) continue;
		const cls = piece.openTag.match(/\sclass="([^"]*)"/);
		if (cls && HOLE_CLASS_ATTR.test(cls[1])) saw.add(Number(at[1]));
	}
	const got = [...saw].sort((a, b) => a - b);
	return got.length === want.length && got.every((v, i) => v === want[i]) ? null : { saw: got, want };
}
 ...
 */
// The proxy withholds a slide EXACTLY the way the projection does — same hole, same class. It was a
// bare empty comment while the projection deleted its withheld slides and the proxy existed only to
// hold deck length fixed for hop 1. Now that the projection emits holes too, any difference between
// the two shapes is drift the comparison would report against the deck, and the class alone was
// enough to refuse every projected export.
const WITHHELD_BODY = HOLE_BODY;
 ...
 *
 * Hop 2 is what keeps this a statement about the artifact rather than about a stand-in for it. When it
 * fires, the projection's own edits changed a kept slide's render, which is equally a refusal.
 *
 * WHERE HOP 2 IS AND IS NOT MEASURED, stated plainly because an earlier version of this paragraph got
 * it wrong. It claimed "over the 147 example decks it fires 0 times, so the proxy is MEASURED to stand
 * for the projection." It fires 0 times there because there is NOTHING TO COMPARE: those decks declare
 * no reader views, so the sweep hands this function `emptyWithheld(src, kept)` as the projection — the
 * same string it builds its own proxy from — and hop 2 diffs a render against itself. An independent
 * checker proved it by deleting both hop-2 comparisons and watching all six corpus shapes stay green.
 * Hop 2 is covered by the two dedicated tests that build a three-document stub, and by the integration
 * tier, which projects a deck that really does declare views. A count from a corpus that cannot
 * exercise an arm is not evidence about that arm.
 *
...
export function crossSlideDrift(fullSource, projected, kept, render) {
	...
	for (let at = 0; at < kept.length; at++) {
		...
		// BOTH SIDES ARE INDEXED BY AUTHORED NUMBER. This read `afterText.get(at)` — the rank among
		// kept slides — because the projection renumbered what it shipped. Holes keep the authored
		// numbering, so rank and authored number are the same thing again and the rank is wrong.
		if (middleText.get(kept[at]) !== afterText.get(kept[at])) return { authored: kept[at], channel: 'section', hop: 2 };
	}
	return null;
}
 ...
 *
 * CSS can address a slide by POSITION, and a projection changes what position means. `crossSlideDrift`
 * above cannot see it, and that is not a gap to be closed by trying harder: the stylesheet is
 * byte-identical on both sides and so is every slide's markup, so there is nothing in either document
 * for a comparison to find.
 *
 * WHAT IS STILL AT RISK IS NARROWER THAN IT WAS, AND THE DIFFERENCE IS MEASURED. While withheld
 * slides were DELETED, every slide after the first one moved and `section:nth-of-type(3)` landed on
 * whatever slid into third place. Position-holding projection closed that family by construction —
 * the slot survives — and it was checked rather than assumed: on real exported files,
 * `section:nth-of-type(4)`, `section:nth-child(6)` and `section:last-of-type` each resolve to the
 * same slide in the projection as in the full deck.
 *
 * A CSS COUNTER DOES NOT, and that is the one this warning is now for. `nth-of-type` is a STRUCTURAL
 * selector and counts a `display:none` element; counters live on the BOX tree, and a hidden element
 * generates no box, so it does not increment. Measured on two real PDFs of one deck: a `counter()`
 * heading reads `#4` under `--lens full` and `#2` under `--lens brief`. `counters()`, a
 * `counter-reset` chain and `::marker` numbering are the same shape, and so — by design rather than
 * by accident — is the visible page number.
 *
 ...
 *
 * MEASURED COST: 3 decks in `examples/` — `finish-backdrops.md` and `finish-override.md`
