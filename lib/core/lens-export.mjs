...
import { applyTag, lensEligibility, parseLensRegistry, parseSlideTags, readerLenses, upsertLensRegistry } from '@workwel/lente';
...
function pruneTags(slideSrc, exported, reg) {
	// ONLY THE SLIDE'S FIRST `_lens` COMMENT IS PRUNED, because only the first is READ.
	// A second one on the same slide survives into the artifact naming a view the recipient
	// was not given — a narrow, known leak, accepted deliberately. Sweeping it needs to know
	// which text is quoted, and a scanner that got inline code wrong shipped a corrupted
	// example, a worse leak and a silently disabled sweep in one commit. The reasoning is in
	// `docs/src/lib/lente/tags.ts`, above `emitTokens`; closing it properly means one shared
	// inline-context reader for every consumer, which is a slice rather than a patch.
	let out = slideSrc;
