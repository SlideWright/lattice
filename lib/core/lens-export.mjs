...
/**
 * Attributes a slide legitimately carries a DIFFERENT value for once the deck is shorter.
 *
 * `id` and `data-lattice-slide` are the rendered position; `data-authored-slide` is the authored
 * one, which the projection renumbers on purpose; the two pagination counters are "page N of M".
 * Everything else — `data-footer`, `data-header`, `class`, `style` (which carries the footer and
 * header TEXT), `data-background-color` — is content, and a difference there is the defect.
 */
const POSITION_ATTRS = /\s(?:id|data-lattice-slide|data-authored-slide|data-lattice-pagination|data-lattice-pagination-total)="[^"]*"/g;

/** …and the PAGE NUMBER the pagination cell prints. Stripping the attribute alone left the rendered
 *  text, which made a shorter deck look like a drifted one: 7 of 15 view subsets of the demo deck
 *  reported drift on nothing but `<span class="lat-pagination">7</span>` becoming `1`. */
const POSITION_TEXT = /(<span class="lat-pagination[^"]*">)[^<]*(<\/span>)/g;

/** Rendered sections grouped by the authored slide they belong to. A single authored slide renders
 *  as several sections when it heading-splits or overflows, so the group is the unit of comparison. */
function sectionsByAuthored(html) {
	const by = new Map();
	for (const sec of String(html).match(/<section\b[\s\S]*?<\/section>/g) ?? []) {
		const m = sec.match(/data-authored-slide="(\d+)"/);
		if (!m) continue;
		const at = Number(m[1]);
		by.set(at, (by.get(at) ?? '') + sec.replace(POSITION_ATTRS, '').replace(POSITION_TEXT, '$1$2'));
	}
	return by;
}
 ...
 * anyway, through at least three mechanisms an adversarial pass measured on the real CLI:
 *
 ...
 *     `backgroundColor:`, `header:` and `paginate:` do the same.
 ...
 *
 * So this asks the question one scope wider, the same move that has worked every other time in this
 * file: render the deck BOTH ways and compare each kept slide against itself. It cannot be wrong
 * about markdown, because it is not reasoning about markdown.
 *
 ...
 * @returns {{ authored: number, position: number } | null} the first slide that drifted, or null
 */
export function crossSlideDrift(fullSource, projected, kept, render) {
	const before = sectionsByAuthored(render(String(fullSource ?? '')));
	const after = sectionsByAuthored(render(String(projected ?? '')));
	for (let at = 0; at < kept.length; at++) {
		if (before.get(kept[at]) !== after.get(at)) return { authored: kept[at], position: at };
	}
	return null;
}
...
export const REFUSAL_REASONS = {
	...
	'cross-slide': 'a slide this view KEEPS renders differently once the slides around it are gone — usually a `footer:`, `header:`, `class:` or `paginate:` directive, or a [link]: definition, set on a slide the view excludes. Move it to a slide the view keeps, or into the deck front matter',
};
