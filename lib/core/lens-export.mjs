...
/**
 * Where does the comment opened at `open` end, or -1 if it never does?
 *
 * THE ABRUPT-CLOSING FORMS ARE THE WHOLE REASON THIS IS A FUNCTION. `<!-->` and `<!--->` are complete,
 * empty comments per the HTML spec's comment-start and comment-start-dash states — so markup after
 * them is LIVE. Measured in Chrome: `<!--><style>p{color:red}</style>` really does paint the paragraph
 * red. A scan that looked only for `-->` treated everything after `<!-->` as commented out, which is
 * the leak direction: it would have certified a deck whose positional stylesheet a browser applies.
 * `--!>` also ends a comment (the comment-end-bang state), and is included for the same reason.
 *
 * @param {string} html the markup
 * @param {number} open index of the `<!--` that opens the comment
 * @returns {number} index where the comment ends, or -1 when it runs to the end of the document
 */
function commentEnd(html, open) {
	// `<!-->` — the comment closes on the `>` immediately after the opener.
	if (html[open + 4] === '>') return open + 4;
	// `<!--->` — one dash, then the `>`.
	if (html[open + 4] === '-' && html[open + 5] === '>') return open + 5;
	const ends = [html.indexOf('-->', open + 4), html.indexOf('--!>', open + 4)].filter((i) => i !== -1);
	return ends.length ? Math.min(...ends) : -1;
}

/**
 * Does `re` match anywhere in `html` that an HTML COMMENT does not cover?
 *
 * AN HTML COMMENT IS NOT MARKUP. markdown-it passes comments through verbatim, so a deck that only
 * MENTIONS a `<style>` — `<!-- TODO: drop the <style> hack from the old deck -->` — was refused, and
 * told a `<style>` element it does not have was the reason.
 *
 * IT WALKS RATHER THAN STRIPPING, and that is not a style preference. Removing comments with
 * `.replace(/<!--[\s\S]*?-->/g, '')` is an incomplete multi-character sanitizer — one pass, so a
 * removal that BRINGS TWO HALVES TOGETHER leaves a live construct behind, and CodeQL flags the shape
 * on sight (high, this PR). The shape is the objection even where the instance is benign: this
 * function only asks a question, it never emits the string, so the worst a stripping bug could do here
 * is answer wrong. A walk cannot bring halves together, because it never rewrites anything.
 *
 * The comment grammar is deliberately the CONSERVATIVE reading, because the two failure directions
 * are not equal: treating a real comment as markup refuses a deck that could have shipped, and
 * treating real markup as a comment SHIPS A LEAK. So a comment opens only at a literal `<!--` and
 * closes at `-->` or `--!>` (both end one per the HTML spec), and an unclosed `<!--` swallows nothing
 * — the scan simply stops, leaving everything after it in scope. Every ambiguity resolves toward
 * refusing.
 *
 * @param {string} html the markup to search
 * @param {RegExp} re   a sticky-able global pattern; `lastIndex` is driven by this function
 * @returns {boolean} true when at least one match falls outside every comment
 */
function outsideComments(html, re) {
	re.lastIndex = 0;
	for (let m = re.exec(html); m; m = re.exec(html)) {
		// Is this match inside a comment? Find the last `<!--` at or before it, and ask whether that
		// comment had already closed. Scanning backwards from the match keeps the whole walk linear in
		// the number of MATCHES rather than in the length of the document.
		const open = html.lastIndexOf('<!--', m.index);
		if (open === -1) return true;
		const close = commentEnd(html, open);
		// An unclosed comment runs to the end of the document — a browser swallows everything after it,
		// measured — so a match inside it is not live markup. A comment that closed before the match
		// leaves the match in the open.
		if (close !== -1 && close < m.index) return true;
	}
	return false;
}

/**
 * DOES THIS DECK CARRY CSS OF ITS OWN? A reducing projection refuses if it does.
 *
...
export function authorCss(renderedHtml, fmText) {
	...
	if (outsideComments(html, /<style\b/gi)) return { channel: 'style' };
	...
	if (outsideComments(html, /<link\b/gi)) return { channel: 'link' };
	...
	if (outsideComments(html, /<script\b/gi)) return { channel: 'script' };
	return null;
}
...
export const REFUSAL_REASONS = {
	...
	'author-css': 'this deck carries CSS of its own, and a reader view cannot be verified against it. CSS can select a slide by POSITION — `section:nth-of-type(3) { … }` counts to the third slide — and a projection makes every slide after the first withheld one a different slide, so the rule lands somewhere else in the file that ships: a paragraph you hid can come back, and a classification marking can print on the wrong slide. Nothing in either document shows this, because the stylesheet and every slide\u2019s markup are identical on both sides. Style the slide through a class you set on it (`<!-- _class: hushed -->`, then `section.hushed …`): a class travels with the slide instead of counting to it',
