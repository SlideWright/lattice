 ...
 *
 * Rendered HTML is the property itself rather than a proxy for it. `<!-- … -->` is stripped from both
 * sides because removing a directive is exactly what the prune is FOR, and whitespace is collapsed so
 * the comparison cannot fail on a blank line nobody can see. Cost: one extra render per pruned slide,
 * measured at ~1 ms on the demo deck against a ~10 s export. Nothing worth trading correctness for.
 */
function renderedShape(slideSrc) {
	return boundaryParser
		.render(String(slideSrc ?? '').replace(/<!--[\s\S]*?-->/g, ''), {})
		.replace(/\s+/g, '');
}
