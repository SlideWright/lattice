 ...
 */
const SECTION_POSITIONAL =
	/(?:^|[\s,>+~])section(?:[.#[:][^\s,{>+~]*)*\s*:(?:nth-(?:last-)?(?:child|of-type)|(?:first|last|only)-(?:child|of-type))\b/i;
const SECTION_SIBLING = /(?:^|[\s,{])section(?:[.#[:][^\s,{+~]*)*\s*[+~]/i;
...
export function positionalCss(source) {
	...
	for (const [what, re] of [
		['a structural pseudo-class on `section`', SECTION_POSITIONAL],
		['a sibling combinator between slides', SECTION_SIBLING],
		['a slide-index attribute the projection renumbers', SLIDE_INDEX_ATTR],
	]) {
		const hit = re.exec(css);
		if (hit) return { what, selector: hit[0].trim() };
	}
	return null;
}
