 ...
 *
 * MEASURED COST: 2 of the 150 decks in `examples/` carry CSS of their own — `finish-backdrops.md` and
 * `finish-override.md`, both a Studio-written `<style>` on slide 0 defining finish tokens. Both are
 * ALREADY refused by `crossSlideDrift` under some projection shapes, so this widens an existing
 * refusal rather than opening a new one; it does not shrink to zero, and the honest number is that
 * across the corpus test's six shapes it newly refuses 7 (deck, shape) pairs, all on those two decks.
 * A deck whose CSS is entirely benign is refused too, and that is the price. The remedy is one line in
 * the message, and it is the same trade this export already makes for `unprunable`, `unsplittable`,
 * `cross-slide` and `drifted`.
 *
 * @param {string} renderedHtml the engine's render of the deck, slide markup only
 * @param {string} fmText       the deck's front-matter body
...
export function authorCss(renderedHtml, fmText) {
	...
	// `rel` is matched loosely on purpose: `rel="stylesheet"`, `rel=stylesheet`, and the alternate and
	// preload spellings all pull CSS in, and this is a presence test, not a parser.
	if (/<link\b[^>]*\brel\s*=\s*["']?[^"'>]*stylesheet/i.test(html)) return { channel: 'link' };
	return null;
}
