...
function pruneTags(slideSrc, exported, reg) {
	// A SLIDE CAN CARRY MORE THAN ONE `_lens` COMMENT, and Lente reads only the first.
	// `parseSlideTags` and `applyTag` both stop there, so a second comment was invisible to
	// the prune and rode into the artifact verbatim — measured: `<!-- _lens: secret -->` on
	// the line below a legitimate tag survived a `--lens brief` export. It is dead weight to
	// every reader (nothing has ever read it) and a withheld view's id to a recipient, so it
	// goes first, before the tokens of the one comment that IS read are pruned.
...
export function projectForExport(source, requestedIds, options = {}) {
	...
	// `--lens full` ALONE IS THE IDENTITY, and pruning it was over-applying the rule.
	//
	...
	// slide — so there is no disclosure to close, and deleting the author's whole view
	// catalog from an envelope that exists to round-trip back into an editable deck is pure
	// loss. It also contradicted this file's own header, which promises `--lens full` is
	// byte-identical to no flag at all; that promise is kept again here.
	//
	// `full` alongside a named view (`--lens full,brief`) is a real selection and still
	// prunes: the carrier offers those two and discloses only those two.
