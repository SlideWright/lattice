 ...
 */
// Returns { patterns, rules }: the <pattern> markup carries NO colour — the slot
// fill + the overlay ink are applied via CSS RULES (returned separately, dropped
// into a <style>). WHY NOT `fill="var(…)"` ATTRIBUTES: `var()` inside an SVG
// PRESENTATION ATTRIBUTE is the newest-supported form and is unresolved on older
// WebKit/Safari (the attribute is invalid → SVG falls back to BLACK — the
// all-black pie on real iPhones, even though Chrome + current WebKit resolve it).
// `var()` in a CSS `fill:`/`stroke:` PROPERTY has far wider support, so route the
// colours through CSS. Geometry strokes inherit the <g>'s ink (SVG inheritance),
// so a per-shape stroke-dasharray attribute still applies.
function patternSet(prefix, count, fillVar, inkVar) {
  ...
  GEOMETRIES.slice(0, count).forEach(({ mode, svg }, i) => {
    ...
    const ink = mode === 'fill'
      ? `fill:var(${inkVar(n)});fill-opacity:.40`
      : `fill:none;stroke:var(${inkVar(n)});stroke-opacity:.45;stroke-width:1;stroke-linecap:square`;
...
function texturePatternDefs() {
  ...
  // this page-level <defs>), but the hue token --chart-catN is rooted on
  // `:root:root` by the a11y theme (so it resolves here), so paint that.
  // White-ish ink reads on the deeper greys.
  const chart = patternSet('latt-a11y-chart-tex', 8, n => `--chart-cat${n}`, () => '--cat-on-mark');
  return `<svg width="0" height="0" aria-hidden="true" style="position:absolute" class="latt-a11y-defs">` +
    `<defs>${cat.patterns}${chart.patterns}</defs>` +
    `<style>${cat.rules}${chart.rules}</style>` +
    `</svg>`;
}
