...
function probeSectionOverflow(s, clipSelector, TOL) {
  ...
  // The section's OWN content overflow, measured from its FLOWED children — NOT the
  // raw scroll dims. Raw scrollHeight/scrollWidth count DECORATIVE, out-of-flow chrome
  // that is intentionally placed against (or bleeding past) the slide edge: a finish
  // MARK/EDGE pseudo (::before/::after — not children at all, so excluded for free) or
  // an absolutely-positioned deck-logo moved/scaled toward a corner. Counting those as
  // content overflow false-trips the ring / export warning / autosplit (a moved logo or
  // a tilted, bleeding ghost numeral is placement, not overflow). So when the section
  // has measurable flowed children, derive its box overflow from their layout rects and
  // SKIP position:absolute/fixed children — the same rule the cell probe below uses for
  // an out-of-flow footer/watermark. Falls back to raw scroll dims for the pure-dims
  // unit fakes (no children / no getBoundingClientRect), keeping the legacy path exact.
  //
  // CRUCIAL: a flowed child can OVERFLOW ITS OWN BOX while the box still fits the section
  // (a height-constrained body — flex:1 / min-height:0 / fixed height — whose descendant
  // content spills, common for the STAGE_DEFERRED bodies that have no clip cell: charts,
  // gantt, timeline, diagrams…). Raw section.scrollHeight caught that via propagated scroll
  // extent; child LAYOUT rects alone would miss it. So each flowed child contributes BOTH
  // its box spill past the section AND its own internal overflow (scrollHeight−clientHeight,
  // folded onto its content bottom/right), restoring the detection the raw path gave —
  // without the decorative-bleed false positive.
  if (typeof s.getBoundingClientRect === 'function' && s.children && s.children.length) {
    ...
    for (let i = 0; i < s.children.length; i++) {
      ...
      if (typeof getComputedStyle === 'function') {
        ...
        if (pos === 'absolute' || pos === 'fixed') continue; // decorative placement, not content
      }
      const r = ch.getBoundingClientRect();
      if (r.width === 0 && r.height === 0) continue; // display:contents / empty
      seen++;
      // The child's own internal overflow (content past its box) spills below/right of the
      // box in the flow direction — fold it onto the child's content extent. Guarded for the
      // pure-dims fakes whose children carry no scroll/client numbers (treated as 0).
      ...
      const cBottom = r.bottom + (ivh > 0 ? ivh : 0);
      const cRight = r.right + (ivw > 0 ? ivw : 0);
    ...
    if (seen) {
      const overV = (sr.top - top > 0 ? sr.top - top : 0) + (bottom - sr.bottom > 0 ? bottom - sr.bottom : 0);
      const overH = (sr.left - left > 0 ? sr.left - left : 0) + (right - sr.right > 0 ? right - sr.right : 0);
      scrollH = clientH + (overV > 0 ? overV : 0);
      scrollW = clientW + (overH > 0 ? overH : 0);
    }
  }
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    // `scrollHeight - clientHeight` UNDER-reports a CENTERED (or bottom-anchored)
    ...
    // and take the larger: flex-start stays correct, center / flex-end get caught.
    ...
    if (typeof c.getBoundingClientRect === 'function' && c.children && c.children.length) {
      ...
      for (let j = 0; j < c.children.length; j++) {
        ...
        // Skip OUT-OF-FLOW children (position:absolute/fixed) — a footer, watermark, rail,
        // or pagination chip is placed against the SECTION, not flowed in the cell, so its
        // layout box legitimately sits far outside the cell box (a full-width footer docked
        // inside a half-width panel spills ~a panel-width to the left). That is placement,
        // not content overflow, and counting it false-positives the ring on a clean slide
        // (e.g. split-panel watermark at 4K). Their own overflow is caught when the section
        // box / their own clip cell is probed. Guarded for the unit fakes (no getComputedStyle).
        if (typeof getComputedStyle === 'function') {
          const pos = getComputedStyle(ch).position;
