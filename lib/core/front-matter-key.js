 ...
 *
 * TWO READERS NEED THE SAME ANSWER (HARD RULE #1). The CLI reads it to build the document. The
 * reader-view export reads it to answer a different question: does this deck carry CSS of its own at
 * all? It has to refuse a reducing projection if so, because CSS can select a slide by POSITION and a
 * projection moves every slide after the first withheld one. The other channels — a `<style>` or
 * `<link>` in the body, a `<style>` inside an inlined SVG — are visible as elements in the rendered
 * slide markup, but a front-matter `style:` never reaches it: the CLI injects that CSS into the
 * document itself, downstream of the render. So the export has to ask the front matter directly, and
 * a reader that disagreed with the one the document is built from would refuse decks that carry no
 * CSS while passing decks that do.
 *
 * `(?=^\S|$(?![\s\S]))` — stop at the next top-level YAML key or at the absolute end of the front
 * matter. JS regex has no `\Z` anchor, so end-of-input is `$` plus a negative lookahead.
 *
