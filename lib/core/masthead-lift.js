...
function transformMastheadSection(innerHtml, cls) {
  ...
  // The stage-Cell overflow fade Tile (design/forms.md §6) — a scrim CSS shows
  // only when the slide overflows (`.overflow`). Last child so it layers over
  // the stage content at its bottom edge.
  const fade = '<div class="tile-fade" aria-hidden="true"></div>';

  return `${header}${band}${rest}${fade}`;
}
