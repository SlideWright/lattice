...
function resolveAssetUrl(url, baseDir) {
  ...
  // decodeURI lets an already-%20-escaped path round-trip, but it throws on a
  // literal `%` in a filename (e.g. `100%.svg`) — guard it so one such asset
  // can't abort the whole deck render. pathToFileURL re-encodes either way.
  let rel = url;
  try { rel = decodeURI(url); } catch { /* malformed %-sequence: use the raw path */ }
