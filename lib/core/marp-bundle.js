...
/**
 * package.json for the bundle. The ONLY dependency is marp-cli — the Lattice
 * engine is shipped pre-bundled as `dist/lattice-emulator.js` (zero-install, see
 * the README's primary render route), so it is NOT an npm dependency. Listing
 * `@slidewright/lattice` here would break `npm install` outright: the package is
 * not published to the public registry, so npm 404s on it and the recipient
 * never gets marp-cli either.
 */
