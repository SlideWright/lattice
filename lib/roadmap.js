...
// Layout:
//   <div class="horizons">
//     <div class="horizon-card" style="--phase-accent:var(--cat-blue)">
...
function applyHorizons(inner) {
  ...
  // are commitments — one per phase column, by index.
  const bodyRows = parseRows(tbodyInner)
    .map(cells => ({
      label: cells[0] ? cells[0].inner.trim() : '',
      cells: cells.slice(1).map(c => c.inner.trim()),
    }))
    .filter(r => r.label !== '');
...
function transformRoadmapSection(inner, cls) {
  ...
  if (tokens.includes('horizons')) {
    // Skip if already transformed (idempotent across multiple passes).
    if (!/class="horizons"/.test(html)) {
      html = applyHorizons(html);
    }
  } else if (tokens.includes('status')) {
...
function applyToRenderedHtml(html) {
  ...
  while (i < html.length) {
    ...
    const hasModifier = ROADMAP_MODIFIERS.some(m => tokens.includes(m));
