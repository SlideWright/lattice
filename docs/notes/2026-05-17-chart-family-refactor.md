...
| The chart-family JS dispatcher | Lives in `lib/engine/` as engine machinery. Calls per-component `.transform.js` files. |
| The kernel protocol (`transformChartSection(html, classTokens)`) | Stays — every component's `<name>.transform.js` implements it. |
| The CHART_LAYOUTS registry | Stays in `lib/engine/chart-family.js`. Adding a new member adds (a) the registry entry, (b) a per-component `.transform.js`. |
