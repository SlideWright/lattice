...
A note on naming: this library was called the **Background Library** before 2026-05-17, with classes prefixed `bg-*`. The rename split decorations into two semantic families (tints and marks) and added a placement axis. See `docs/notes/2026-05-17-treatments-rename.md` for the rationale, the rename mapping table, and the architectural lessons from the catalog rebuild.
...
See `docs/references/gotchas.md` → "Chromium PDF output of CSS mask-image renders inconsistently across viewers" for the underlying browser/PDF behaviour.
