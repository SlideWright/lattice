...
The repository ships a gallery deck that exercises every layout — the
ground-truth fixture for what the renderer produces. Render it with the
bundled `lattice` CLI (no network required):
