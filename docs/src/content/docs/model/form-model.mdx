...
description: "How a Lattice slide is composed: a Frame divides the slide into Cells, each Cell holds a Tile, and the author's Component fills the content Tile. Plus the render chain for a single title, and where orientation fits."
---
...
A slide has exactly **one** Frame — the root chrome frame, or a **sovereign** frame (like `split-panel`) that claims the whole canvas and suppresses the chrome. Cells hold Tiles; **Frames do not nest inside content cells** — that recursion was considered and [rejected](https://github.com/slidewright/lattice/blob/main/engineering/decisions/2026-06-18-frame-recursion-cells.md).
