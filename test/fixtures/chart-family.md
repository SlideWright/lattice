---
marp: true
theme: indaco
paginate: true
---

<!-- _class: progress -->

`H1 2026 · Phase 1 readiness`

## Phase 1 readiness, by workstream.

Snapshot taken at 14:00 UTC. Status pills tint the bar fill.

- Orchestration mesh `92%` `on-track`
- Operations runbook `68%` `at-risk`
- Compliance audit pack `81%` `on-track`
- SDK polyglot parity `34%` `deferred`
- Dependency dashboard `12%` `blocked`

_Source: Linear · refreshed 2026-05-07_

---

<!-- _class: progress dark minimal -->

`Phase 1 readiness`

## Same data, dark + minimal.

- Orchestration mesh `92%` `on-track`
- SDK polyglot parity `34%` `deferred`

---

<!-- _class: timeline-list -->

`Mesh architecture`

## How the mesh architecture arrived in production.

Four stages over eighteen months.

1. `2024 Q3` Gateway round-trip
   - First production orchestration shipped.
2. `2025 Q1` Capability-pack proposal `decision`
   - Architecture review accepts the in-process model.
3. `2025 Q3` Pilot `pilot`
   - One internal team, one workload, one quarter.
4. `2026 Q1` Production `live`
   - Capability-pack signing live across all production tenants.

_Cross-functional sign-off · 2026-04-29_

---

<!-- _class: piechart donut -->

`H1 2026 · 1,840 person-hours`

## Where the engineering quarter went.

Wedges drawn proportionally; legend reads in author order.

- Orchestration mesh `46%`
- Operations runbook `22%`
- Compliance work `18%`
- Pilot support `9%`
- Toil and on-call `5%`
