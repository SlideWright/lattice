...
describe('lattice-engine: css emission (P1.1)', () => {
  ...
  test('orientationFor classifies aspect → name + canvas-scale (landscape stays 1)', () => {
    ...
    assert.deepEqual(orientationFor({ width: 1080, height: 1080 }), { name: 'square', scale: 1.35 });
    // Portrait scale ramps with how tall the canvas is (4:5 large → 9:19.5 capped).
    assert.deepEqual(orientationFor({ width: 1080, height: 1350 }), { name: 'portrait', scale: 1.70 }); // 4:5
    assert.deepEqual(orientationFor({ width: 1080, height: 1920 }), { name: 'portrait', scale: 1.94 }); // 9:16
    assert.deepEqual(orientationFor({ width: 1080, height: 2340 }), { name: 'portrait', scale: 2.0 });  // 9:19.5 (capped)
  ...
  test('orientationCss is empty for landscape (byte-identical) and scales portrait/square', () => {
    ...
    assert.match(story, /--canvas-scale:\s*1\.94/);
    assert.match(story, /justify-content:\s*center/);
    assert.match(orientationCss({ width: 1080, height: 1080 }), /--canvas-scale:\s*1\.35/);
  });
  ...
  test('composeCss bakes portrait geometry into @page and appends the orientation block', () => {
    ...
    assert.match(out, /section\s*\{\s*--canvas-scale:\s*1\.94;\s*justify-content:\s*center;\s*\}/);
  ...
  test('runtime injectOrientationStyle scale stays in step with orientationFor', () => {
    ...
    const runtimeScale = (aspect) => {
      ...
      return aspect >= 0.95 ? 1.35 : Math.min(2.0, Math.round((1.5 + (1 - aspect) * 1.0) * 100) / 100);
    };
    ...
    assert.match(block, /1\.5 \+ \(1 - aspect\) \* 1\.0/);
    assert.match(block, /Math\.min\(2\.0/);
  });
