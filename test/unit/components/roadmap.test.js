/**
 * Unit: lib/components/progression/roadmap/roadmap.transform.js — roadmap modifier transforms.
 *
...
const {
  ...
  applyToRenderedHtml,
  markerToState,
} = require('../../../lib/components/progression/roadmap/roadmap.transform');
...
describe('roadmap', () => {
  ...
  const STATUS_TABLE = (
    '<table>' +
      ...
      '<tbody>' +
        '<tr><td>Platform</td><td>[x] Model sign-off</td><td>[-] Per-team weighting</td></tr>' +
  ...
  const HORIZONS_TABLE = (
    '<table>' +
      ...
      '<tbody>' +
        '<tr><td>Platform</td><td>Model sign-off</td><td>Per-team weighting</td></tr>' +
  ...
  test('horizons: each card carries workstream rows with label + commitment', () => {
    ...
    assert.match(out, /<span class="row-text">Model sign-off<\/span>/);
    assert.match(out, /<span class="row-text">Per-team weighting<\/span>/);
  ...
  test('dispatch: tags state markers on plain roadmap sections too (universal)', () => {
    ...
    assert.ok(!out.includes('[x] Model sign-off'));
  });
  ...
  test('dispatch: handles multiple sections in the same document', () => {
    ...
    assert.match(out, /\[x\] Model sign-off/);
  });
});
