...
describe('frame-css geometry helpers', () => {
	test('slideBox() defaults to the HD box', async () => {
		...
		assert.equal(slideBox(), '.lattice>section{width:1280px;height:720px}');
	});

	test('slideBox(w, h) sizes the box to the deck geometry (4K)', async () => {
		const { slideBox } = await load();
		assert.equal(slideBox(3840, 2160), '.lattice>section{width:3840px;height:2160px}');
	});
	...
	test('singleSlideFrame() pins viewport + wrapper + box to one geometry', async () => {
		...
		assert.match(hd, /\.lattice\{width:1280px;height:720px\}/);
		...
		assert.match(k, /\.lattice\{width:3840px;height:2160px\}/);
