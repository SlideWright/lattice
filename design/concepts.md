...
| **Substance** | Content | what fills it? | engine maintainers | `substance` field + the per-substance render kernel; 4 sources (prose · structure · series · graph) | `design-system.md` §5 |
| **Finish** | Style | what should it feel like? | theme designers | palette tokens (`themes/*.css`) + variant tiers (`_universal.css`, `_semi-universal.css`, per-component) | `design-system.md` §6.5 · `theming.md` |
...
tree of three nouns — slicers that make boxes, fillers that fill them, and the
slot between. Three, not two, because each has an independent axis of change
(`forms.md` §3, the SOLID reading).
...
Frame-generation guardrail. It is what keeps a footer Tile out of a masthead Cell. The non-negotiable Cell contract: **every Cell resolves to a
deterministic px box appropriate to the active resolution, independent of its
content, and clips rather than bleeds** (`forms.md` §6).

**Today's catalogs:** 10 Frames, 9 Cells, 14 Tiles (`dist/docs/forms.json`).
...
**Today's catalog:** 52 components across 12 buckets (`dist/docs/components.json`).

---

## 6. The relationships, named
...
| Component → Frame | **selects** | `form` field (one of 12 Frame types) |
...
- **"Which Frame should I pick?"** → the 12 Frame types (`forms.json`); a
...
## 8. Encoded vs. prose — current state
...
- **The cross-level relationship graph itself** — the §2 lattice, the §6 edge
