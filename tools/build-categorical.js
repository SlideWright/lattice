...
// ── CLI ──────────────────────────────────────────────────────────────────────
function cfgFor(css, argv) {
  const cfg = { ...CFG };
  const mc = css.match(/CATEGORICAL-CFG\s+([^*]+)/);
  if (mc) for (const tok of mc[1].trim().split(/\s+/)) { const [k, v] = tok.split('='); if (k in cfg && v) cfg[k] = parseFloat(v); }
  ...
  for (const [flag, key] of Object.entries(flagMap)) { const i = argv.indexOf(`--${flag}`); if (i >= 0 && argv[i + 1]) cfg[key] = parseFloat(argv[i + 1]); }
  return cfg;
}

function buildBlock(css, cfg) {
  const anchors = [];
  for (let i = 1; i <= 12; i++) {
...
const SENTINEL_RE = new RegExp(`${esc(START)}[\\s\\S]*?${esc(END)}`);

/** Regenerate one theme file. Returns {status:'patched'|'ok'|'stale'|'skip'}. */
function processFile(file, { dry, check, argv }) {
  const css = fs.readFileSync(file, 'utf8');
  if (!css.includes(START) || !css.includes(END)) return { status: 'skip' };
  const block = buildBlock(css, cfgFor(css, argv));
  if (block == null) return { status: 'skip' };
  const next = `${START}\n${block}\n  ${END}`;
  if (dry) { console.log(block); return { status: 'ok' }; }
  const current = (css.match(SENTINEL_RE) || [''])[0];
  if (current === next) return { status: 'ok' };
  if (check) return { status: 'stale' };
  fs.writeFileSync(file, css.replace(SENTINEL_RE, next));
...
if (require.main === module) {
  ...
  const all = argv.includes('--all') || !file; // no explicit file ⇒ all anchored themes

  const files = all
    ? fs.readdirSync(path.join(__dirname, '..', 'themes')).filter(f => f.endsWith('.css'))
        .map(f => path.join(__dirname, '..', 'themes', f))
    : [file];

  let stale = 0, patched = 0;
  for (const f of files) {
    const r = processFile(f, { dry, check, argv });
    if (r.status === 'stale') { stale++; console.error(`STALE: ${path.basename(f)} — regenerate with \`npm run categorical:build\``); }
    else if (r.status === 'patched') { patched++; if (!all) console.log(`patched ${path.basename(f)} (12 anchors → 24 same-hue tokens)`); }
  }
  if (all && !dry) console.log(check ? (stale ? `${stale} theme(s) stale` : 'categorical: all anchored themes up to date') : `categorical: regenerated ${patched} theme(s)`);
