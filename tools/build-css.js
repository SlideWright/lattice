...
/**
 * Stamp the engine's size registry into a stylesheet's `@theme` comment, as the
 * `@size` directives Marp reads.
 *
 * The engine owns the page box (lib/engine/sizes.js) and resolves geometry from
 * that table, so no SOURCE stylesheet carries `@size` any more. Marp is a
 * different consumer: marp-cli and the Marp for VS Code extension read geometry
 * out of the theme comment, and they only ever see dist/ — so the requirement is
 * satisfied where it actually applies, at the artifact boundary, instead of in 33
 * hand-maintained files. The directives are inserted immediately after the
 * `@theme` line, which is where they lived before, so the generated bytes are
 * unchanged by the move. `minifyCss` then preserves the whole comment on the
 * minified path (its `DIRECTIVE_RE` is exactly this contract).
 *
 * See engineering/decisions/2026-08-16-size-registry-ownership.md §5.
 */
const THEME_DIRECTIVE_LINE = /(\/\*[\s\S]*?@theme[ \t]+\S+[^\S\r\n]*)(\r?\n)/;
function stampSizeDirectives(css) {
  if (!THEME_DIRECTIVE_LINE.test(css)) {
    throw new Error('build-css: no `@theme` directive comment to stamp `@size` into — Marp would get a theme with no geometry');
  }
  return css.replace(THEME_DIRECTIVE_LINE, (_m, head, nl) => `${head}${nl}${sizeBlock()}${nl}`);
}
...
const MIN_BANNER =
  '/* dist/lattice.min.css — GENERATED by tools/build-css.js (minified). Do not edit. SPDX-License-Identifier: AGPL-3.0-only. (c) 2025-2026 SlideWright */';
...
// Stable, machine-checked sentinel emitted directly above LAYER_DECLARATION in the
// bundle so a reader of dist/lattice.css (human OR agent) learns at the point of
// confusion that the layers are INERT — the declaration reads active but governs
// nothing. HARD RULE #26 gate (checkCascadeLayers) asserts this exact token is
// emitted here; keep the `LATTICE-LAYERS-INERT` token verbatim if you reword.
const LAYER_INERT_NOTE = [
  '/* LATTICE-LAYERS-INERT — the @layer order below is RESERVED, NOT ACTIVE. No rule in',
  '   this bundle is wrapped in a layer, so plain SOURCE ORDER (not layer order) decides',
  '   the cascade. Do NOT wrap rules in @layer: unlayered beats layered regardless of',
  '   specificity, so a partially-layered rule silently loses (the rule-3 trap). Full',
  '   activation is vetoed while export-to-Marp ships marp-core\'s unlayered scaffold —',
  '   see engineering/cascade.md (HARD RULE #26). */',
].join('\n');
...
// Until fetched, the reference is inert and color emoji uses the system font.
function emojiFontFaceCss() {
  return (
    '/* dist/lattice-emoji.css — GENERATED by tools/build-css.js. Do not edit.\n' +
    ' * Opt-in full-offline color emoji for air-gapped environments. Link this\n' +
...
function bundle() {
  ...
  // use — see file header for why @layer is inactive today). The inert-note
  // sentinel rides directly above it so the dist reader isn't misled (#26).
  parts.push(LAYER_INERT_NOTE);
...
module.exports = {
  bundle, buildThemes, main, OUTPUT, MIN_OUTPUT, formsSlicingCss,
  LAYER_DECLARATION, LAYER_INERT_NOTE,
};
