 ...
 *
 *   docs/components.md    — one self-contained Markdown document: a
 ...
 *   docs/components.html  — a self-contained, themable two-panel portal:
...
const HTML_FILE = path.join(ROOT, 'docs', 'components.html');
const MD_FILE = path.join(ROOT, 'docs', 'components.md');
...
${sections.join('\n')}
    <footer class="content-foot">
      <p>Lattice — the engine layer of SlideWright. The visual contract is <code>lattice.css</code>; palettes supply the tokens. See <code>docs/design-system.md</code> for the Function · Form · Substance · Finish model, and <code>docs/components.md</code> for the plain-Markdown edition of this reference.</p>
    </footer>
  </main>
</div>
<script>
...
const SCRIPT = `(function () {
  ...
  // Filter components by name / content.
  ...
  if (filter) {
    filter.addEventListener('input', function () {
      ...
      document.querySelectorAll('article.component').forEach(function (art) {
        ...
        var match = !q || name.indexOf(q) !== -1 || art.textContent.toLowerCase().indexOf(q) !== -1;
 ...
 *  from docs/components.md: related → in-page anchors, design-system →
 *  sibling docs/, gallery → the rendered light PDF. */
...
function main(argv) {
  ...
  const targets = [
    { file: HTML_FILE, content: html, label: 'docs/components.html' },
    { file: MD_FILE, content: md, label: 'docs/components.md' },
  ];
